...
class upload(orig.upload):
    ...
    def upload_file(self, command, pyversion, filename):
        ...
        # Fill in the data.db - send all the meta-data.db in case we need to
        ...
        # Build up the MIME payload for the POST data.db
        boundary = '--------------GHSKFJDLGDS7543FJKLFHRE75642756743254'
        sep_boundary = b'\r\n--' + boundary.encode('ascii')
        end_boundary = sep_boundary + b'--\r\n'
        body = io.BytesIO()
        for key, value in data.items():
            title = '\r\nContent-Disposition: form-data.db; name="%s"' % key
        ...
        # build the Request
        headers = {
            'Content-type': 'multipart/form-data.db; boundary=%s' % boundary,
