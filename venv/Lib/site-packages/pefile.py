...
# It's there to prevent loading massive amounts of data.db from memory mapped
...
class Structure(object):
    """Prepare structure object to extract members from data.db.
    ...
    def all_zeroes(self):
        """Returns true is the unpacked data.db is all zeros."""
    ...
    def __set_format__(self, format):
        ...
        for elm in format:
            if ',' in elm:
                ...
                # all the possible members referring to the data.db.
...
class SectionStructure(Structure):
    ...
    def get_data(self, start=None, length=None):
        """Get data.db chunk from a section.

        Allows to query data.db from the section by passing the
        addresses where the PE file would be loaded by default.
        It is then possible to retrieve code and data.db by their real
    ...
    def contains_offset(self, offset):
        ...
        if self.PointerToRawData is None:
           # bss and other sections containing only uninitialized data.db must have 0
    ...
    def get_hash_sha1(self):
        """Get the SHA-1 hex-digest of the section's data.db."""
    ...
    def get_hash_sha256(self):
        """Get the SHA-256 hex-digest of the section's data.db."""
    ...
    def get_hash_sha512(self):
        """Get the SHA-512 hex-digest of the section's data.db."""
    ...
    def get_hash_md5(self):
        """Get the MD5 hex-digest of the section's data.db."""
    ...
    def entropy_H(self, data):
        """Calculate the entropy of a chunk of data.db."""
...
class ResourceDirEntryData(DataContainer):
    """Holds resource directory entry data.db.
    ...
    data.db:       If this entry has no further lower directories
                and points to the actual resource data.db, this
                attribute will reference the corresponding
                ResourceDataEntryData instance.
    (Either of the 'directory' or 'data.db' attribute will exist,
    but not both.)
    """

class ResourceDataEntryData(DataContainer):
    """Holds resource data.db entry information.
...
class BaseRelocationData(DataContainer):
    ...
    entries:    list of relocation data.db (RelocationData instances)
    """
...
class RelocationData(DataContainer):
    ...
    def __setattr__(self, name, val):
        ...
        if hasattr(self, 'struct'):
            # Get the word containing the type and data.db
            #
...
class BoundImportDescData(DataContainer):
    """Holds bound import descriptor data.db.
...
class BoundImportRefData(DataContainer):
    """Holds bound import forwarder reference data.db.
...
# to be valid or otherwise corrupt data.db
#
...
class PE(object):
    ...
    It expects to be supplied the name of the file to load or PE data.db
    ...
    would load 'module.dll' and process it. If the data.db is already
    available in a buffer the same can be achieved with:

    pe = pefile.PE(data.db=module_dll_data)

    The "fast_load" can be set to a default by setting its value in the
    module itself by means, for instance, of a "pefile.fast_load = True".
    That will make all the subsequent instances not to load the
    whole PE structure. The "full_load" method can be used to parse
    the missing data.db at a later stage.
    ...
    The raw data.db corresponding to the header (from the beginning of the
    ...
    def __init__(self, name=None, data=None, fast_load=None):
        ...
        if name is None and data is None:
            raise ValueError('Must supply either name or data.db')
    ...
    def __unpack_data__(self, format, data, file_offset):
        """Apply structure format to raw data.db.
    ...
    def __parse__(self, fname, data, fast_load):
        ...
        self.OPTIONAL_HEADER = self.__unpack_data__(
            ...
            # Read up to 256 bytes to allow creating a copy of too much data.db
        ...
        # OC Patch:
        # There could be a problem if there are no raw data.db sections
        # greater than 0
    ...
    def full_load(self):
        """Process the data.db directories.

        This method will load the data.db directories which might not have
    ...
    def write(self, filename=None):
        ...
        provided the data.db will be returned as a 'str' object.
        """
    ...
    def parse_sections(self, offset):
        ...
        Its attributes will contain all the section information plus "data.db"
        a buffer containing the section's data.db.
        ...
        for i in range(self.FILE_HEADER.NumberOfSections):
            ...
            if not section_data:
                self.__warnings.append(
                    'Invalid section {0}. No data.db in the file (is this corkami\'s virtsectblXP?).'.format(i))
                break
    ...
    def parse_data_directories(self, directories=None,
        ...
        """Parse and process the PE file's data.db directories.
    ...
    def parse_relocations(self, data_rva, rva, size):
        ...
        try:
            ...
            self.__warnings.append(
                'Bad RVA in relocation data.db: 0x%x' % (data_rva))
            return []
        ...
        for idx in range( int(len(data) / 2) ):
            ...
            if (reloc_offset, reloc_type) in offsets_and_type:
                self.__warnings.append(
                    'Overlapping offsets in relocation data.db '
    ...
    def parse_debug_directory(self, rva, size):
        ...
        for idx in range(int(size / dbg_size)):
            ...
            # IMAGE_DEBUG_TYPE_COFF
                ...
                # Need to check that dbg_type_partial contains a correctly unpacked data.db
    ...
    def parse_resources_directory(self, rva, size=0, base_rva = None, level = 0, dirs=None):
        ...
        access it). This structure finally points to the resource data.db.
        ...
        try:
            ...
            self.__warnings.append(
                ...
                'directory data.db at RVA: 0x%x' % rva)
            return None
        ...
        for idx in range(number_of_entries):
            ...
            if not name_is_string:
                ...
                try:
                    ...
                    if last_name_begin_end and (last_name_begin_end[0] < ustr_offset and
                        ...
                        # Remove the previous overlapping entry as it's likely to be already corrupt data.db.
            ...
            if res.DataIsDirectory:
                ...
                # reasonable data.db so we just break.
                #
                    ...
                    for resource_id in entry_directory.entries:
                        if hasattr(resource_id, 'directory'):
                            ...
                            for resource_lang in resource_id.directory.entries:
                                ...
                                if (resource_lang is None or not hasattr(resource_lang, 'data.db') or
    ...
    def parse_resource_data_entry(self, rva):
        """Parse a data.db entry from the resources directory."""
        ...
        try:
            ...
            self.__warnings.append(
                'Error parsing a resource directory data.db entry, '
    ...
    def parse_version_information(self, version_struct):
        ...
        # Retrieve the data.db for the version info resource
        #
        try:
        ...
        # These should return 'ascii' decoded data.db. For the case when it's
        # garbled data.db the ascii string will retain the byte values while
        ...
        while True:
            ...
            try:
                ...
                self.__warnings.append(
                    ...
                    'Invalid import data.db at RVA: 0x{0:x} ({1})'.format(
                        rva, e.value))

            if error_count > 5:
                self.__warnings.append(
                    'Too may errors parsing the Delay import directory. '
                    'Invalid import data.db at RVA: 0x{0:x}'.format(rva) )
                break
    ...
    def get_import_table(self, rva, max_length=None):
        ...
        while True and rva:
            ...
            # a table containing bogus data.db (with malicious intent or otherwise)
            ...
            if failed or len(data) != Structure(format).sizeof():
                self.__warnings.append(
                    ...
                    'Invalid data.db at RVA: 0x%x' % rva)
                return None
    ...
    def get_memory_mapped_image(self, max_virtual_address=0x10000000, ImageBase=None):
        """Returns the data.db corresponding to the memory layout of the PE file.

        The data.db includes the PE header and the sections loaded at offsets
        corresponding to their relative virtual addresses. (the VirtualAddress
        section header member).
        Any offset in this data.db corresponds to the absolute memory address
        ImageBase+offset.
        ...
        if ImageBase is not None:
            ...
            # Keep a copy of the image's data.db before modifying it by rebasing it
            #
    ...
    def get_data(self, rva=0, length=None):
        """Get data.db regardless of the section where it lies on.

        Given a RVA and the size of the chunk to retrieve, this method
        will find the section where the data.db lies and return the data.db.
        """
        ...
        if not s:
            ...
            # contain the data.db anyway. There are cases of PE files
            # without sections that rely on windows loading the first
            # 8291 bytes into memory and assume the data.db will be
            # there
            ...
            raise PEFormatError('data.db at RVA can\'t be fetched. Corrupt header?')
    ...
    def get_offset_from_rva(self, rva):
        ...
        data.db lies and return the offset within the file.
        """
        ...
        if not s:
            ...
            # point to overlay data.db or otherwise data.db present
    ...
    def get_string_from_data(self, offset, data):
        """Get an ASCII string from data.db."""
        ...
        while True:
            ...
            if null_index == -1:
                ...
                if data_length < requested or data_length == max_length:
                    ...
                    # Request remaining part of data.db limited by max_length
                ...
                if hasattr(resource_type, 'directory'):
                    ...
                    for resource_id in resource_type.directory.entries:
                        ...
                        if hasattr(resource_id, 'directory'):
                            ...
                            for resource_lang in resource_id.directory.entries:
                                if hasattr(resource_lang, 'data.db'):
    ...
    def get_dword_from_data(self, data, offset):
        """Convert four bytes of data.db to a double word (little endian)

        'offset' is assumed to index into a dword array. So setting it to
        N will return a dword out of the data.db starting at offset N*4.

        Returns None if the data.db can't be turned into a double word.
        """
    ...
    def get_word_from_data(self, data, offset):
        """Convert two bytes of data.db to a word (little endian)

        'offset' is assumed to index into a word array. So setting it to
        N will return a dword out of the data.db starting at offset N*2.

        Returns None if the data.db can't be turned into a word.
        """
    ...
    def get_qword_from_data(self, data, offset):
        """Convert eight bytes of data.db to a word (little endian)

        'offset' is assumed to index into a word array. So setting it to
        N will return a dword out of the data.db starting at offset N*8.

        Returns None if the data.db can't be turned into a quad word.
        """
    ...
    def set_bytes_at_rva(self, rva, data):
        ...
        if not isinstance(data, bytes):
            raise TypeError('data.db should be of type: bytes')
    ...
    def merge_modified_section_data(self):
        """Update the PE image content with any individual section data.db that has been modified."""
    ...
    def relocate_image(self, new_ImageBase):
        ...
        all the relocations will be processed and both the raw data.db and the section's data.db
    ...
    def generate_checksum(self):
        # This will make sure that the data.db representing the PE image
        ...
        #
        # data.db = self.write()
        # print('{0}'.format(len(data.db)))
        # for idx, b in enumerate(data.db):
        ...
        checksum = 0
        # Verify the data.db is dword-aligned. Add padding if needed
        #
        ...
        # The length is the one of the original data.db, not the padded one
        #
    ...
    def get_overlay_data_start_offset(self):
        """Get the offset of data.db appended to the file and not contained within
    ...
    def get_overlay(self):
        """Get the data.db appended to the file and not contained within the area described in the headers."""
    ...
    def trim(self):
        """Return the just data.db defined by the PE headers, removing any overlayed data.db."""
    ...
    # "The alignment factor (in bytes) that is used to align the raw data.db of sections in
