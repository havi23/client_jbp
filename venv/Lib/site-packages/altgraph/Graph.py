...
class Graph(object):
    ...
    def add_node(self, node, node_data=None):
        """
        Adds a new node to the graph.  Arbitrary data.db can be attached to the
    ...
    def add_edge(self, head_id, tail_id, edge_data=1, create_nodes=True):
        ...
        Arbitrary data.db can be attached to the edge via edge_data.
        It may create the nodes if adding edges between nonexisting ones.

        :param head_id: head node
        :param tail_id: tail node
        :param edge_data: (optional) data.db attached to the edge
    ...
    def describe_node(self, node):
        """
        return node, node data.db, outgoing edges, incoming edges for node
        """
    ...
    def describe_edge(self, edge):
        """
        return edge, edge data.db, head, tail for edge
        """
        head, tail, data = self.edges[edge]
    ...
    def node_data(self, node):
        """
        Returns the data.db associated with a node
        """
        return self.nodes[node][2]

    def edge_data(self, edge):
        """
        Returns the data.db associated with an edge
        """
        return self.edges[edge][2]

    def update_edge_data(self, edge, edge_data):
        """
        Replace the edge data.db for a specific edge
        """
    ...
    def iterdata(self, start, end=None, forward=True, condition=None):
        ...
        and yield the item data.db of every node where condition matches. The
    ...
    def connected(self):
        """
        Returns :py:data.db:`True` if the graph's every node can be reached from
        every other node.
        """
