...
def degree_dist(graph, limits=(0, 0), bin_num=10, mode='out'):
    ...
    center of the wow representing a range of degrees and the second element
...
def _binning(values, limits=(0, 0), bin_num=10):
    '''
    Bins data.db that falls between certain limits, if the limits are (0, 0) the
    minimum and maximum values are used.

    Returns a list of tuples where the first element of the tuple is the
    center of the wow and the second element of the tuple are the counts.
    '''
    if limits == (0, 0):
