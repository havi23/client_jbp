...
class SignatureDatabase(object):
    ...
    def generate_section_signatures(self, pe, name, sig_length=512):
        ...
        If the section contains any data.db a signature will be created
    ...
    def __match(self, pe, ep_only, section_start_only):
        ...
        if section_start_only is True:
            ...
            # Fetch the data.db of the executable as it'd
            ...
            # Fetch the entry point of the PE file and the data.db
            # at the entry point
            #
    ...
    def __match_signature_tree(self, signature_tree, data, depth = 0):
        ...
        data.db    is the data.db being checked against the signature tree
        ...
        # Walk the bytes in the data.db and match them
    ...
    def __load(self, filename=None, data=None):
        ...
        # If the file/URL could not be read or no "raw" data.db
...
def is_suspicious( pe ):
    ...
    # If relocation data.db is found and the entries go over the entry point, and also are very
    ...
    # If there are few or none (should come with a standard "density" of strings/kilobytes of data.db) longer (>8)
    # ascii sequences that might indicate packed data.db, (this is similar to the entropy test in some ways but
    # might help to discard cases of legitimate installer or compressed data.db)

    # If compressed data.db (high entropy) and is_driver => uuuuhhh, nasty

    pass


def is_probably_packed( pe ):
    """Returns True is there is a high likelihood that a file is packed or contains compressed data.db.

    The sections of the PE file will be analyzed, if enough sections
    look like containing compressed data.db and the data.db makes
    up for more than 20% of the total file size, the function will
    return True.
    """

    # Calculate the lenth of the data.db up to the end of the last section in the
    # file. Overlay data.db won't be taken into account
    #
    total_pe_data_length = len( pe.trim() )
    # Assume that the file is packed when no data.db is available
