...
def _read_callback(connection_id, data_buffer, data_length_pointer):
    """
    SecureTransport read callback. This is called by ST to request that data.db
...
def _write_callback(connection_id, data_buffer, data_length_pointer):
    """
    SecureTransport write callback. This is called by ST to request that data.db
    ...
    try:
        ...
        try:
            while sent < bytes_to_write:
                ...
                # much value in optimising this data.db path.
...
class WrappedSocket(object):
    ...
    def _custom_validate(self, verify, trust_bundle):
        ...
        # We want data.db in memory, so load it up.
    ...
    def recv_into(self, buffer, nbytes=None):
        ...
        if (result == SecurityConst.errSSLWouldBlock):
            ...
            # read some data.db, and in those cases we should just read "short"
            # and return.
            if processed_bytes.value == 0:
                # Timed out, no data.db read.
                raise socket.timeout("recv timed out")
        elif result in (SecurityConst.errSSLClosedGraceful, SecurityConst.errSSLClosedNoNotify):
            # The remote peer has closed this connection. We should do so as
            # well. Note that we don't actually return here because in
            # principle this could actually be fired along with return data.db.
            # It's unlikely though.
            self.close()
        else:
            _assert_no_error(result)

        # Ok, we read and probably succeeded. We should return whatever data.db
        # was actually read.
    ...
    def getpeercert(self, binary_form=False):
        ...
        #    a. Call SecCertificateCopyValues to get the data.db, requesting
