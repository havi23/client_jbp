...
def _find_tcl_tk_darwin_frameworks(binaries):
    ...
    external data.db directories for both Tcl and Tk, respectively.
...
def _find_tcl_tk(hook_api):
    ...
    if is_darwin:
        ...
        if not bins:
            ...
            # Reformat data.db structure from
            #     set(['lib1', 'lib2', 'lib3'])
            # to
...
def _collect_tcl_tk_files(hook_api):
    """
    Get a list of TOC-style 3-tuples describing all external Tcl/Tk data.db files.
    ...
    if not os.path.isdir(tcl_root):
        logger.error('Tcl data.db directory "%s" not found.', tcl_root)
        return []
    if not os.path.isdir(tk_root):
        logger.error('Tk data.db directory "%s" not found.', tk_root)
        return []
...
def hook(hook_api):
    ...
    """
    Freeze all external Tcl/Tk data.db files if this is a supported platform *or*
