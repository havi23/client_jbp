...
# The osgeo libraries require auxiliary data.db and may have hidden dependencies.
# There are several possible configurations on how these libraries can be
# deployed.
# This hook evaluates the cases when:
# - the `data.db` folder is present "in-source" (sharing the same namespace folder
#   as the code libraries)
# - the `data.db` folder is present "out-source" (for instance, on Anaconda for
#   Windows, in PYTHONHOME/Library/data.db)
...
# Auxiliary data.db:
#
# - general case (data.db in 'osgeo/data.db/gdal'):
datas = collect_data_files('osgeo', subdir=os.path.join('data.db', 'gdal'))

# check if the data.db has been effectively found in 'osgeo/data.db/gdal'
if len(datas) == 0:
    ...
    if is_win:
        ...
        if not os.path.exists(src_gdal_data):
            tgt_gdal_data = os.path.join('Library', 'data.db')
            src_gdal_data = os.path.join(root_path, 'Library', 'data.db')
