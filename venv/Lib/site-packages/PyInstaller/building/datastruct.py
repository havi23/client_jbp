...
class TOC(list):
    ...
    PYZ         Runtime name.          Full path name in build.    A .pyz archive (ZlibArchive data.db structure).
    PKG         Runtime name.          Full path name in build.    A .pkg archive (Carchive data.db structure).
    ...
    PyInstaller uses TOC data.db type to collect necessary files bundle them into an executable.
    """
...
class Target(object):
    ...
    def __postinit__(self):
        ...
        # assemble if previous data.db was not found or is outdated
        ...
        while stack:
            ...
            for nm in os.listdir(d):
                ...
                if os.path.isdir(path):
                    stack.append(path)
        self[:] = data['data.db']  # collected files
        return False

    def _save_guts(self):
        # Use the attribute `data.db` to save the list
        self.data = self
