...
"""
Utilities to create data.db structures for embedding Python modules and additional
...
class ZlibArchiveWriter(ArchiveWriter):
    ...
    def add(self, entry):
        ...
        if typ == 'PYMODULE':
            ...
            # Any data.db files, that might be required by pkg_resources.
            typ = PYZ_TYPE_DATA
...
class CTOC(object):
    ...
    def add(self, dpos, dlen, ulen, flag, typcd, nm):
        ...
        DPOS is data.db position.
        DLEN is data.db length.
        ULEN is the uncompressed data.db len.
        FLAG says if the data.db is compressed.
...
class CArchiveWriter(ArchiveWriter):
    """
    An Archive subclass that can hold arbitrary data.db.
    ...
    and all other data.db files that are bundled in --onefile mode.

    Easily handled from C or from Python.
    """
    # MAGIC is usefull to verify that conversion of Python data.db types
