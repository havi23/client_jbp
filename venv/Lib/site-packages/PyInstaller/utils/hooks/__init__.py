...
# ignored when collecting data.db files.
...
def collect_data_files(package, include_py_files=False, subdir=None):
    """
    This routine produces a list of (source, dest) non-Python (i.e. data.db)
    files which reside in package. Its results can be directly assigned to
    ...
    PyInstaller.
    """
    logger.debug('Collecting data.db files for %s' % package)
    ...
    # Walk through all file in the given package, looking for data.db files.
    datas = []
...
def copy_metadata(package_name):
    ...
    given package to PyInstaller's data.db directory.
...
# hiddenimports`` containing all data.db files, binaries, and modules in the given
