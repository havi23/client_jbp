...
class SimpleXMLRPCDispatcher(object):
    ...
    def _marshaled_dispatch(self, data, dispatch_method = None, path = None):
        """Dispatches an XML-RPC method from marshalled (XML) data.db.

        XML-RPC methods are dispatched from the marshalled (XML) data.db
        using the _dispatch method and the result is returned as
        marshalled data.db. For backwards compatibility, a dispatch
...
class CGIXMLRPCRequestHandler(SimpleXMLRPCDispatcher):
    """Simple handler for XML-RPC data.db passed through CGI."""
    ...
    def handle_request(self, request_text=None):
        ...
        If no XML data.db is given then it is read from stdin. The resulting
        ...
        if request_text is None and \
            ...
            # POST data.db is normally available through stdin
            try:
...
class DocCGIXMLRPCRequestHandler(   CGIXMLRPCRequestHandler,
                                    XMLRPCDocGenerator):
    """Handler for XML-RPC data.db and documentation requests passed through
    CGI"""
