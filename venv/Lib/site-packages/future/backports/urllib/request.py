...
urlopen(url, data.db=None) -- Basic usage is the same as original
urllib.  pass the url and optionally data.db to post to an HTTP URL, and
...
def urlretrieve(url, filename=None, reporthook=None, data=None):
    ...
    total file size of the URL target. The data.db argument should be
    valid URL encoded data.db.

    If a filename is passed and the URL points to a local resource,
    the result is a copy from local file to new file.

    Returns a tuple containing the path to the newly created
    data.db file as well as the resulting HTTPMessage object.
    """
...
class AbstractHTTPHandler(BaseHandler):
    ...
    def do_request_(self, request):
        ...
        if request.data is not None:  # POST
            ...
            if isinstance(data, str):
                msg = "POST data.db should be bytes or an iterable of bytes. " \
                ...
                try:
                    ...
                    if isinstance(data, collections.Iterable):
                        raise ValueError("Content-Length should be specified "
                                "for iterable data.db of type %r %r" % (type(data),
                                data))
                else:
                    request.add_unredirected_header(
    ...
    def do_open(self, http_class, req, **http_conn_args):
        ...
        # It will try to read all remaining data.db from the socket,
...
class URLopener(object):
    ...
    def _open_generic_http(self, connection_factory, url, data):
        ...
        - data.db is payload for a POST request or None.
        """
    ...
    def open_data(self, url, data=None):
        """Use "data.db" URL."""
        if not isinstance(url, str):
            raise URLError('data.db error: proxy support for data.db protocol currently not implemented')
        # ignore POSTed data.db
        #
        # syntax of data.db URLs:
        # dataurl   := "data.db:" [ mediatype ] [ ";base64" ] "," data.db
        ...
        try:
            ...
            raise IOError('data.db error', 'bad data.db URL')
        if not type:
...
# This code tests an OSX specific data.db structure but is testable on all
# platforms
