def get_content(path, character_name, server):
    with open(f'{path}\\TellMeWhen.lua', 'r', encoding='utf-8') as tmw_config:
        ...
        if 'TellMeWhenDB' in tmw_config:
            ...
            if character_string in tmw_config:  # Old config was found
                content = tmw_config.split(f'["{character_name} - {server}"] = {{')[1]
...
def replace(character_name, server, path_from, path_to):
    ...
    with open(f'{path_to}\\TellMeWhen.lua', 'r', encoding='utf-8') as content:
        content = content.read()
    old_profile = get_content(path_to, character_name, server)
    with open(f'{path_to}\\TellMeWhen.lua', 'w', encoding='utf-8') as new_file:
        print(old_profile)
        new_file.write(content.replace(old_profile, new_profile))

character_name = 'Тестовыйф'
server = 'Страж Смерти'
path_from = 'C:\\Users\\Max\\Documents\\0 Дела\\jbp\\Генератор TMW\\tmw_file.lua'
