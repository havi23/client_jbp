...
-- GLOBALS: LibStub
-- GLOBALS: JBPOptDB
-- GLOBALS: JUSTBECOMEPRO_VERSION_FULL, JUSTBECOMEPRO_VERSIONNUMBER, JUSTBECOMEPRO_MAXROWS
...
-- GLOBALS: GameTooltip
-- GLOBALS: UIParent, WorldFrame, JustBecomePro_IconEditor, GameFontDisable, GameFontHighlight, CreateFrame, collectgarbage 
...
---------- Globals ----------
--GLOBALS: BINDING_HEADER_JUSTBECOMEPRO, BINDING_NAME_JUSTBECOMEPRO_ICONEDITOR_UNDO, BINDING_NAME_JUSTBECOMEPRO_ICONEDITOR_REDO
BINDING_HEADER_JUSTBECOMEPRO = "JustBecomePro"
BINDING_NAME_JUSTBECOMEPRO_ICONEDITOR_UNDO = L["UNDO"]
BINDING_NAME_JUSTBECOMEPRO_ICONEDITOR_REDO = L["REDO"]
...
function JBP:ConvertContainerToScrollFrame(container, exteriorScrollBarPosition, scrollBarXOffs, scrollBarSizeX, leftSide)
	local name = container:GetName() and container:GetName() .. "ScrollFrame"
	local ScrollFrame = JBP.C.Config_ScrollFrame:New("ScrollFrame", name, container:GetParent(), "JustBecomePro_ScrollFrameTemplate")
...
function GameTooltip:JBP_SetEquiv(equiv)
...
JBP:NewClass("ChatEdit_InsertLink_Hook"){
	OnNewInstance = function(self, editbox, func)		
		JBP:ValidateType(2, "ChatEdit_InsertLink_Hook:New()", editbox, "frame")
		JBP:ValidateType(3, "ChatEdit_InsertLink_Hook:New()", func, "function")
	...
	Call = function(self, text, linkType, linkData)
		if self.editbox:HasFocus() then
			return JBP.safecall(self.func, self, text, linkType, linkData)
		end
	end,
}
...
local function hook_ChatEdit_InsertLink(text)	
	...
	for _, instance in pairs(JBP.Classes.ChatEdit_InsertLink_Hook.instances) do
...
function ChatEdit_InsertLink(...)
	local executionSuccess, insertSuccess = JBP.safecall(hook_ChatEdit_InsertLink, ...)
...
IE = JBP:NewClass("IE", "Frame", "Core_Upgrades"):New("Frame")
IE = JBP:NewModule("IconEditor", IE, "AceEvent-3.0", "AceTimer-3.0") JBP.IE = IE
IE.Tabs = {}
...
function IE:OnInitialize()
	...
	if not IE.TabGroups.ICON or not JBP.indentLib then
		-- GLOBALS: StaticPopupDialogs, StaticPopup_Show, EXIT_GAME, CANCEL, ForceQuit
		StaticPopupDialogs["JBPOPT_RESTARTNEEDED"] = {
		...
		StaticPopup_Show("JBPOPT_RESTARTNEEDED", JUSTBECOMEPRO_VERSION_FULL, L["ERROR_MISSINGFILE_REQFILE"]) -- arg3 could also be "JustBecomePro_Options/IconConfig.lua"
		return
	...
	elseif not JBP.DOGTAG then
		...
		StaticPopup_Show("JBPOPT_RESTARTNEEDED", JUSTBECOMEPRO_VERSION_FULL, "JustBecomePro/Components/Core/Common/DogTags/config.lua") -- arg3 could also be L["ERROR_MISSINGFILE_REQFILE"]
	end

	if JBP.db.global.CreateImportBackup then
		JBP.Backupdb = CopyTable(JustBecomeProDB)
		JBP.BackupDate = date("%I:%M:%S %p")
	end

	JBP:Fire("JBP_OPTIONS_LOADING")
	JBP:UnregisterAllCallbacks("JBP_OPTIONS_LOADING")

	-- Make JBP.IE be the same as IE.
	-- IE[0] = JustBecomePro_IconEditor[0] (already done in .xml)
	-- local meta = CopyTable(getmetatable(IE))
	-- meta.__index = getmetatable(JustBecomePro_IconEditor).__index
	...
	-- Create resizer
	self.resizer = JBP.Classes.Resizer_Generic:New(self)

	JBP:TT(self.resizer.resizeButton, self.resizer.tooltipTitle, function()
		return L["RESIZE_TOOLTIP"] .. (not JBP.IE.db.global.ScaleIE and "\r\n\r\n" .. L["RESIZE_TOOLTIP_IEEXTRA"] or "")
	end, 1, 1)
	...
	function self.resizer:SizeUpdated()
		JBP.IE.db.global.EditorHeight = IE:GetHeight()
		JBP.IE.db.global.EditorScale = IE:GetScale()
	end

	IE.Initialized = true

	JBP:Fire("JBP_OPTIONS_LOADED")
	JBP:UnregisterAllCallbacks("JBP_OPTIONS_LOADED")
	IE.OnInitialize = nil
end
...
function IE:RegisterDatabaseDefaults(defaults)
	...
	JBP:MergeDefaultsTables(defaults, IE.Defaults)
end
...
function IE:GetBaseUpgrades()			-- upgrade functions
	return {
		...
		[62218] = {
			global = function(self)
				IE.db.global.EditorScale = JBP.db.global.EditorScale or 0.9
				JBP.db.global.EditorScale = nil
				
				IE.db.global.EditorHeight = JBP.db.global.EditorHeight or 600
				JBP.db.global.EditorHeight = nil
				
				IE.db.global.ConfigWarning = JBP.db.global.ConfigWarning or true
				JBP.db.global.ConfigWarning = nil
...
IE:SetUpgradePerformedEvent("JBP_IE_UPGRADE_PERFORMED")

JBP:RegisterCallback("JBP_IE_UPGRADE_PERFORMED", function(event, type, upgradeData, ...)
	if type == "global" then
		-- delegate to locale
		if IE.db.sv.locale then
			...
			for locale, ls in pairs(IE.db.sv.locale) do
				...
				if locale ~= currentLocale then
					JBP.IE.db.sv.locale[locale] = nil
					JBP:Printf("Deleted cache for locale %s", locale)
				else
...
function IE:RawUpgrade()
	...
	if JBPOptDB and JBPOptDB.profiles then
		--[[
		if JBPOptDB.Version < 41402 then
			...

			for _, p in pairs(JBPOptDB.profiles) do
				...
			end
		end
		]]
	...
	JBP:Fire("JBP_IE_DB_PRE_DEFAULT_UPGRADES")
	JBP:UnregisterAllCallbacks("JBP_IE_DB_PRE_DEFAULT_UPGRADES")
end

function IE:UpgradeGlobal()
	if JBPOptDB.Version < JUSTBECOMEPRO_VERSIONNUMBER then
		IE:StartUpgrade("global", JBPOptDB.Version, IE.db.global)

		JBPOptDB.Version = JUSTBECOMEPRO_VERSIONNUMBER
	end
...
function IE:UpgradeProfile()
	...
	IE.db.profile.Version = IE.db.profile.Version or JUSTBECOMEPRO_VERSIONNUMBER
	
	if IE.db.profile.Version < JUSTBECOMEPRO_VERSIONNUMBER then
		IE:StartUpgrade("profile", IE.db.profile.Version, IE.db.profile)

		IE.db.profile.Version = JUSTBECOMEPRO_VERSIONNUMBER
	end
end
...
function IE:InitializeDatabase()
	...
	JBP:Fire("JBP_IE_DB_INITIALIZING")
	JBP:UnregisterAllCallbacks("JBP_IE_DB_INITIALIZING")
	
	--------------- Database ---------------
	local JBPOptDB_alias
	if JBPOptDB and JBPOptDB.Version == nil then
		-- if JBPOptDB.Version is nil then we are upgrading from a version from before
		-- AceDB-3.0 was used for the options settings.

		JBPOptDB_alias = JBPOptDB

		-- Overwrite the old database (we will restore from the alias in a second)
		-- 62216 was the first version to use AceDB-3.0
		_G.JBPOptDB = {Version = 62216}

	elseif type(JBPOptDB) ~= "table" then
		-- JBPOptDB might not exist if this is a fresh install
		-- or if the user is upgrading from a really old version that doesn't use JBPOptDB.
		_G.JBPOptDB = {Version = JUSTBECOMEPRO_VERSIONNUMBER}
	end
	...
	IE.db = AceDB:New("JBPOptDB", IE.Defaults)
	
	if JBPOptDB_alias then
		for k, v in pairs(JBPOptDB_alias) do
			IE.db.global[k] = v
		end
	...
	if JBP.DBWasEmpty and IE.db.global.JustBecomeProDBBackup then
		-- JustBecomeProDB was corrupted. Restore from the backup and notify user.
		JustBecomeProDB = IE.db.global.JustBecomeProDBBackup

		JBP:InitializeDatabase()
		JBP.db.profile.Locked = false

		JBP:ScheduleUpdate(1)

		JustBecomePro_DBRestoredNofication:SetTime(IE.db.global.JustBecomeProDBBackupDate)
		JustBecomePro_DBRestoredNofication:Show()

	elseif not JBP.db.global.BackupDbInOptions then
		-- User has disabled storing of DB backups in the options. Get rid of it.
		IE.db.global.JustBecomeProDBBackupDate = nil
		if IE.db.global.JustBecomeProDBBackup then
			collectgarbage()
			IE.db.global.JustBecomeProDBBackup = nil
		end
	elseif not JBP.DBWasEmpty then
		-- JustBecomeProDB was not corrupt, so back it up.
		IE.db.global.JustBecomeProDBBackupDate = time()
		IE.db.global.JustBecomeProDBBackup = JustBecomeProDB
	end

	JBP:Fire("JBP_IE_DB_INITIALIZED")
	JBP:UnregisterAllCallbacks("JBP_IE_DB_INITIALIZED")
end
...
function IE:OnProfile(event, arg2, arg3)
	...
	if IE.Initialized then
		...
		JBP:Fire("JBP_IE_ON_PROFILE", event, arg2, arg3)
	end
end

JBP:RegisterCallback("JBP_ON_PROFILE", function(event, arg2, arg3)
	IE.db:SetProfile(JBP.db:GetCurrentProfile())
end)
...
function IE:OnUpdate()
	...
	if tabGroup then
		...
		if not IE.Header:GetText() then
			IE.Header:SetText("JustBecomePro v" .. JUSTBECOMEPRO_VERSION_FULL)
		end
	end
...
JBP:RegisterCallback("JBP_CONFIG_TAB_CLICKED", function(event, tab)
...
JBP:RegisterCallback("JBP_GLOBAL_UPDATE", function()
	-- GLOBALS: JustBecomePro_ConfigWarning
	if not JBP.Locked then
		if IE.db.global.ConfigWarning then
			JustBecomePro_ConfigWarning:Show()
		else
			JustBecomePro_ConfigWarning:Hide()
		end
	else
		JustBecomePro_ConfigWarning:Hide()
	end
...
IE:RegisterEvent("PLAYER_REGEN_DISABLED", function()
	if not JBP.ALLOW_LOCKDOWN_CONFIG then
		IE:Hide()
	end
end)

JBP:RegisterCallback("JBP_LOCK_TOGGLED", function(event, Locked)
...
function IE:PositionPanels(parentPageName, panelList)
	JBP:SortOrderedTables(panelList)
	

	local panelColumns = JustBecomePro_IconEditor.Pages[parentPageName].panelColumns
	for _, panelColumn in ipairs(panelColumns) do
		for _, panel in JBP:Vararg(panelColumn:GetChildren()) do
			panel:Hide()
		end
...
function IE:DistributeCheckAnchorsEvenly(parent, ...)
	...
	for i = 1, numChildFrames do
		...
		JBP:ValidateType("...", "DistributeCheckAnchorsEvenly(parent, ...)", child, "Config_CheckButton")
...
function IE:Load(isRefresh)
	if not isRefresh then
		...
		JBP:GetModule("SpellCache"):SetNumCachePerFrame(3000)
...
function IE:Reset()	
	...
	JBP.CI.icon.group:GetSettings().Icons[CI.icon.ID] = nil
	
	JBP:Fire("JBP_ICON_SETTINGS_RESET", CI.icon)
...
function IE:Equiv_GenerateTips(equiv)
	local IDs = JBP:SplitNames(JBP.EquivFullIDLookup[equiv])
	local original = JBP.EquivOriginalLookup[equiv]

	for k, v in pairs(IDs) do
		local name, _, texture = GetSpellInfo(v)
		if not name then
			if JBP.debug then
				JBP:Error("INVALID ID FOUND: %s:%s", equiv, v)
				name = "INVALID " .. v
			else
				name = v
			end
	...
	local r = ""
	for name, line in JBP:OrderedPairs(tiptemp) do
		r = r .. line .. "\r\n"
	end
...
JBP:MakeSingleArgFunctionCached(IE, "Equiv_GenerateTips")
...
-- These classes are declared in JustBecomePro.lua. This extends them.
JBP.C.ConfigPanelInfo {
	GetPanel = function(self, panelColumn)
		...
		local success
		success, self.panel = JBP.safecall(self.MakePanel, self, panelColumn)
...
JBP.C.LuaConfigPanelInfo {
	GetFrameName = function(self)
		return self.frameName
	end,

	MakePanel = function(self, panelColumn)
		local panel = JBP.C.Config_Panel:New("Frame", self:GetFrameName(), panelColumn, "JustBecomePro_OptionsModuleContainer")

		JBP.safecall(self.constructor, panel)
...
JBP:NewClass("StaticConfigPanelInfo", "ConfigPanelInfo"){
	OnNewInstance_Static = function(self, order, frameKey)
		JBP:ValidateType(2, "LuaConfigPanelInfo:New()", order, "number")
		JBP:ValidateType(3, "StaticConfigPanelInfo:New()", frameKey, "string")
...
local function tunnel(frame, get, script, ...)
	...
	for _, child in JBP:Vararg(frame:GetChildren()) do
...
CScriptProvider = JBP:NewClass("CScriptProvider"){
	DEBUG_MaxDepth = 25,
	...
	CScriptTunnel = function(self, script, ...)
		...
		for _, child in JBP:Vararg(self:GetChildren()) do
	...
	CScriptAdd = function(self, script, func)
		JBP:ValidateType("script", "CScriptAdd(script, func)", script, "string")
		JBP:ValidateType("func", "CScriptAdd(script, func)", func, "function")
	...
	CScriptAddPre = function(self, script, func)
		JBP:ValidateType("script", "CScriptAddPre(script, func)", script, "string")
		JBP:ValidateType("func", "CScriptAddPre(script, func)", func, "function")
	...
	CScriptRemove = function(self, script, func)
		JBP:ValidateType("script", "CScriptRemove(script, func)", script, "string")
		JBP:ValidateType("func", "CScriptRemove(script, func)", func, "function;nil")
		...
		if type(existing) == "function" then
			...
			JBP.tDeleteItem(existing, func, 1)
		end
	end,
	...
	CScriptCall = function(self, script, ...)
		...
		if type(cscript) == "function" then
			JBP.safecall(cscript, self, ...)
		else
			for i = 1, #cscript do
				JBP.safecall(cscript[i], self, ...)
			end
		end
		CS_SDepth = CS_SDepth - 1
	end,
	...
	DEBUG_CScriptCallBase = function(self, script, ...)
		...
		elseif CS_SDepth > self.DEBUG_MaxDepth then
			...
			error("JustBecomePro: CScript Overflow: " .. str)
		end
	end,
}


JBP:NewClass("Config_Frame", "Frame", "CScriptProvider"){

	-- Constructor
	OnNewInstance_Frame = function(self)

		-- Setup callbacks that will load the settings when needed.
		if self.ReloadSetting ~= JBP.NULLFUNC then
	...
	SetTooltip = function(self, title, text)
		if self.SetMotionScriptsWhileDisabled then
			JBP:TT(self, title, text, 1, 1, nil)
		else
			JBP:TT(self, title, text, 1, 1, "IsEnabled")
		end
	end,
	...
	SetAdjustHeightExclusion = function(self, frame, exclude)
		...
		JBP.tDeleteItem(self.adjustHeightExclusions, frame)
		...
		for i = 1, child:GetNumPoints() do
			...
			if relTo == self and relPoint:find("BOTTOM") then
				error("Attempted to adjust height of a frame with a child anchored to the bottom of the frame: " 
					.. (JBP.tContains(self, child) or child:GetName() or "???"), 3)
			end
		end
	end,
	...
	CalculateAutoHeight = function(self, bottomPadding)
		...
		for _, child in JBP:Vararg(self:GetChildren()) do
			...
			if child:IsShown()
			and (not exclusions or not JBP.tContains(exclusions, child))
			then
		...
		for _, child in JBP:Vararg(self:GetRegions()) do
			if child:IsShown()
			and child:GetBottom()
	...
	AdjustHeightAnimated = function(self, bottomPadding, duration)
		...
		JBP:AnimateHeightChange(self, height, duration or 0.1)
	end,


	GetParentKey = function(self)
		return JBP.tContains(self:GetParent(), self)
	end,
	...
	ReloadSetting = JBP.NULLFUNC
}

JBP:NewClass("Config_Panel", "Config_Frame"){
	SetHeight_base = JBP.C.Config_Panel.SetHeight,
}{
	...
	Flash = function(self, dur)
		...
		ticker = C_Timer.NewTicker(0.01, function() 
			local bg = JustBecomePro_DotwatchSettings.Background
	...
	BuildSimpleCheckSettingFrame = function(self, arg2, arg3)
		...
		JBP:ValidateType("panel",     sig, self,     "Config_Panel")
		JBP:ValidateType("className", sig, className, "string")
		JBP:ValidateType("allData",   sig, allData,   "table")
		...
		for i, data in ipairs(allData) do
			if data then -- skip over falses (dont freak out about them, they are probably intentional)
				...
				local f = class:New(objectType, nil, self, "JustBecomePro_CheckTemplate", i)
				data(f)
...
JBP:NewClass("Config_Page", "Config_Frame"){
...
JBP:NewClass("Config_ScrollFrame", "ScrollFrame", "Config_Frame"){
...
JBP:NewClass("Config_Button", "Button", "Config_Frame"){
	SetTexts = function(self, title, tooltip)
		self:SetText(title)
		self:SetTooltip(title, tooltip)
	end,
...
JBP:NewClass("Config_CheckButton", "CheckButton", "Config_Frame"){
	-- Constructor
...
JBP:NewClass("Config_EditBox", "EditBox", "Config_Frame"){
	...
	MakeScrollable = function(self, ...)
		local ScrollFrame = JBP:ConvertContainerToScrollFrame(self, ...)
	...
	SetAcceptsJBPLinks = function(self, accepts, linkDesc)
		self.acceptsJBPLinks = accepts
		self.acceptsJBPLinksDesc = accepts and linkDesc or nil
	end,

	GetAcceptsJBPLinks = function(self, accepts, linkDesc)
		return self.acceptsJBPLinks, self.acceptsJBPLinksDesc
	end,
}

JBP:NewClass("Config_EditBox_Lua", "Config_EditBox") {
	GetText_original = JBP.C.Config_EditBox_Lua.GetText,
	SetText_original = JBP.C.Config_EditBox_Lua.SetText,
	padNewlines = true,

	ColorTable = (function()
		local colorTable = {}
		local tokens = JBP.indentLib.tokens
		local function set(color, ...)
			for i, v in JBP:Vararg(...) do
	...
	OnNewInstance_EditBox_Lua = function(self)
		JBP.indentLib.enable(self, self.ColorTable, 4)

		self:SetFont("Interface/Addons/JustBecomePro/Fonts/VeraMono.ttf", 11)

		self:SetNewlineOnEnter(true)

		local old = JBP.indentLib.padWithLinebreaks
		JBP.indentLib.padWithLinebreaks = function(code)
		...
		self:SetAcceptsJBPLinks(true, JBP.L["LUA_INSERTGUID_TOOLTIP"])
		JBP.Classes.ChatEdit_InsertLink_Hook:New(self, self.InsertLinkHook)
	end,
	...
	GetCursorLineNumber = function(self)
		...
		local pos = 0
		for lineNum, line in JBP:Vararg(strsplit("\n", text)) do
			pos = pos + #line
			if pos >= curPos then
				return lineNum
			end
	...
	InsertLinkHook = function(self, text, linkType, linkData)
		...
		if linkType == "JBP" then
			-- Reconstruct the GUID
			local GUID = linkType .. ":" .. linkData

			self.editbox:Insert(("JBP:GetDataOwner(%q)"):format(GUID))
...
JBP:NewClass("Config_TimeEditBox", "Config_EditBox"){
	...
	ModifyValueForLoad = function(self, value)
		...
		return JBP.C.Formatter.TIME_COLONS_FORCEMINS:Format(value)
	end,
}

JBP:NewClass("Config_EditBoxWithCheck", "Config_Frame"){
	...
	SetTexts = function(self, title, tooltip)
		...
		self.EnableCheck:SetTexts(ENABLE, JBP.L["GENERIC_NUMREQ_CHECK_DESC"]:format(tooltip:gsub("^%u", strlower)))
	end,
...
JBP:NewClass("Config_Slider", "Slider", "Config_Frame")
{
	-- Saving base methods.
	-- This is done in a separate call to make sure it happens before 
	-- new ones overwrite the base methods.

	Show_base = JBP.C.Config_Slider.Show,
	Hide_base = JBP.C.Config_Slider.Hide,

	SetValue_base = JBP.C.Config_Slider.SetValue,
	GetValue_base = JBP.C.Config_Slider.GetValue,

	GetValueStep_base = JBP.C.Config_Slider.GetValueStep,

	GetMinMaxValues_base = JBP.C.Config_Slider.GetMinMaxValues,
	SetMinMaxValues_base = JBP.C.Config_Slider.SetMinMaxValues,
}{

	Config_EditBox_Slider = JBP:NewClass("Config_EditBox_Slider", "Config_EditBox"){
	...
	formatter = JBP.C.Formatter.PASS,
	extremesFormatter = JBP.C.Formatter.PASS,
	...
	SetValue = function(self, value)
		JBP:ValidateType("value", "SetValue(value)", value, "number")
	...
	UseEditBox = function(self)
		...
		if not self.EditBox then
			...
			self.EditBox = self.Config_EditBox_Slider:New("EditBox", name, self:GetParent(), "JustBecomePro_InputBoxTemplate", nil, {})
	...
	SetTextFormatter = function(self, formatter, extremesFormatter)
		JBP:ValidateType("2 (formatter)", (self:GetName() or "<unnamed>") .. ":SetTextFormatter(formatter)", formatter, "Formatter;nil")
		JBP:ValidateType("3 (extremesFormatter)", (self:GetName() or "<unnamed>") .. ":SetTextFormatter(formatter [,extremesFormatter])", extremesFormatter, "Formatter;nil")

		self.formatter = formatter or JBP.C.Formatter.PASS
		self.extremesFormatter = extremesFormatter or formatter or JBP.C.Formatter.PASS
	...
	TT_textFunc = function(self)
		...
		if self:GetObjectType() == "Slider" then
			...
			if self.Slider:ShouldForceEditBox() then
				text = text .. L["CNDT_SLIDER_DESC_CLICKSWAP_TOSLIDER_DISALLOWED"]:format(JBP.C.Formatter.COMMANUMBER:Format(self.Slider.FORCE_EDITBOX_THRESHOLD))
			else
	...
	SetTooltip = function(self, title, text)
		...
		JBP:TT(self, title, self.TT_textFunc, 1, 1)

		if self.EditBox then
			JBP:TT(self.EditBox, title, self.TT_textFunc, 1, 1)
	...
	SaveSetting = function(self)
		...
		if settings and self.setting then
			...
			if self.ttData then
				JBP:TT_Update(self.EditBoxShowing and self.EditBox or self)
			end
...
JBP:NewClass("Config_Slider_Alpha", "Config_Slider"){
	-- Constructor
	...
	Formatter = JBP.C.Formatter:New(function(value)
		if value == 0 then
			return L["CONDITIONPANEL_ICON_HIDDEN"]
		else
			return JBP.C.Formatter.PERCENT100:Format(value)
		end
	end),
	
	OrangeAt100Formatter = JBP.C.Formatter:New(function(value)
		if value == 0 then
			return L["CONDITIONPANEL_ICON_HIDDEN"]
		else
			if value == 1 then
				return "|cffff7400" .. JBP.C.Formatter.PERCENT100:Format(value)
			end
			return JBP.C.Formatter.PERCENT100:Format(value)
		end
	end),
...
JBP:NewClass("Config_BitflagBase"){
	-- Constructor
...
JBP:NewClass("Config_CheckButton_BitToggle", "Config_BitflagBase", "Config_CheckButton")

JBP:NewClass("Config_Frame_IconStateSet", "Config_Frame"){
	-- Constructor
	...
	SettingTableRequested = function(self)
		return JBP.CI.ics and JBP.CI.ics.States[self.setting] or false
	end,
...
JBP:NewClass("Config_ColorButton", "Button", "Config_Frame"){
	hasOpacity = false,
	hasDesaturate = false,

	OnNewInstance_ColorButton = function(self)
		assert(self.background1 and self.text and self.swatch, 
			"This setting frame doesn't inherit from JustBecomePro_ColorButtonTemplate")
	...
	OnClick = function(self, button)
		...
		if settings and self.setting then
			...
			JustBecomePro_ColorPicker:Load(
				self,
				"PickerCallback",
	...
	GetRGBA = function(self)
		...
		if settings and self.setting then
			...
			local c = JBP:StringToCachedRGBATable(settings[self.setting])
...
JBP:NewClass("Config_Button_Rune", "Button", "Config_BitflagBase", "Config_Frame"){
	-- Constructor
...
JBP:NewClass("Config_PointSelect", "Config_Frame"){
	...
	SetTooltipBody = function(self, body)
		JBP:ValidateType("2 (body)", "SetTooltipBody", body, "string")
		...
		for k, v in pairs(self) do
			if type(v) == "table" and v.GetObjectType and v:GetObjectType() == "CheckButton" then
				JBP:TT(v, JBP.L[k], body:format(JBP.L[k]), 1, 1)
			end
		end
	end,

	SetSelectedPoint = function(self, point)
		JBP:ValidateType("2 (point)", "SetSelectedPoint", point, "string")
		...
		for k, v in JBP:Vararg(self:GetChildren()) do
	...
	SelectChild = function(self, child)
		JBP:ValidateType("2 (child)", "SelectChild", child, "frame")
...
JBP:NewClass("Config_ColorPicker", "Config_Frame"){
	...
	OnNewInstance = function(self)
		...
		for i, slider in JBP:Vararg(self.HueSlider, self.SaturationSlider, self.ValueSlider, self.AlphaSlider) do
	...
	UpdateRecentColors = function(self)
		...
		for i, color in ipairs(IE.db.global.RecentColors) do
			...
			if not f then
				f = CreateFrame("Button", nil, self.RecentColorFrame, "JustBecomePro_ColorButtonTemplate")
			...
			f:Show()
			JBP:TT(f, color, "COLORPICKER_RECENT_DESC", 1, nil)
			f.string = color
			f.swatch:SetColorTexture(JBP:StringToRGBA(color))
		end
	...
	LiveUpdate = function(self)
		...
		self.StringEditbox:SetText(JBP:HSVAToColorString(h, s, v, a))

		local r,g,b=JBP:HSVToRGB(h, 0, v)
		self.SaturationSlider.Background:SetGradient("HORIZONTAL", r,g,b, JBP:HSVToRGB(h, 1, v))

		local r,g,b=JBP:HSVToRGB(h, s, 0)
		self.ValueSlider.Background:SetGradient("HORIZONTAL", r,g,b, JBP:HSVToRGB(h, s, 1))

		for i = 1, self.HueSlider.NUM_SEGMENTS do
			local r,g,b=JBP:HSVToRGB((i-1)/self.HueSlider.NUM_SEGMENTS, s, v)
			self.HueSlider.textures[i]:SetGradient("HORIZONTAL", r,g,b, JBP:HSVToRGB(i/self.HueSlider.NUM_SEGMENTS, s, v))
		end

		local r,g,b=JBP:HSVToRGB(h, s, v)
		...
		if self.hasTexture then
			...
			if textureValue and textureValue ~= "" then
				textureValue = JBP.COMMON.Textures:EvaluateTexturePath(textureValue, JBP.NULLFUNC)

				self.iconTexture:SetTexture(textureValue)
			end
		end		
		self.iconTexture:SetVertexColor(JBP:HSVToRGB(h, s, v))
	...
	SetColorString = function(self, value, loadFlags)
		...
		local c = JBP:ColorStringToCachedHSVATable(value)
	...
	GetColorString = function(self, value)
		return JBP:HSVAToColorString(self.h, self.s, self.v, self.a, {desaturate=self.desaturate})
	end,
	...
	Load = function(self, callbackObj, callback, value, texture, hasOpacity, hasDesaturate, textureValue)
		...
		JBP.SUG:EnableEditBox(self.Texture, "texture_withVarTex", true, true, self)
		...
		local c = JBP:StringToCachedRGBATable(value)
...
JBP.C.IE:Inherit("Config_Frame")

---------------------------------
-- Icon Editor Tabs
---------------------------------

JBP:NewClass("IconEditorTabBase", "Config_Button"){

	SetTexts = function(self, title, tooltip)
		self:SetText(title)
		JBP:TT(self, title, tooltip, 1, 1)
	end,
...
JBP:NewClass("IconEditorTabGroup", "IconEditorTabBase"){
	...
	OnClick = function(self)
		...
		JBP.IE.Tabs.art.pSelectedHorizontal:ClearAllPoints()
		JBP.IE.Tabs.art.pSelectedHorizontal:SetPoint("TOPLEFT", self)
		JBP.IE.Tabs.art.pSelectedHorizontal:SetPoint("TOPRIGHT", self)

		for i, tab in JBP:Vararg(JBP.IE.Tabs.secondary:GetChildren()) do
			tab:Hide()
		end
		...
		if not IE.CurrentTab then
			JBP.IE.Tabs.art.sSelectedHorizontal:ClearAllPoints()
			JBP.IE.Tabs.art.sSelectedHorizontal:SetPoint("TOP")
		end
...
JBP:NewClass("IconEditorTab", "IconEditorTabBase"){
	endPadding = 6,
	interPadding = 5,
	...
	SetHistorySet = function(self, historySet)
		JBP:ValidateType("historySet", "SetHistorySet(historySet)", historySet, "HistorySet")
	...
	OnClick = function(self)
		...
		JBP:Fire("JBP_CONFIG_TAB_CLICKED", self, oldTab)
	...
	GetPage = function(self)
		return JBP.IE.Pages[self.pageKey]
	end,
...
function IE:RegisterTabGroup(identifier, text, order, setupHeaderFunc)
	...
	JBP:ValidateType("identifier",      sig, identifier,      "string")
	JBP:ValidateType("text",            sig, text,            "string")
	JBP:ValidateType("order",           sig, order,           "number")
	JBP:ValidateType("setupHeaderFunc", sig, setupHeaderFunc, "function")
	...
	local tab = JBP.C.IconEditorTabGroup:New("Button", nil, JBP.IE.Tabs.primary, "JustBecomePro_IE_Tab")
	JBP.IE.Tabs.primary[identifier] = tab
	...
	for i, tab in ipairs(JBP.IE.Tabs.primary) do
		tab:Hide()
	end
...
function IE:RegisterTab(groupIdentifier, identifier, pageKey, order)
	...
	JBP:ValidateType("groupIdentifier", sig, groupIdentifier, "string")
	JBP:ValidateType("identifier",      sig, identifier,      "string")
	JBP:ValidateType("pageKey",         sig, pageKey,         "string")
	JBP:ValidateType("order",           sig, order,           "number")
	...
	local tab = JBP.C.IconEditorTab:New("Button", nil, JBP.IE.Tabs.secondary, "JustBecomePro_IE_Tab")
	JBP.IE.Tabs.secondary[identifier] = tab
	...
	JBP:SortOrderedTables(tabGroup.Tabs)

	return tab
end

function IE:ResizeTabs()
	local endPadding = JBP.C.IconEditorTab.endPadding
	local interPadding = JBP.C.IconEditorTab.interPadding

	-- First, resize the primary tabs (the tab groups) so that we can figure out their exact width
	local primaryWidth = -interPadding
	local prevTabGroup
	for identifier, tabGroup in JBP:OrderedPairs(IE.TabGroups, JBP.OrderSort, true) do
	...
	JBP.IE.Tabs.primary:SetWidth(primaryWidth)
	...
	if not JBP.IE.Tabs.primary:GetLeft() then return end
	local availableWidth = JBP.IE.Tabs.primary:GetLeft() - JBP.IE.Tabs.secondary:GetLeft() - 20
	...
	for i, tab in JBP:OrderedPairs({JBP.IE.Tabs.secondary:GetChildren()}, JBP.OrderSort, true) do
		tab:AdjustWidth()
		if tab:IsShown() then
	...
	-- All done!
	JBP.IE.Tabs:SetHeight(8 + (25 * #rows))
	JBP.IE.Tabs.secondary:SetWidth(width + endPadding)
end
...
function IE:RefreshTabs()
	...
	if not tabGroup then
		for _, tabGroup in JBP:OrderedPairs(IE.TabGroups, JBP.OrderSort, true) do
			tabGroup:Click()
			return
		end
	else
...
function IE:DisplayPage(pageKey)
	for _, otherPage in JBP:Vararg(JBP.IE.Pages:GetChildren()) do
		otherPage:Hide()
	end
	...
	local page = JustBecomePro_IconEditor.Pages[pageKey]
	if not page then
		JBP:Error(("Couldn't find child of JustBecomePro_IconEditor.Pages with key %q"):format(pageKey))
	end
...
function JBP:Import(SettingsItem, ...)
	...
	JBP:Fire("JBP_IMPORT_PRE", SettingsItem, ...)
	
	local SharableDataType = JBP.approachTable(JBP, "Classes", "SharableDataType", "types", SettingsItem.Type)
	if SharableDataType and SharableDataType.Import_ImportData then
		...
		JBP:Print(L["IMPORT_SUCCESSFUL"])
	else
		JBP:Print(L["IMPORTERROR_INVALIDTYPE"])
	end

	JBP:Fire("JBP_IMPORT_POST", SettingsItem, ...)
end

function JBP:ImportPendingConfirmation(SettingsItem, luaDetections, callArgsAfterSuccess)
	JustBecomePro_ConfirmImportedLuaDialog:StartConfirmations(SettingsItem, luaDetections, callArgsAfterSuccess)
end

---------- Serialization ----------
function JBP:SerializeData(data, type, ...)
	-- nothing more than a wrapper for AceSerializer-3.0
	assert(data, "No data to serialize!")
	assert(type, "No data type specified!")
	return JBP:Serialize(data, JUSTBECOMEPRO_VERSIONNUMBER, " ~", type, ...)
end

function JBP:MakeSerializedDataPretty(string)
	return string:
...
function JBP:DeserializeDatum(string, silent)
	local success, data, version, spaceControl, type = JBP:Deserialize(string)
	...
	if spaceControl then
		if spaceControl:find("`|") then
			...
			return JBP:DeserializeDatum(string:gsub("`", "~`"):gsub("~`|", "~`~|"), silent)
		elseif spaceControl:find("`") then
			-- if spaces have become corrupt, then reformat them and... re-deserialize
			return JBP:DeserializeDatum(string:gsub("`", "~`"), silent)
		elseif spaceControl:find("~|") then
	...
	if not JBP.Classes.SharableDataType.types[type] then
		-- unknown data type
		return nil
	end
	...
	local result = {
		...
		type = type,
		version = version,
		select(6, JBP:Deserialize(string)), -- capture all extra args
	}

	return result
end

function JBP:DeserializeData(str, silent)
	if not str then 
		return
	end
	...
	for string in gmatch(str, "(^%d+.-^^)") do
		...
		local result = JBP:DeserializeDatum(string, silent)
...
function JBP:GetSettingsString(type, settings, defaults, ...)
	...
	settings = JBP:CleanSettings(type, settings, defaults)
	return JBP:SerializeData(settings, type, ...)
end

function JBP:GetSettingsStrings(strings, type, settings, defaults, ...)
	assert(settings, "No data to serialize!")
	assert(type, "No data type specified!")
	assert(defaults, "No defaults specified!")
	...
	local string = JBP:GetSettingsString(type, settings, defaults, ...)
	if not JBP.tContains(strings, string) then
		tinsert(strings, string)

		JBP:Fire("JBP_EXPORT_SETTINGS_REQUESTED", strings, type, settings)
	end

	JBP.tRemoveDuplicates(strings)

	return strings
end

function JBP:CleanDefaults(settings, defaults, blocker)
	...
	for k,v in pairs(defaults) do
			...
			if type(v) == "table" then
				...
				for key, value in pairs(settings) do
					if type(value) == "table" then
						...
						elseif k == "**" then
							JBP:CleanDefaults(value, v, defaults[key])
                            -- if the table is empty afterwards, remove it
                            if next(value) == nil then
                                settings[key] = nil
                            end
		...
		elseif type(v) == "table" and type(settings[k]) == "table" then
			...
			JBP:CleanDefaults(settings[k], v, blocker and blocker[k])
...
function JBP:CleanSettings(type, settings, defaults)
	return JBP:CleanDefaults(settings, defaults)
end


---------- Dropdown ----------
JBP:RegisterCallback("JBP_CONFIG_REQUEST_AVAILABLE_IMPORT_EXPORT_TYPES", function(event, editbox, import, export)
	if editbox == JBP.IE.ExportBox then
...
JBP:NewClass("HistorySet") {
	HistorySets = {},

	OnNewInstance = function(self, identifier)
		JBP:ValidateType("identifier", "HistorySet:New(identifier)", identifier, "string")
	...
	AddBlocker = function(self, blocker)
		JBP:CopyInPlaceWithMetatable(blocker, self.blocker)
	end,
	...
	AttemptBackup = function(self, location, settings)
		...
		if not location.history then
			...
			location.history = {JBP:CopyWithMetatable(settings, self.blocker)}
			...
			if type(result) == "string" then
				...
				location.history[#location.history + 1] = JBP:CopyWithMetatable(settings, self.blocker)
	...
	DoUndoRedo = function(self, direction)
		...
		JBP:CopyInPlaceWithMetatable(location.history[location.historyState], settings, self.blocker)
...
local function makeColorFunc(greenBelow, redAbove)
	--local completeColor = JBP:StringToCachedRGBATable("ff00ff00")
	--local halfColor = JBP:StringToCachedRGBATable("ffff8000")
	--local startColor = JBP:StringToCachedRGBATable("ffff0000")
	local completeColor = JBP:StringToCachedRGBATable("ff00ff47")
	local halfColor = JBP:StringToCachedRGBATable("ff00f9ff")
	local startColor = JBP:StringToCachedRGBATable("ff0078ff")
...
JBP.IE.CpuReportParameters = {
	Columns = {
		...
		},
		{
			...
			value = function(icon) return icon.cpu_updateTotal / (JBP.time - icon.cpu_startTime) end,
			format = "%.2f ms/s",
		...
		},
		{
			...
			value = function(icon) return icon.cpu_eventTotal / (JBP.time - icon.cpu_startTime) end,
			format = "%.2f ms/s",
		...
		},
		{
			...
			value = function(icon) return icon.cpu_cndtTotal / (JBP.time - icon.cpu_startTime) end,
			format = "%.2f ms/s",
			color = makeColorFunc(0.02, 0.6)
		},
	}
}
function JBP.IE:GetCpuProfileReport()
	...
	for group in JBP:InGroups() do
		...
		for icon in group:InIcons() do 
			if icon.cpu_updateTotal > 0 or icon.cpu_eventTotal > 0 or icon.cpu_cndtTotal > 0 then
				...
				local time = JBP.time - icon.cpu_startTime
				...
				if not printedGroup then
					...
					for i, column in pairs(JBP.IE.CpuReportParameters.Columns) do
