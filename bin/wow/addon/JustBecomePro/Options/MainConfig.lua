...
local BaseConfig = JBP:NewClass("Main_BaseConfig", "GenericComponent")
BaseConfig.DefaultPanelSet = "profile"



BaseConfig:RegisterConfigPanel_ConstructorFunc(2, "JustBecomePro_Main_General", function(self)
...
BaseConfig:RegisterConfigPanel_ConstructorFunc(2, "JustBecomePro_Main_GeneralGlobal", function(self)
	...
	self:BuildSimpleCheckSettingFrame({
		...
		function(check)
			...
			check:CScriptAdd("SettingSaved", function()
				...
				JBP.IE.db.global.ConfigWarningN = -10e6
			end)
		end,
	})
end):SetPanelSet("global")

BaseConfig:RegisterConfigPanel_ConstructorFunc(9, "JustBecomePro_Main_CommSettings", function(self)
...
BaseConfig:RegisterConfigPanel_XMLTemplate(30, "JustBecomePro_Main_Media")
BaseConfig:RegisterConfigPanel_XMLTemplate(50, "JustBecomePro_Main_Efficiency"):SetPanelSet("global")
BaseConfig:RegisterConfigPanel_XMLTemplate(1, "JustBecomePro_Main_Profiles")
...
local HistorySet = JBP.C.HistorySet:New("MAIN")
...
function HistorySet:GetCurrentLocation()
	return profileHistories[JBP.db:GetCurrentProfile()]
end
function HistorySet:GetCurrentSettings()
	-- Proxy into AceDB settings. Can't give it JBP.db or everything will blow up - even with appropriate blockers.
	return {profile = JBP.db.profile, global = JBP.db.global}
end
...
(JBP.CHANGELOG_LASTVER or "???") .. ", click the tab below again.|r</p>"
local changelogEndAll = "<p align='center'>|cff666666For older versions, visit JustBecomePro's AddOn page on Curse.com|r</p><br/>"
...
function IE:ShowChangelog(lastVer)
	...
	for version, text in JBP:OrderedPairs(CHANGELOGS, nil, nil, true) do
	...
	tinsert(texts, 1, "<p align='center'>|cff999999" .. L["CHANGELOG_INFO2"]:format(JUSTBECOMEPRO_VERSION_FULL) .. "|r</p>")
...
function IE:ProcessChangelogData()
	...
	if not JBP.CHANGELOG then
		JBP:Error("There was an error loading JBP's changelog data.")
		JBP:Print("There was an error loading JBP's changelog data.")

		return CHANGELOGS
	end

	local log = JBP.CHANGELOG
	...
	repeat
		...
		local versionString = log:match("JustBecomePro v([0-9%.]+)", subStart):gsub("%.", "")
...
JBP:RegisterCallback("JBP_CONFIG_LOADED", function()
	if IE.db.global.LastChangelogVersion > 0 then		
		if IE.db.global.LastChangelogVersion < JUSTBECOMEPRO_VERSIONNUMBER then
			if IE.db.global.LastChangelogVersion < JUSTBECOMEPRO_FORCECHANGELOG -- forced
			or JUSTBECOMEPRO_VERSION_MINOR == "" -- upgraded to a release version (e.g. 7.0.0 release)
			or floor(IE.db.global.LastChangelogVersion/100) < floor(JUSTBECOMEPRO_VERSIONNUMBER/100) -- upgraded to a new minor version (e.g. 6.2.6 release -> 7.0.0 alpha)
			then
			...
			IE.db.global.LastChangelogVersion = JUSTBECOMEPRO_VERSIONNUMBER
		end
	else
		IE.db.global.LastChangelogVersion = JUSTBECOMEPRO_VERSIONNUMBER
	end
