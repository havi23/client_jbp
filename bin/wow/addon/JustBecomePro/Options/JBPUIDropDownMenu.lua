...
local DD = JBP:NewClass("Config_DropDownMenu_NoFrame"){
	FORCE_SCALE = nil,
...
DD.LISTS = CreateFrame("Frame", "JBPDropDowns", UIParent)
...
info.value = [ANYTHING]  --  The value that JBP.DD.MENU_VALUE is set to when the button is clicked
...
function DD:CreateFrames(level, index)
	...
	while ( level > DD.MAXLEVELS ) do
		...
		local newList = CreateFrame("Button", "$parentList" .. DD.MAXLEVELS, DD.LISTS, "JBP_UIDropDownListTemplate", DD.MAXLEVELS);
		...
		for i=DD.MINBUTTONS+1, DD.MAXBUTTONS do
			newList[i] = CreateFrame("Button", nil, newList.Buttons, "JBP_UIDropDownMenuButtonTemplate", i);
	...
	while ( index > DD.MAXBUTTONS ) do
		...
		for i=1, DD.MAXLEVELS do
			...
			local button = CreateFrame("Button", nil, listFrame.Buttons, "JBP_UIDropDownMenuButtonTemplate", DD.MAXBUTTONS);
	...
	if ( listFrame:IsShown() and (DD.OPEN_MENU == (dropDownFrame or button:GetParent())) ) then
		...
		if ( level == 1 ) then	
			...
			if self.FORCE_SCALE then
				JBPDropDowns:SetScale(self.FORCE_SCALE)
			else
				JBPDropDowns:SetScale(1)
				for _, frame in JBP:Vararg(dropDownFrame, anchorName) do
					if type(frame) == "table" and frame.GetEffectiveScale then
						JBPDropDowns:SetScale(frame:GetEffectiveScale() / JBPDropDowns:GetParent():GetScale())
						break
					end
				end
			end
			...
			elseif ( anchorName == "cursor" ) then
				...
				cursorX = cursorX/JBPDropDowns:GetScale();
				cursorY = cursorY/JBPDropDowns:GetScale();
		...
		if level == 1 then
			local scale = JBPDropDowns:GetScale()
...
local DD_Frame = JBP:NewClass("Config_DropDownMenu", "Config_Frame", "Config_DropDownMenu_NoFrame"){
...
JBP:NewClass("Config_DropDownMenu_Icon", "Config_DropDownMenu"){
	previewSize = 18,
	...
	SetUIDropdownGUIDText = function(self, GUID, text)
		...
		local owner = JBP:GetDataOwner(GUID)
		...
		elseif GUID and GUID ~= "" then
			if type == "icon" then
				...
				local ics = JBP:GetSettingsFromGUID(GUID)
				if ics then
					text = JBP:GetIconMenuText(ics)
				end
			elseif type == "group" then
	...
	SetIconPreviewIcon = function(self, GUID)
		local icon = JBP:GetDataOwner(GUID)
		local type = JBP:ParseGUID(GUID)
		...
		if not icon then
			local ics, _, gs, domain, groupID, iconID = JBP:GetSettingsFromGUID(GUID)
			if not ics then return end

			texture = JBP:GuessIconTexture(ics)
			title = L["GROUPICON"]:format(JBP:GetGroupName(gs.Name, groupID, 1), iconID)
		...
		JBP:TT(self.IconPreview, title, desc, 1, 1)
	...
	SetGUID = function(self, GUID)
		local icon = JBP.GUIDToOwner[GUID]
