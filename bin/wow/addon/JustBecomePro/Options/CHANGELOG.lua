...
JBP.CHANGELOG_LASTVER="7.4.0"

JBP.CHANGELOG = [==[
## v8.6.8
* Re-releasing JustBecomePro for Retail WoW as 8.6.8 so it will be the latest file for people with out-of-date Twitch apps.
 * IMPORTANT: If your Twitch app was installing JustBecomePro Classic into your Retail WoW installation, that means your Twitch app is out of date and needs to be updated.
...
* Switched WoW-built-in sounds that JBP registers with LSM to use FileDataIDs instead of paths, since paths aren't allowed anymore in WoW 8.2.
...
### Discord
* JustBecomePro now has a Discord! Come ask questions, share configuration, or just hang out with other JustBecomePro users. https://discord.gg/NH7RmcP

### General
* Created a new system to collect performance metrics on a per-icon basis. You can view this new feature via `/jbp cpu` or under the "Performance" section in the main options.
...
### Bug Fixes
* Fixed #1509 - "Couldn't open Interface/AddOns/JustBecomePro_Options/"
...
## v8.5.0
* Battle For Azeroth support. Please report bugs to https://wow.curseforge.com/projects/justbecomepro/issues. 
...
## v8.4.4
* JustBecomePro_Options no longer saves a spell cache to disk. Performance improvements have made it feasible to compute this each time you log in as it needed, resulting in faster log-in/log-out times.
...
* Added options to disable JBP's built-in settings backup and the "backup" import source.
...
### Bug Fixes
* Lua conditions should once again properly resolve members defined in JBP.CNDT.Env.
...
### Bug Fixes
* Attempted a permanant fix to very rare and very old bug where some users' includes.*.xml files were getting scrambled around in their JBP install, leading to the problem of most of the addon not getting loaded.
...
## v8.0.2
### Bug Fixes
* Fixed: JustBecomeProComponentsCoreIcon.lua:491 attempt to index field 'typeData' (a nil value)
...
* All JustBecomePro configuration is now done in the icon editor.
