...
JBP:NewClass("Config_ArrowButton", "Config_CheckButton") {
	OnNewInstance_CheckButton = function(self)
		if not self:GetParent().class == JBP.C.Config_ArrowButtonSet then
	...
	SetOrientation = function(self, orientation)
		JBP:ValidateType("orientation", "SetOrientation(orientation)", orientation, "string")
...
JBP:NewClass("Config_ArrowButtonSet", "Config_Frame") {

	OnNewInstance_ArrowButtonSet = function(self)
		self.arrows = {}

		for i, orientation in JBP:Vararg("LEFT", "DOWN", "RIGHT", "UP") do
			local arrow = JBP.C.Config_ArrowButton:New("CheckButton", nil, self, "JustBecomePro_ArrowButton_Template")
	...
	EnableOrientations = function(self, ...)
		for _, orientation in JBP:Vararg(...) do
	...
	SetOrientation = function(self, orientation)
		JBP:ValidateType("orientation", "SetOrientation(orientation)", orientation, "string;nil")
...
JBP:NewClass("JustBecomePro_GM_IconPosition_Sortable_Dir", "Config_Panel") {
	map = {
		["UP,LEFT"] = 7,
		["UP,RIGHT"] = 8,
		["DOWN,LEFT"] = 6,
	...
	SaveSetting = function(self)
		...
		if not value then
			...
			error("COULDNT FIND A VALUE FOR JustBecomePro_GM_IconPosition_Sortable_Dir")
		end
	...
	ReloadSetting = function(self)
		...
		local primary, secondary = strsplit(",", JBP.tContains(self.map, value))
...
JBP:NewClass("Config_IconSortFrame", "Button", "Config_Frame") {
...
function IconPosition_Sortable:LoadConfig()
	...
	local panel = JustBecomePro_GM_IconPosition_Sortable

	for i, data in ipairs(gs.SortPriorities) do
		local frame = sortFrames[i]
		if not frame then
			frame = JBP.C.Config_IconSortFrame:New("Button", nil, panel, "JustBecomePro_GM_IPS_IconSortTemplate")
			sortFrames[i] = frame
			frame:SetID(i)
...
local function AddOnClick(button, identifier, data)
	local SortPriorities = JBP.CI.gs.SortPriorities
...
function IconPosition_Sortable:AddDropdown()
	local used = {}
	for _, data in pairs(JBP.CI.gs.SortPriorities) do
		used[data.Method] = true
	end

	local addedOne = false
	for identifier, data in JBP:OrderedPairs(IconPosition_Sortable.Sorters, SorterCompare) do
...
local function PresetOnClick(button, settings)
	JBP.CI.gs.SortPriorities = CopyTable(settings)
...
function IconPosition_Sortable:PresetDropdown()
	...
	for text, settings in JBP:OrderedPairs(IconPosition_Sortable.Presets) do
		local info = JBP.DD:CreateInfo()
