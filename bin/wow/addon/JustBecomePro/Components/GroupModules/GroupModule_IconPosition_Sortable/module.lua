...
local IconPosition_Sortable = JBP:NewClass("GroupModule_IconPosition_Sortable", "GroupModule_IconPosition")
...
JBP:RegisterCallback("JBP_DB_PRE_DEFAULT_UPGRADES", function() -- 74004
	...
	local needFixing = {}
	if JustBecomeProDB.Version < 74004 then
		...
		if JustBecomeProDB.global and JustBecomeProDB.global.Groups then
			for _, gs in pairs(JustBecomeProDB.global.Groups) do
				tinsert(needFixing, gs.SortPriorities)
			end
		end
...
IconPosition_Sortable:RegisterConfigPanel_XMLTemplate(20, "JustBecomePro_GM_IconPosition_Sortable")
IconPosition_Sortable:RegisterConfigPanel_XMLTemplate(21, "JustBecomePro_GM_IconPosition_Sortable_Dir")
...
function IconPosition_Sortable:RegisterIconSorter(identifier, data, func)
	...
	JBP:ValidateType("data", sig, data, "table")
	JBP:ValidateType("func", sig, func, "function")

	JBP:ValidateType("data.DefaultOrder", sig, data.DefaultOrder, "number")
	JBP:ValidateType("data[1]", sig, data[1], "string")
	JBP:ValidateType("data[-1]", sig, data[-1], "string")
...
function IconPosition_Sortable:OnNewInstance_IconPosition_Sortable()
	self.SortedIcons = {}
	self.SortedIconsManager = JBP.Classes.UpdateTableManager:New()
...
function IconPosition_Sortable:OnEnable()
	...
	if (self.ShrinkGroup or group.SortPriorities[1].Method ~= "id") and group.numIcons > 1 then
		JBP:RegisterCallback("JBP_ONUPDATE_TIMECONSTRAINED_POST", self)
		JBP:RegisterCallback("JBP_ICON_UPDATED", self)
	end
	JBP:RegisterCallback("JBP_GROUP_SETUP_POST", self)
end
	
function IconPosition_Sortable:OnDisable()
	wipe(self.SortedIcons)
	
	JBP:UnregisterCallback("JBP_ONUPDATE_TIMECONSTRAINED_POST", self)
	JBP:UnregisterCallback("JBP_ICON_UPDATED", self)
	JBP:UnregisterCallback("JBP_GROUP_SETUP_POST", self)
end
	...
	for p = 1, #SortPriorities do
		...
		if Locked or method == "id" then
			...
			if data then
				...
				JBP:Warn("Missing icon sorter with identifier " .. method)
			end
		end
	end
end
...
function IconPosition_Sortable:AdjustIconsForModNumRowsCols(deltaRows, deltaCols)
	...
	if deltaRows ~= 0 and LayoutDirection >= 5 then
		...
		local iconsCopy = JBP.shallowCopy(group:GetSettings().Icons)
...
function IconPosition_Sortable:JBP_ONUPDATE_TIMECONSTRAINED_POST(event, time, Locked)
...
function IconPosition_Sortable:JBP_ICON_UPDATED(event, icon)
	if self.group == icon.group then
		self.iconSortNeeded = true
	end
end

function IconPosition_Sortable:JBP_GROUP_SETUP_POST(event, group)
