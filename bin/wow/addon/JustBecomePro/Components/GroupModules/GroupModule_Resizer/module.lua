...
JBP:NewClass("GroupModule_Resizer", "GroupModule", "Resizer_Generic"){
	...
	METHOD_EXTENSIONS = {
		OnImplementIntoGroup = function(self)
			...
			if JBP.Locked or not GroupPosition:CanMove() then
				self:Disable()
			else
				self:Enable()
			end
			...
			JBP:TT(self.resizeButton, self.tooltipTitle, self.tooltipText .. "\r\n" .. L["RESIZE_TOOLTIP_CHANGEDIMS"], 1, 1)
		end,
	...
	OnDisable = function(self)
		...
		-- This module is never really being disabled unless we are LockToggle()-ing JBP,
	...
	SizeUpdate_RightButton = function(resizeButton)
		...
		newColumns = min(JUSTBECOMEPRO_MAXROWS, max(1, newColumns))
		...
		newRows = min(JUSTBECOMEPRO_MAXROWS, max(1, newRows))
		
		if newColumns ~= group.Columns or newRows ~= group.Rows then
			local gs = group:GetSettings()

			local ics_old = JBP.CI.ics
			...
			if JBP.CI.ics ~= ics_old then
				JBP.IE:LoadIcon(1, false)
			end
		end
	end,
}
...
JBP:NewClass("GroupModule_Resizer_ScaleXY", "GroupModule_Resizer"){
...
JBP:NewClass("GroupModule_Resizer_ScaleY_SizeX", "GroupModule_Resizer"){
...
JBP:NewClass("GroupModule_Resizer_ScaleX_SizeY", "GroupModule_Resizer"){
