...
JBP.Classes.SharableDataType.types.group:RegisterMenuBuilder(10, function(Item_group)
	...
	info.func = function()
		...
		destgs.Point = CopyTable(JBP.Group_Defaults.Point)
		
		JBP:CopyTableInPlaceUsingDestinationMeta(gs.Point, destgs.Point, true)

		destgs.Scale = gs.Scale or JBP.Group_Defaults.Scale
		destgs.Level = gs.Level or JBP.Group_Defaults.Level
		destgs.Strata = gs.Strata or JBP.Group_Defaults.Strata
...
function Module:Sorter_Bucket(suggestions, buckets)
	...
	for i = 1, #suggestions do
		...
		if strlowerCache[name] == lastName_unmodified then
			-- Exact matches first
			depth = -2
		elseif frame.class == JBP.C.Group then
...
function GameTooltip:JBP_SetFrameHighlight(frame)
