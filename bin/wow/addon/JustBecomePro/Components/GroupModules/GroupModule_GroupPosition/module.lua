...
local GroupPosition = JBP:NewClass("GroupModule_GroupPosition", "GroupModule")
...
GroupPosition:RegisterConfigPanel_XMLTemplate(10, "JustBecomePro_GM_GroupPosition")
...
local function GetAnchoredPoints(group, wasMove)
	...
	local relframe = JBP.GUIDToOwner[p.relativeTo] or _G[p.relativeTo] or UIParent
	...
	if JBP:ParseGUID(p.relativeTo) then
...
JBP_CursorAnchor = CreateFrame("Frame", "JBP_CursorAnchor", UIParent)
function JBP_CursorAnchor:Initialize()
	self:SetSize(24, 24)
	...
	self.fs = JBP_CursorAnchor:CreateFontString(nil, "OVERLAY", "NumberFontNormalSmall")
	...
	self:SetScript("OnDragStop", function(self)
		...
		JBP.IE.db.profile.CursorAnchorPoint = {self:GetPoint()}
		JBP.IE.db.profile.CursorAnchorPoint[2] = nil -- don't store the parent. Its always UIParent.
	end)

	JBP:RegisterCallback("JBP_GLOBAL_UPDATE_POST", self, "CheckState")

	self:CheckState()

	self.Initialize = JBP.NULLFUNC
end


JBP:RegisterCallback("JBP_OPTIONS_LOADING", function()
	JBP.IE:RegisterDatabaseDefaults({
		profile = {
			CursorAnchorPoint = {
...
function JBP_CursorAnchor:Start()
	self.Started = true
end

function JBP_CursorAnchor:CheckState()
	self:ClearAllPoints()

	self:SetScale(1/UIParent:GetScale())

	if JBP.Locked or not JBP.IE  then
		...
		JBP:TT(self, "ANCHOR_CURSOR_DUMMY", "ANCHOR_CURSOR_DUMMY_DESC")

		if JBP.IE then
			self:SetPoint(unpack(JBP.IE.db.profile.CursorAnchorPoint))
		else
...
function JBP_CursorAnchor:OnUpdate()
	...
	if FrameStackTooltip and FrameStackTooltip:IsShown() then
		...
		if not warnedFstack then
			warnedFstack = true
			JBP:Print("Framestack detected. Shifting cursor anchor by 1px while fstack is up so it isn't in the way.")
		end
	end
...
function GroupPosition:OnDisable()
	JBP:UnregisterCallback("JBP_ONUPDATE_TIMECONSTRAINED_PRE", self, "DetectFrame")
end
...
function GroupPosition:DetectFrame(event, time, Locked)
	...
	if JBP.GUIDToOwner[frameToFind] or _G[frameToFind] then
		self:SetPos()
		JBP:UnregisterCallback("JBP_ONUPDATE_TIMECONSTRAINED_PRE", self, "DetectFrame")
	end
end
...
function GroupPosition:UpdatePositionAfterMovement(wasMove)
	...
	elseif wasMove then
		...
		JBP:Print(L["GROUP_CANNOT_INTERACTIVELY_POSITION"])
	end
...
function GroupPosition:SetPos()
	...
	if relativeTo:lower() == "cursor" then
		relativeTo = "JBP_CursorAnchor"
	end

	relativeTo = JBP.GUIDToOwner[relativeTo] or _G[relativeTo]
	
	if not relativeTo then
		self.frameToFind = p.relativeTo
		JBP:RegisterCallback("JBP_ONUPDATE_TIMECONSTRAINED_PRE", self, "DetectFrame")
		group:SetPoint("CENTER", UIParent)
	else
		if relativeTo == JBP_CursorAnchor then
			JBP_CursorAnchor:Start()
		end
		...
		if not success then
			...
			if err:find("trying to anchor to itself") then
				JBP:Print(L["ERROR_ANCHORSELF"]:format(L["fGROUP"]):format(group:GetGroupName(1)))

			elseif err:find("dependent on this") then
				local thisName = L["fGROUP"]:format(group:GetGroupName(1))
				local relativeToName

				if relativeTo.class == JBP.Classes.Group then
					relativeToName = L["fGROUP"]:format(relativeTo:GetGroupName(1))
				else
					relativeToName = relativeTo:GetName()
				end

				JBP:Print(L["ERROR_ANCHOR_CYCLICALDEPS"]:format(thisName, relativeToName, relativeToName, thisName))
			end
...
function GroupPosition:Reset()
	...
	for k, v in pairs(JBP.Group_Defaults.Point) do
		gs.Point[k] = v
	end

	gs.Level = JBP.Group_Defaults.Level
	gs.Scale = 1
	gs.Locked = JBP.Group_Defaults.Locked
	gs.Strata = JBP.Group_Defaults.Strata
