...
local IconPosition = JBP:NewClass("GroupModule_IconPosition", "GroupModule")
...
IconPosition:RegisterConfigPanel_XMLTemplate(30, "JustBecomePro_GM_IconPosition")
...
function IconPosition:ClobberCheck(ics)
	if not JBP:DeepCompare(JBP.DEFAULT_ICON_SETTINGS, ics) then
		if not clobberWarned then
			JBP:Print(JBP.L["RESIZE_GROUP_CLOBBERWARN"])
			clobberWarned = true
		end
		return true
	end
...
function IconPosition:JBP_GROUP_SETUP_POST(event, group)
