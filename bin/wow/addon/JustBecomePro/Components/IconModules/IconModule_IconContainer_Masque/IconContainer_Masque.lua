...
local IconContainer_Masque = JBP:NewClass("IconModule_IconContainer_Masque", "IconModule_IconContainer")


if LibStub("LibButtonFacade", true) and select(6, GetAddOnInfo("Masque")) == "MISSING" then
	JBP:Warn("JustBecomePro no longer supports ButtonFacade. If you wish to continue to skin your icons, please upgrade to ButtonFacade's successor, Masque.")
end


local function GetLMBGroup(icon)
	if icon.group.Domain == "global" then
		return LMB:Group("JustBecomePro", L["DOMAIN_GLOBAL"] .. " " .. L["fGROUP"]:format(icon.group:GetID()))
	else
		return LMB:Group("JustBecomePro", L["fGROUP"]:format(icon.group:GetID()))
	end
end
...
if LMB.GetSpellAlert then
	-- Copied (and slightly modified) from masque so that JBP's activation borders will get skinned properly when they aren't square.
...
IconContainer_Masque:SetIconEventListner("JBP_ICON_SETUP_POST", function(Module, icon)
	Module:DoSkin()
end)
