...
local IconContainer = JBP:NewClass("IconModule_IconContainer", "IconModule")
...
function IconContainer:GetOverlayGlow()
	...
	if ( not overlay ) then
		...
		overlay = CreateFrame("Frame", "JBP_ActionButtonOverlay" .. numOverlays, UIParent, "ActionBarButtonSpellActivationAlert");
...
IconContainer:RegisterEventHandlerData("Animations", 60, "ACTVTNGLOW", {
	...
	OnUpdate = function(icon, table)
		if table.Duration - (JBP.time - table.Start) < 0 then
