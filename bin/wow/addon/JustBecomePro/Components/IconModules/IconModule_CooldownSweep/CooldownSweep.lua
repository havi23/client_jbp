...
local CooldownSweep = JBP:NewClass("IconModule_CooldownSweep", "IconModule")
...
CooldownSweep:RegisterConfigPanel_ConstructorFunc(200, "JustBecomePro_TimerSettings", function(self)
	...
	self:BuildSimpleCheckSettingFrame({
		...
		function(check)
			...
			check:CScriptAdd("ReloadRequested", function()
				check:SetEnabled(JBP.CI.ics.ShowTimer)
			end)
		end,
		function(check)
			...
			check:CScriptAdd("ReloadRequested", function()
				check:SetShown(not JBP.CI.icon.typeData.hasNoGCD)

				check:SetEnabled(JBP.CI.ics.ShowTimer or JBP.CI.ics.ShowTimerText or JBP.CI.ics.ShowTimerTextnoOCC)
			end)
		end,
		function(check)
...
CooldownSweep:RegisterConfigPanel_ConstructorFunc(8, "JustBecomePro_TimerSettings_Main", function(self)
...
JBP:RegisterUpgrade(60436, {
	icon = function(self, ics)
		ics.ShowTimerTextnoOCC = ics.ShowTimerText
	end,
})

JBP:RegisterUpgrade(60315, {
	icon = function(self, ics)
		...
		local old = JBP.db.profile.ClockGCD
		if old == nil then
			old = true
		end
...
JBP:RegisterCallback("JBP_DB_PRE_DEFAULT_UPGRADES", function() -- 45607
	...
	if JustBecomeProDB.profiles and JustBecomeProDB.Version < 45607 then
...
function CooldownSweep:SetupForIcon(icon)
	...
	if omnicc_loaded
	or tullacc_loaded
	or JBP.db.profile.ForceNoBlizzCC
	...
	self.cooldown:SetDrawEdge(self.ShowTimer and JBP.db.profile.DrawEdge)
	self.cooldown:SetDrawSwipe(self.ShowTimer)
	self.cooldown:SetDrawBling(not JBP.db.profile.HideBlizzCDBling)
...
function CooldownSweep:UpdateCooldown()
	...
	if mainDuration > 0 then
		...
		if self.ShowTimer then
			cd:SetDrawEdge(JBP.db.profile.DrawEdge)
...
function CooldownSweep:DURATION(icon, start, duration)
	if (not self.ClockGCD and OnGCD(duration)) or (duration - (JBP.time - start)) <= 0 or duration <= 0 then
		start, duration = 0, 0
	end
...
JBP:RegisterCallback("JBP_ONUPDATE_TIMECONSTRAINED_POST", function()
