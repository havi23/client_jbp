...
local TEXT = JBP:NewModule("TextDisplay")
JBP.TEXT = TEXT
...
JBP:RegisterDatabaseDefaults{
	global = {
		TextLayouts = {
			...
			-- It has no displays and cannot be edited. It should also be hidden from the text layout configuration in JBP's main options.
			[""] = {
	...
	profile = function(self, profile, profileName)
		...
		for GUID, layout in pairs(profile.TextLayouts) do
			if type(layout) == "table" and not layout.NoEdit and type(layout.GUID) == "string" then
				...
				if not layout.GUID:match("^JBP:textlayout") then
					-- Change the GUID into the new GUID syntax.

					local newGUID = "JBP:textlayout:" .. layout.GUID

					GUIDmap[GUID] = newGUID
					layout.GUID = newGUID
				end

				if not rawget(JBP.db.global.TextLayouts, layout.GUID) then
					...
					JBP:CopyTableInPlaceUsingDestinationMeta(layout, JBP.db.global.TextLayouts[layout.GUID])

					-- Make sure we upgrade the copy of the layout, not the old one that will be discarded
					layout = JBP.db.global.TextLayouts[layout.GUID]
					...
					-- old upgrades.
					JBP:StartUpgrade("textlayout", profile.Version, layout, layout.GUID)
				else
					-- The layout does already exist.

					local existingName = JBP.db.global.TextLayouts[layout.GUID].Name
					JBP.db.global.TextLayouts[layout.GUID].Name = ""
					...
					local layoutWithMetatables = JBP:CopyTableInPlaceUsingDestinationMeta(layout, JBP.db.global.TextLayouts["\000"])
					JBP.db.global.TextLayouts["\000"] = nil


					local comparison = JBP:DeepCompare(JBP.db.global.TextLayouts[layout.GUID], layoutWithMetatables)

					JBP.db.global.TextLayouts[layout.GUID].Name = existingName
					...
					if comparison then
						...
						local newGUID = JBP:GenerateGUID("textlayout", JBP.CONST.GUID_SIZE)
...
JBP:RegisterUpgrade(60038, {
	group = function(self, gs)
		gs.Fonts = nil
	end
})

JBP:RegisterUpgrade(60029, {
	textlayout = function(self, settings, GUID)
...
JBP:RegisterUpgrade(51019, {
	textlayout = function(self, settings, GUID)
		...
		if GUID == "icon" and settings.GUID == "" then
			JBP.db.global.TextLayouts[GUID] = nil
			JBP:Warn("JBP has deleted the invalid text layout keyed as 'icon' that was probably causing errors for you. If you were using it on any of your icons, then I apologize, but you probably weren't because it probably wasn't even named")
		end
	end,
})

JBP:RegisterUpgrade(51003, {
	...
	group = function(self, gs)
		...
		for GUID, layoutSettings in pairs(JBP.db.global.TextLayouts) do
		...
		repeat
			...
			for k, layoutSettings in pairs(JBP.db.global.TextLayouts) do
				if layoutSettings.Name == Name then
					...
					Name = JBP.oneUpString(Name) or GUID -- fallback on the GUID if we cant increment the name for some reason
		...
		JBP.db.global.TextLayouts[GUID] = layout
...
JBP:RegisterUpgrade(51002, {
	-- This is the upgrade that handles the transition from JBP's ghetto text substitutions to DogTag.
	
	-- self.translateString is a function defined in the v51002 upgrade in JustBecomePro.lua.
...
JBP:RegisterCallback("JBP_UPGRADE_PERFORMED", function(event, type, upgradeData, ...)
	...
	if type == "global" then
		for GUID, settings in pairs(JBP.db.global.TextLayouts) do
			JBP:Upgrade("textlayout", upgradeData, settings, GUID)
		end
	end
end)
...
function TEXT:GetTextLayoutForIconSettings(gs, ics, view)
	...
	local layoutSettings = GUID and rawget(JBP.db.global.TextLayouts, GUID)
	...
	if not layoutSettings then
		...
		local GroupDefaultsPerView = JBP.Group_Defaults.SettingsPerView
		...
		layoutSettings = rawget(JBP.db.global.TextLayouts, GUID)
...
function TEXT:GetTextFromSettingsAndLayout(Texts, layoutSettings, textID)
	JBP:ValidateType(2, "TEXT:GetTextForIconAndLayout()", Texts, "table")
	JBP:ValidateType(3, "TEXT:GetTextForIconAndLayout()", layoutSettings, "table")
	JBP:ValidateType(4, "TEXT:GetTextForIconAndLayout()", textID, "number")
...
local Texts = JBP:NewClass("IconModule_Texts", "IconModule")

Texts:RegisterConfigPanel_XMLTemplate(400, "JustBecomePro_TextDisplayOptions")
...
function Texts:CreateFontString(id)
	...
	JBP:Fire("JBP_DEBUG_FS_CREATED", self.icon, fontString, id)
	...
	if relativeTo:sub(1, 2) == "$$" then
		...
		if not _G[relativeTo] then
			if self.hasSetupOnce then
				...
				JBP:RegisterSelfDestructingCallback("JBP_ICON_SETUP_POST", reSetup, self)
...
function Texts:CheckAnchorValidity()
	local _, layoutSettings = JBP.TEXT:GetTextLayoutForIcon(self.icon) 

	for textID, fontStringSettings in JBP:InNLengthTable(layoutSettings) do
...
function Texts:SetupForIcon(sourceIcon)
	...
	local _, layoutSettings = JBP.TEXT:GetTextLayoutForIcon(sourceIcon) 
	...
	for _, fontString in pairs(self.fontStrings) do
		fontString.JBP_QueueForRemoval = true
	end
	local queueMasqeSkin = nil

	if layoutSettings then
		-- Detect if masque skinning is being used on this icon.
		local isDefaultSkin = not JBP.C.IconModule_IconContainer_Masque:IsIconSkinned(icon)
	...
	-- JBP_QueueForRemoval gets set to nil for valid stings in OnKwargsUpdated, among other things
	...
	for fontStringID, fontString in pairs(self.fontStrings) do		
		if fontString.JBP_QueueForRemoval then
			fontString.JBP_QueueForRemoval = nil
			DogTag:RemoveFontString(fontString)
			fontString:Hide()
		end
	end
...
function Texts:OnKwargsUpdated()
	if self.layoutSettings and self.Texts then
		for textID, fontStringSettings in JBP:InNLengthTable(self.layoutSettings) do
			...
			if fontString and text and text ~= "" then
				...
				DogTag:AddFontString(fontString, self.icon, styleString .. text, JBP.DOGTAG.nsList, self.kwargs)
			end
		end
	end
end
