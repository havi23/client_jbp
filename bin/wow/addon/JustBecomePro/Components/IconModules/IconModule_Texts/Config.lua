...
-- GLOBALS: JustBecomePro_TextDisplayOptions, JUSTBECOMEPRO_VERSIONNUMBER
...
local HistorySet = JBP.C.HistorySet:New("TEXTLAYOUTS")
...
JBP:NewClass("Config_TextLayout_List", "Config_Frame") {
	OnNewInstance = function(self)
		self.frames = {}

		JBP:ConvertContainerToScrollFrame(self, true, nil, 8)
	end,

	GetTextLayoutFrame = function(self, id)
		local frame = self.frames[id]
		if not frame then
			frame = JBP.C.Config_TextLayout_ListItem:New("Frame", nil, self, "JustBecomePro_TextLayout_ListItem", id)
	...
	ReloadSetting = function(self)
		local id = 0
		local firstGUID
		for GUID, layoutSettings in JBP:OrderedPairs(JBP.db.global.TextLayouts, layoutSort) do
			if GUID ~= "" then
				id = id + 1
...
JBP:NewClass("Config_TextLayout_ListItem", "Config_Frame") {
	OnNewInstance = function(self)
		self.frames = {}
	...
	SettingTableRequested = function(self)
		...
		local TextLayouts = JBP.db.global.TextLayouts
	...
	GetTextDisplayFrame = function(self, id)
		local frame = self.frames[id]
		if not frame then
			frame = JBP.C.Config_TextDisplay_ListItem:New("CheckButton", nil, self, "JustBecomePro_TextDisplay_ListItem", id)
			self.frames[id] = frame
	...
	tooltipFunction = function(Layout)
		...
		if layoutSettings.NoEdit then
			...
			if tooltip ~= "" then
				return JBP.L["TEXTLAYOUTS_USEDBY_HEADER"] .. "\n\n" .. tooltip .. "\n\n" .. L["CLICK_TO_EDIT"]
			else
				return JBP.L["TEXTLAYOUTS_USEDBY_NONE"] .. "\n\n" .. L["CLICK_TO_EDIT"]
			end
		end
	end,
	...
	ReloadSetting = function(self)
		...
		if layoutSettings then
			...
			JBP:TT(self.Layout, name, self.tooltipFunction, 1, 1)

			-- Setup text display frames
			for id, displaySettings in JBP:InNLengthTable(layoutSettings) do
		...
		if self.Layout:GetChecked() then
			...
			JBP:AnimateHeightChange(self, self.Layout:GetHeight(), 0.2)
		end
	end,
...
JBP:NewClass("Config_TextDisplay_ListItem", "Config_CheckButton") {
	ReloadSetting = function(self)
		local layoutSettings = self:GetSettingTable()
...
function TEXT:GetCurrentLayoutAndDisplay()
	if not TEXT.currentLayout or not rawget(JBP.db.global.TextLayouts, TEXT.currentLayout) then
		TEXT.currentLayout = nil
		TEXT.currentDisplay = nil
	...
	local layoutSettings = JBP.db.global.TextLayouts[TEXT.currentLayout]
...
function TEXT:GetCurrentLayoutAndDisplaySettings()
	...
	local layoutSettings = JBP.db.global.TextLayouts[layoutGUID]
...
local panels = {
	JBP.C.StaticConfigPanelInfo:New(1, "LayoutSettings"),
	JBP.C.StaticConfigPanelInfo:New(2, "DisplaySettings"),
	JBP.C.StaticConfigPanelInfo:New(3, "MasqueWarn"),
	JBP.C.StaticConfigPanelInfo:New(4, "DisplayFontSettings"),
}
local anchorPanels = {}
...
function TEXT:SetupPanels()
	...
	if displaySettings then
		...
		for id, anchorSettings in JBP:InNLengthTable(Anchors) do
			if not anchorPanels[id] then
				...
				local panel = CreateFrame("Frame", nil, page.Panels, "JustBecomePro_TextDisplay_Anchor", id)
				local key = "Anchor" .. id
				page.Panels[key] = panel

				anchorPanels[id] = JBP.C.StaticConfigPanelInfo:New(10 + id, key)
				tinsert(panels, anchorPanels[id])
			end
		end
	end

	JBP.IE:PositionPanels("TextLayouts", panels)
end

function TEXT:Clonelayout(sourceGUID)
	local GUID = JBP:GenerateGUID("textlayout", JBP.CONST.GUID_SIZE)

	local Item = JBP.Classes.SettingsItem:New("textlayout")
	Item.Settings = TEXT:GetTextLayoutSettings(sourceGUID)
	Item.Version = JUSTBECOMEPRO_VERSIONNUMBER
	Item.ImportSource = JBP.C.ImportSource.types.Profile
...
function TEXT:DeleteDisplay(layoutGUID, displayID)
	...
	for i, fontStringSettings in JBP:InNLengthTable(layoutSettings) do
		for _, anchorSettings in JBP:InNLengthTable(fontStringSettings.Anchors) do
...
function TEXT:AddTextLayout()
	local GUID = JBP:GenerateGUID("textlayout", JBP.CONST.GUID_SIZE)
	local newLayout = JBP.db.global.TextLayouts[GUID]
	newLayout.GUID = GUID
	
	local Name = "New 1"
	repeat
		local found
		for k, layoutSettings in pairs(JBP.db.global.TextLayouts) do
			if layoutSettings.Name == Name then
				Name = JBP.oneUpString(Name)
				found = true
				break
			end
		end
		...
		elseif v == GUID then
			...
			elseif parentTableKey == "Groups" then
				...
				if not TEXT.TextLayout_NumTimesUsedTemp[gs] then
					...
					n = n + ((gs.Rows or JBP.Group_Defaults.Rows) * (gs.Columns or JBP.Group_Defaults.Columns))
				end
			end
		end
	end
...
function TEXT:GetNumTimesUsed(layoutGUID)
	...
	do  -- Global groups
		local n = deepRecScanTableForLayout(JBP.db.global, layoutGUID, JBP.db.global)
		if n > 0 then
			result = result .. L["TEXTLAYOUTS_DELETELAYOUT_CONFIRM_LISTING"]:format(L["DOMAIN_GLOBAL"], n) .. "\r\n"
		end
	end

	for profileName, profile in pairs(JBP.db.profiles) do
		local n = deepRecScanTableForLayout(profile, layoutGUID, profile)

		if n > 0 then
			if profileName == JBP.db:GetCurrentProfile() then
...
function TEXT.Anchor_DropdownGenerator(dropdown)
	...
	local displayID = JBP.tContains(layoutSettings, displaySettings)

	local t = {
		[""] = L["ICON"],
	}
	
	for otherDisplayID, otherDisplaySettings in JBP:InNLengthTable(layoutSettings) do
	...
	for IconModule in pairs(JBP.Classes.IconModule.inheritedBy) do
...
function TEXT:GetTextLayoutSettings(GUID)
	return GUID and rawget(JBP.db.global.TextLayouts, GUID) or nil
end
...
function TEXT.Layout_DropDown(dropdown)
	for GUID, settings in JBP:OrderedPairs(JBP.db.global.TextLayouts, layoutSort) do
		if GUID ~= "" then
			...
			local displays = ""
			for i, fontStringSettings in JBP:InNLengthTable(settings) do
...
function TEXT:CacheUsedStrings()
	...
	for ics, gs in JBP:InIconSettings() do
...
function TEXT.CopyString_DropDown(dropdown)
	...
	for text, num in JBP:OrderedPairs(usedStrings, nil, true, true) do
		local info = JBP.DD:CreateInfo()
...
function TEXT:LoadConfig()
	if not JustBecomePro_TextDisplayOptions or not CI.icon then
		return
	end
	...
	if layoutSettings then
		local previousFrame
		for i, stringSettings in JBP:InNLengthTable(layoutSettings) do
			local frame = TEXT[i]
			
			local text = TEXT:GetTextFromSettingsAndLayout(Texts, layoutSettings, i)
			
			if not frame then
				frame = CreateFrame("Frame", "$parentString"..i, JustBecomePro_TextDisplayOptions, "JustBecomePro_TextDisplayGroup", i)
				TEXT[i] = frame

				if i == 1 then
					frame:SetPoint("TOPLEFT", JustBecomePro_TextDisplayOptions.Layout, "BOTTOMLEFT", 0, 0)
				else
					frame:SetPoint("TOP", previousFrame, "BOTTOM")
				end

				-- Setup the tooltip for the reset button.
				JBP:TT(frame.Default, "TEXTLAYOUTS_STRING_SETDEFAULT", ttText, nil, 1)
			end
			...
			frame.Error:SetText(JBP:TestDogTagString(CI.icon, text))
	...
	JustBecomePro_TextDisplayOptions.Layout.PickLayout:SetText("|cff666666" .. L["TEXTLAYOUTS_HEADER_LAYOUT"] .. ": |r" .. layoutName)
	

	-- Set the error text for the entire layout (show if we are using a fallback layout)
	JustBecomePro_TextDisplayOptions.Layout.Error:SetText(isFallback and L["TEXTLAYOUTS_ERROR_FALLBACK"] or nil)
	
	-- Validate the anchors for the text layout on the icon.
	-- If there are invalid anchors, display an error message.
	local IconModule_Texts = JBP.CI.icon:GetModuleOrModuleChild("IconModule_Texts")
	if IconModule_Texts then
		local err = IconModule_Texts:CheckAnchorValidity()
		if err then
			JustBecomePro_TextDisplayOptions.Layout.Error:SetText(err)
		end
	end
	...
	JBP:TT(JustBecomePro_TextDisplayOptions.Layout.LayoutSettings, "TEXTLAYOUTS_LAYOUTSETTINGS", L["TEXTLAYOUTS_LAYOUTSETTINGS_DESC"]:format(layoutName), nil, 1)
end

function TEXT:ResizeParentFrame()
	local layoutHeight = 26 + JustBecomePro_TextDisplayOptions.Layout.Error:GetHeight()
	
	JustBecomePro_TextDisplayOptions.Layout:SetHeight(layoutHeight)
	
	JustBecomePro_TextDisplayOptions:AdjustHeight(10)
end
...
JBP.C.GroupModule_BaseConfig:RegisterConfigPanel_XMLTemplate(400, "JustBecomePro_GM_TextLayout")
...
JBP:RegisterCallback("JBP_ICON_PREPARE_SETTINGS_FOR_COPY", function(event, ics, gs)
	...
	for view, settingsPerView in pairs(ics.SettingsPerView) do
		...
		if not GUID then
			local GUID_group = JBP.approachTable(gs, "SettingsPerView", view, "TextLayout")
			if GUID_group and GUID_group ~= JBP.approachTable(JBP.Group_Defaults, "SettingsPerView", view, "TextLayout") then
				GUID = GUID_group
			end
		end
...
local textlayout = JBP.Classes.SharableDataType:New("textlayout", 15)
textlayout.extrasMap = {"GUID"}
function textlayout:Import_ImportData(Item, GUID)
	assert(type(GUID) == "string")
	
	JBP.db.global.TextLayouts[GUID] = nil -- restore defaults
	local textlayout = JBP.db.global.TextLayouts[GUID]
	JBP:CopyTableInPlaceUsingDestinationMeta(Item.Settings, textlayout, true)
	...
	repeat
		...
		for k, layoutSettings in pairs(JBP.db.global.TextLayouts) do
			if layoutSettings ~= textlayout and layoutSettings.Name == textlayout.Name then
				textlayout.Name = JBP.oneUpString(textlayout.Name)
				found = true
				break
			end
		end
	...
	if version then
		if version > JUSTBECOMEPRO_VERSIONNUMBER then
			...
			JBP:StartUpgrade("textlayout", version, textlayout, GUID)
		end
	end
...
function textlayout:Import_CreateMenuEntry(info, Item, doLabel)
	info.text = JBP.TEXT:GetLayoutName(Item.Settings, Item:GetExtra("GUID"))
...
JBP.C.SharableDataType.types.database:RegisterMenuBuilder(17, function(Item_database)
	...
	if db.global.TextLayouts then
		...
		local SettingsBundle = JBP.Classes.SettingsBundle:New("textlayout")
...
JBP.C.SharableDataType.types.profile:RegisterMenuBuilder(20, function(Item_profile)
...
textlayout:RegisterMenuBuilder(1, function(Item_textlayout)
	...
	local layoutSettings = JBP.TEXT:GetTextLayoutSettings(GUID)
	...
	if layoutSettings then
		...
		info.func = function()
			Item_textlayout:Import(JBP:GenerateGUID("textlayout", JBP.CONST.GUID_SIZE))
		end
		JBP.DD:AddButton(info)
	else
...
function textlayout:Export_SetButtonAttributes(editbox, info)
	...
	local text = L["fTEXTLAYOUT"]:format(JBP.TEXT:GetLayoutName(nil, GUID))
	info.text = text
...
function textlayout:Export_GetArgs(editbox)
	...
	local GUID = EXPORTS[self.type]
	assert(type(GUID) == "string")
	local settings = JBP.TEXT:GetTextLayoutSettings(GUID)
	
	return settings, JBP.Defaults.global.TextLayouts["**"], GUID
end
	...
	if type == "icon" or type == "group" then
		for view, settingsPerView in pairs(settings.SettingsPerView) do
			...
			if GUID and GUID ~= "" then
				local layout = rawget(JBP.db.global.TextLayouts, GUID)
				if layout and not layout.NoEdit then
					JBP:GetSettingsStrings(strings, "textlayout", layout, JBP.Defaults.global.TextLayouts["**"], GUID)
				end
			end
		end
	end
...
JBP:RegisterCallback("JBP_EXPORT_SETTINGS_REQUESTED", GetTextLayouts)