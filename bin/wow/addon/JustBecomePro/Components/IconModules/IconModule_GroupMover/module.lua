...
local Module = JBP:NewClass("IconModule_GroupMover", "IconModule"){
...
Module:SetScriptHandler("OnDragStart", function(Module, icon, button)
	if button == "LeftButton" then
		...
		if not JBP.Locked and GroupPosition and GroupPosition:CanMove() then
...
JBP:RegisterCallback("JBP_GROUP_HIDE_PRE", function(event, group)
...
JBP.IconDragger:RegisterIconDragHandler(330,	-- Anchor
	function(IconDragger, info)
		...
		elseif IconDragger.destFrame and IconDragger.destFrame:GetName() then
			if IconDragger.destFrame == WorldFrame and IconDragger.srcicon.group.Point.relativeTo ~= "UIParent" then
				...
				if JBP:ParseGUID(currentFrameName) then
					thing = JBP.GUIDToOwner[currentFrameName]
				else
					thing = _G[currentFrameName]
				end

				if type(thing) == "table" then
					if thing.class == JBP.Classes.Icon then
						currentFrameName = thing:GetIconName(1)
					elseif thing.class == JBP.Classes.Group then
	...
	function(IconDragger)
		if IconDragger.desticon then
			-- we are anchoring to another JBP group, so dont operate on the same group.
