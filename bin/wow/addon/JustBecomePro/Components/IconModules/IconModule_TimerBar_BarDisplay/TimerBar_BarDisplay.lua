...
local TimerBar_BarDisplay = JBP:NewClass("IconModule_TimerBar_BarDisplay", "IconModule_TimerBar")
...
JBP:RegisterUpgrade(80006, {
	icon = function(self, ics)
		ics.TimerBar_StartColor    = JBP:RGBATableToStringWithFallback(ics.BarDisplay_StartColor,    "ffff0000")
		ics.TimerBar_MiddleColor   = JBP:RGBATableToStringWithFallback(ics.BarDisplay_MiddleColor,   "ffffff00")
		ics.TimerBar_CompleteColor = JBP:RGBATableToStringWithFallback(ics.BarDisplay_CompleteColor, "ff00ff00")
...
TimerBar_BarDisplay:RegisterConfigPanel_XMLTemplate(210, "JustBecomePro_BarDisplayBarOptions")
	...
	if duration then
		...
		if self.Invert then
			if duration == 0 then
				...
				local value = JBP.time - self.start + self.Offset
			...
			if duration == 0 then
				...
				local value = duration - (JBP.time - self.start) + self.Offset
...
function TimerBar_BarDisplay:SetupColors(icon, valueColor, unit)
			...
			if class then
				...
				if color then
					...
					if color.colorStr then
						...
						color = JBP:RGBATableToStringWithoutFlags(color)
					end
	...
	self:SetColors(
		JBP:GetColors(colorSettingNames, "TimerBar_EnableColors",
		              icon.group:GetSettings(), JBP.db.global)
	)
end
...
TimerBar_BarDisplay:SetIconEventListner("JBP_ICON_SETUP_POST", function(Module, icon)
	if JBP.Locked then
