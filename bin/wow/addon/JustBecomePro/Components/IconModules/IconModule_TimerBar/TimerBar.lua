...
local TimerBar = JBP:NewClass("IconModule_TimerBar", "IconModule", "UpdateTableManager")
...
-- Group defaults
JBP:MergeDefaultsTables(settings, JBP.Group_Defaults)

-- Global defaults (global doesn't have the Enable_Colors setting)
settings.TimerBar_EnableColors = nil
JBP:MergeDefaultsTables(settings, JBP.Defaults.global)


TimerBar:RegisterConfigPanel_XMLTemplate(52, "JustBecomePro_TimerBar_GroupColors")
	:SetPanelSet("group")
	:SetColumnIndex(1)


TimerBar:RegisterConfigPanel_XMLTemplate(52, "JustBecomePro_TimerBar_GlobalColors")
	:SetPanelSet("global")
...
function TimerBar:GetValue()
	...
	if Invert then
		if duration == 0 then
			...
			local value = JBP.time - start + self.Offset
		...
		if duration == 0 then
			...
			local value = duration - (JBP.time - start) + self.Offset
...
function TimerBar:SetColors(startColor, halfColor, completeColor)
	self.startColor    = startColor and JBP:StringToCachedRGBATable(startColor)
	self.halfColor     = halfColor and JBP:StringToCachedRGBATable(halfColor)
	self.completeColor = completeColor and JBP:StringToCachedRGBATable(completeColor)
end
...
JBP:RegisterCallback("JBP_ONUPDATE_POST", function(event, time, Locked)
	local offs = 0
