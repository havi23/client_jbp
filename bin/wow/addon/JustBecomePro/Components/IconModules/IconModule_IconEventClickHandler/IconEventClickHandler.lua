...
local Module = JBP:NewClass("IconModule_IconEventClickHandler", "IconModule")
...
function Module:OnDisable()
	-- No reason to unregister from clicks. When JBP is locked, we unregister clicks towards the end of icon setup.
	-- When JBP is unlocked, clicks should always be enabled.
end

Module:SetIconEventListner("JBP_ICON_SETUP_POST", function(Module, icon)
...
function Module:REALALPHA(icon, realAlpha)
	if JBP.Locked and not icon.IsSettingUp then
