...
local Type = JBP.Classes.IconType:New("wpnenchant")
...
JBP:RegisterUpgrade(62216, {
	global = function(self)
		-- This table is now locale-specific.

		if type(JBP.db.global.WpnEnchDurs) == "table" then
			for k, v in pairs(JBP.db.global.WpnEnchDurs) do
				JBP.db.locale.WpnEnchDurs[k] = max(JBP.db.locale.WpnEnchDurs[k] or 0, v)
			end
			JBP.db.global.WpnEnchDurs = nil
		end
	end
})
JBP:RegisterUpgrade(62008, {
...
Type:RegisterConfigPanel_ConstructorFunc(120, "JustBecomePro_WeaponSlot", function(self)
	self:SetTitle(JBP.L["ICONMENU_WPNENCHANTTYPE"])
...
Type:RegisterConfigPanel_ConstructorFunc(150, "JustBecomePro_WpnEnchantSettings", function(self)
...
local Parser = CreateFrame("GameTooltip", "JustBecomePro_Parser", JBP, "GameTooltipTemplate")
local function GetWeaponEnchantName(slot)
	...
	local i = 1
	while _G["JustBecomePro_ParserTextLeft" .. i] do
		local t = _G["JustBecomePro_ParserTextLeft" .. i]:GetText()
		if t and t ~= "" then
...
JBP:RegisterCallback("JBP_GLOBAL_UPDATE", function()
	WpnEnchDurs = JBP.db.locale.WpnEnchDurs
end)
