...
local Type = rawget(JBP.Types, "wpnenchant")
...
function Module:OnInitialize()
	...
	for k, v in pairs(JBP.db.locale.WpnEnchDurs) do
...
function Module:OnSuggest()
	JBP:GetModule("ItemCache"):CacheItems()
	CurrentItems = JBP:GetModule("ItemCache"):GetCurrentItems()
end
...
function Module:Etc_GetTexture(name)
	...
	name = strlower(name)
	JBP.SpellTexturesMetaIndex[name] = JBP.SpellTexturesMetaIndex[name] or tex
...
function Module:Table_GetSorter()
	JBP:GetModule("ItemCache"):CacheItems(true)
	
	PlayerSpells = JBP:GetModule("ClassSpellCache"):GetPlayerSpells()
...
function Module.Sorter(a, b)
	...
	local haveA = rawget(JBP.db.locale.WpnEnchDurs, a)
	local haveB = rawget(JBP.db.locale.WpnEnchDurs, b)
	if haveA or haveB then
		if haveA and haveB then
			return a < b
		else
...
function Module:Entry_Colorize_1(f, name)
	...
	elseif rawget(JBP.db.locale.WpnEnchDurs, name) then
