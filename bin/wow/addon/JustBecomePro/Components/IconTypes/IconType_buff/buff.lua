...
-- GLOBALS: JustBecomePro_ChooseName


local Type = JBP.Classes.IconType:New("buff")
...
local STATE_PRESENT = JBP.CONST.STATE.DEFAULT_SHOW
local STATE_ABSENT = JBP.CONST.STATE.DEFAULT_HIDE
...
Type:RegisterIconDefaults{
	...
	-- EITHER is handled specially by JBP by having looping a second time for a second filter (FilterH in the code).
	...
	-- Hide the icon if JBP's unit system left icon.Units empty.
...
Type:RegisterConfigPanel_XMLTemplate(105, "JustBecomePro_Unit", {
	implementsConditions = true,
})

Type:RegisterConfigPanel_ConstructorFunc(120, "JustBecomePro_BuffOrDebuff", function(self)
	self:SetTitle(JBP.L["ICONMENU_BUFFTYPE"])
...
Type:RegisterConfigPanel_ConstructorFunc(125, "JustBecomePro_BuffSettings", function(self)
	...
	local function OnClick(button, arg1)
		JBP.CI.ics.ShowTTText = arg1
		JBP.IE:LoadIcon(1)
	end
	self.ShowTTText = JBP.C.Config_DropDownMenu:New("Frame", "$parentShowTTText", self, "JBP_DropDownMenuTemplate")
	...
	self.ShowTTText:SetFunction(function(self)
		...
		info.func = OnClick
		info.arg1 = false
		info.checked = info.arg1 == JBP.CI.ics.ShowTTText
		...
		for _, var in JBP:Vararg(1, 2, 3) do
			local info = JBP.DD:CreateInfo()
	...
	self:CScriptAdd("ReloadRequested", function(self, panel, panelInfo)
		self.ShowTTText:SetText((JBP.CI.ics.ShowTTText ~= false and "|cffff5959" or "") .. L["ICONMENU_SHOWTTTEXT2"])
	end)

	JBP.IE:DistributeFrameAnchorsLaterally(self, 2, self.HideIfNoUnits, self.ShowTTText)
...
Type:RegisterConfigPanel_ConstructorFunc(170, "JustBecomePro_SortSettingsWithStacks", function(self)
	self:SetTitle(JBP.L["SORTBY"])
	self:BuildSimpleCheckSettingFrame({
		numPerRow = 3,
		function(check)
			check:SetTexts(JBP.L["SORTBYNONE_DURATION"], JBP.L["SORTBYNONE_DESC"])
			check:SetSetting("Sort", false)
		end,
		function(check)
			check:SetTexts(JBP.L["ICONMENU_SORTASC"], JBP.L["ICONMENU_SORTASC_DESC"])
			check:SetSetting("Sort", -1)
		end,
		function(check)
			check:SetTexts(JBP.L["ICONMENU_SORTDESC"], JBP.L["ICONMENU_SORTDESC_DESC"])
			check:SetSetting("Sort", 1)
		end,

		function(check)
			check:SetTexts(JBP.L["SORTBYNONE_STACKS"], JBP.L["SORTBYNONE_DESC"])
			check:SetSetting("StackSort", false)
		end,
		function(check)
			check:SetTexts(JBP.L["ICONMENU_SORT_STACKS_ASC"], JBP.L["ICONMENU_SORT_STACKS_ASC_DESC"])
			check:SetSetting("StackSort", -1)
		end,
		function(check)
			check:SetTexts(JBP.L["ICONMENU_SORT_STACKS_DESC"], JBP.L["ICONMENU_SORT_STACKS_DESC_DESC"])
			check:SetSetting("StackSort", 1)
		end,
	})
...
local function Buff_OnEvent(icon, event, arg1)
	...
	elseif event == "JBP_UNITSET_UPDATED" and arg1 == icon.UnitSet then
...
local function Buff_OnUpdate(icon, time)
	...
	for u = 1, #Units do
		...
		-- is known by JBP.UNITS to definitely exist.
...
local Processor = JBP.Classes.IconDataProcessor:New("BUFF_SOURCEUNIT", "auraSourceUnit, auraSourceGUID")
function Processor:CompileFunctionSegment(t)
	...
	if attributes.auraSourceUnit ~= auraSourceUnit or attributes.auraSourceGUID ~= auraSourceGUID then
		...
		JBP:Fire(BUFF_SOURCEUNIT.changedEvent, icon, auraSourceUnit, auraSourceGUID)
		doFireIconUpdated = true
	end
	--]]
end
Processor:RegisterDogTag("JBP", "AuraSource", {
	code = function(icon)
	...
	events = JBP:CreateDogTagEventString("BUFF_SOURCEUNIT"),
	ret = "string",
	doc = L["DT_DOC_AuraSource"] .. "\r\n \r\n" .. L["DT_INSERTGUID_GENERIC_DESC"],
	example = ('[AuraSource] => "target"; [AuraSource:Name] => "Kobold"; [AuraSource(icon="JBP:icon:1I7MnrXDCz8T")] => %q; [AuraSource(icon="JBP:icon:1I7MnrXDCz8T"):Name] => %q'):format(UnitName("player"), JBP.NAMES and JBP.NAMES:TryToAcquireName("player", true) or "???"),
	category = L["ICON"],
})
...
function Type:Setup(icon)
	...
	icon.Units, icon.UnitSet = JBP:GetUnits(icon, icon.Unit, icon:GetSettings().UnitConditions)
	...
	-- GLOBALS: JustBecomePro_ChooseName
	if icon:IsBeingEdited() == "MAIN" and JustBecomePro_ChooseName then
		JBP.HELP:Hide("ICONTYPE_BUFF_NOSOURCERPPM")
		if icon.OnlyMine then
			for _, badSpell in pairs(aurasWithNoSourceReported) do
				if type(badSpell) == "string" and icon.Spells.StringHash[badSpell:lower()] then
					JBP.HELP:Show{
						...
						codeOrder = 2,
						icon = icon,
						relativeTo = JustBecomePro_ChooseName,
						x = 0,
						y = 0,
	...
	if icon.UnitSet.allUnitsChangeOnEvent then
		...
		JBP:RegisterCallback("JBP_UNITSET_UPDATED", Buff_OnEvent, icon)
	end
