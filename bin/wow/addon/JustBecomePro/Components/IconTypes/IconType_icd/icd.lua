...
local Type = JBP.Classes.IconType:New("icd")
...
Type:RegisterConfigPanel_ConstructorFunc(120, "JustBecomePro_ICDType", function(self)
	self:SetTitle(JBP.L["ICONMENU_ICDTYPE"])
	self:BuildSimpleCheckSettingFrame({
		numPerRow = 1,
		function(check)
			check:SetTexts(JBP.L["ICONMENU_ICDBDE"], JBP.L["ICONMENU_ICDAURA_DESC"])
			check:SetSetting("ICDType", "aura")
		end,
		function(check)
			check:SetTexts(JBP.L["ICONMENU_SPELLCAST_COMPLETE"], JBP.L["ICONMENU_SPELLCAST_COMPLETE_DESC"])
			check:SetSetting("ICDType", "spellcast")
		end,
		function(check)
			check:SetTexts(JBP.L["ICONMENU_SPELLCAST_START"], JBP.L["ICONMENU_SPELLCAST_START_DESC"])
			check:SetSetting("ICDType", "caststart")
		end,
	})
end)

Type:RegisterConfigPanel_ConstructorFunc(150, "JustBecomePro_ICDSettings", function(self)
...
local function ICD_OnEvent(icon, event, ...)
	...
	if valid then
		...
		if Key and not (icon.DontRefresh and (JBP.time - icon.ICDStartTime) < icon.Spells.Durations[Key]) then
