...
local Type = JBP.Classes.IconType:New("guardian")
...
Type:RegisterConfigPanel_ConstructorFunc(120, "JustBecomePro_GuardianDuration", function(self)
	self:SetTitle(JBP.L["ICONMENU_GUARDIAN_DUR"])
...
Type:RegisterConfigPanel_ConstructorFunc(170, "JustBecomePro_GuardianSortSettings", function(self)
	self:SetTitle(JBP.L["SORTBY"])

	self:BuildSimpleCheckSettingFrame({
		numPerRow = 3,
		function(check)
			check:SetTexts(JBP.L["SORTBYNONE"], JBP.L["SORTBYNONE_DESC"])
...
function Type:RefreshNames()
	for npcID, data in pairs(GuardianInfo) do
		if not data.nameKnown then
			local Parser, LT1 = JBP:GetParser()
...
local ManualIcons = {}
local ManualIconsManager = JBP.Classes.UpdateTableManager:New()
...
local Guardian = JBP:NewClass(){
	OnNewInstance = function(self, GUID, name)
		self.summonedAt = JBP.time
		self.GUID = GUID
		self.name = name
	...
	GetTimeRemaining = function(self, durationMode)
		...
		local guardianRemaining = duration - (JBP.time - start)
		...
		if guardianRemaining > 0 then
			...
			local empowerRemaining = empowerDuration - (JBP.time - empowerStart)
...
local function OnUpdate(icon, time)
	...
	if icon.Sort == false then
		...
		for GUID, Guardian in JBP:OrderedPairs(Guardians, icon.GuardianCompareFunc, true) do
			if icon.Name == "" or NameHash[Guardian.nameLower] or NameHash[Guardian.npcID] then
				if not YieldMatchedGuardian(icon, count, Guardian) then
					return
				end
...
function Type:Setup(icon)
	-- Get "Spells"
	icon.NPCs = JBP:GetSpells(icon.Name, false)
	...
	JBP:RegisterCallback("JBP_ICON_DISABLE", Type)
	JBP:RegisterCallback("JBP_ONUPDATE_TIMECONSTRAINED_PRE", Type)
...
function Type:JBP_ONUPDATE_TIMECONSTRAINED_PRE(event, time)
...
JBP:RegisterCallback("JBP_GLOBAL_UPDATE", function()
	...
	-- JBP_GLOBAL_UPDATE is good enough.
