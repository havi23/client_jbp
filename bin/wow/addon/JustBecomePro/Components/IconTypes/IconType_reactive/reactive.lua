...
local Type = JBP.Classes.IconType:New("reactive")
...
Type:RegisterConfigPanel_ConstructorFunc(150, "JustBecomePro_ReactiveSettings", function(self)
	...
	self:BuildSimpleCheckSettingFrame({
		...
		function(check)
			...
			check:CScriptAdd("ReloadRequested", function()
				check:SetEnabled(JBP.CI.ics.UseActvtnOverlay)
			end)
		end,
		function(check)
		...
		pclass == "DEATHKNIGHT" and function(check)
			...
			check:CScriptAdd("ReloadRequested", function()
				check:SetEnabled(JBP.CI.ics.CooldownCheck)
				if JBP.CI.ics.CooldownCheck then
