...
local Type = JBP.Classes.IconType:New("value")
...
local STATE_UNITFOUND = JBP.CONST.STATE.DEFAULT_SHOW
local STATE_NOUNIT = JBP.CONST.STATE.DEFAULT_HIDE
...
Type:RegisterConfigPanel_ConstructorFunc(100, "JustBecomePro_ValueSettings", function(self)
	...
	self.PowerType = JBP.C.Config_DropDownMenu:New("Frame", "$parent", self, "JBP_DropDownMenuTemplate")
	...
	self:CScriptAdd("ReloadRequested", function()
		for k, v in pairs(types) do
			if v.id == JBP.CI.ics.PowerType then
...
Type:RegisterConfigPanel_ConstructorFunc(105, "JustBecomePro_ValueCheckSettings", function(self)
...
function Type:Setup(icon)
	...
	--	JBP:RegisterCallback("JBP_UNITSET_UPDATED", Value_OnEvent, icon)
	--end
...
JBP:RegisterCallback("JBP_CONFIG_ICON_TYPE_CHANGED", function(event, icon, type, oldType)
	local icspv = icon:GetSettingsPerView()

	if type == Type.type then
		icon:GetSettings().CustomTex = "NONE"
		local layout = JBP.TEXT:GetTextLayoutForIcon(icon)
