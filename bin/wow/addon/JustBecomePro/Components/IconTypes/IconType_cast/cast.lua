...
local Type = JBP.Classes.IconType:New("cast")
...
Type:RegisterConfigPanel_ConstructorFunc(150, "JustBecomePro_CastSettings", function(self)
...
function Type:Setup(icon)
	...
	if icon.UnitSet.allUnitsChangeOnEvent then
		...
		JBP:RegisterCallback("JBP_UNITSET_UPDATED", Cast_OnEvent, icon)
...
function Type:GuessIconTexture(ics)
	if ics.Name and ics.Name ~= "" then
		...
		if name then
			return JBP.GetSpellTexture(name)
		end
	end
