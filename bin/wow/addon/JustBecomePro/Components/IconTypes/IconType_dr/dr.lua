...
local Type = JBP.Classes.IconType:New("dr")
...
local STATE_UNDIMINISHED = JBP.CONST.STATE.DEFAULT_SHOW
local STATE_DIMINISHED = JBP.CONST.STATE.DEFAULT_HIDE
...
JBP:RegisterUpgrade(72506, {
	global = function(self, ics)
		...
		JBP.db.global.DRDuration = nil
	end,
})

JBP:RegisterUpgrade(71035, {
...
Type:RegisterConfigPanel_ConstructorFunc(150, "JustBecomePro_DRSettings", function(self)
	...
	local slider = JBP.C.Config_Slider:New("Slider", nil, self, "JustBecomePro_SliderTemplate")
	...
	slider:SetTextFormatter(JBP.C.Formatter.S_SECONDS, JBP.C.Formatter.F_0)

	self:AdjustHeight(5)
end)


JBP:RegisterCallback("JBP_EQUIVS_PROCESSING", function()
	-- Create our own DR equivalencies in JBP using the data from DRData-1.0
	...
	if DRData then
		...
		for spellID, category in pairs(DRData.spells) do
			...
			elseif JBP.debug and not ignored[category] then
				JBP:Error("The DR category %q is undefined!", category)
			end
		end
	end
end)
			...
			if NameHash[spellID] or NameHash[strlowerCache[spellName]] then
				...
				if PvEDRs[spellID] or bitband(destFlags, CL_CONTROL_PLAYER) == CL_CONTROL_PLAYER then
					...
					if cevent == "SPELL_AURA_APPLIED" then
						...
						if dr and dr.start + dr.duration <= JBP.time then
							dr.start = 0
							dr.duration = 0
							dr.amt = 100
						end
...
do	-- CheckCategories
	...
	local func = function(NameArray)
		...
		for i, IDorName in ipairs(NameArray) do
			for category, tbl in pairs(JBP.BE.dr) do

				local Names = JBP:GetSpells(category)
				...
				if Names.Hash[IDorName] or Names.StringHash[IDorName] then
					...
					categories[category].str = categories[category].str .. ";" .. JBP:RestoreCase(IDorName)
				end
			end
		end
		...
		for category, data in JBP:OrderedPairs(categories, JBP.OrderSort, true) do
			append = append .. format("\r\n\r\n%s:\r\n%s", L[category], JBP:CleanString(data.str))
		end
	...
	CheckCategories = function(icon)
		...
		if icon:IsBeingEdited() == "MAIN" and JustBecomePro_ChooseName then
			if doWarn then
				...
				JBP.HELP:Hide("ICON_DR_MISMATCH")
			end
		end
...
function Type:Setup(icon)
	...
	if icon.UnitSet.allUnitsChangeOnEvent then
		...
		JBP:RegisterCallback("JBP_UNITSET_UPDATED", DR_OnEvent, icon)
	end
