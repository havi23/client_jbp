...
local Type = JBP.Classes.IconType:New("meta")
...
-- IDP that works with JBP's state arbitrator to inherit the state of the icon that it is replicating.
local Processor = JBP.Classes.IconDataProcessor:New("STATE_METACHILD", "state_metaChild")
...
local function CheckCompiledIcons(icon)
	...
	for _, iconGUID in pairs(icon.CompiledIcons) do
		local ic = JBP.GUIDToOwner[iconGUID]
...
JBP:RegisterCallback("JBP_GLOBAL_UPDATE_POST", function()
...
if JBP.EVENTS:GetEventHandler("Animations") then
	JBP:RegisterCallback("JBP_ICON_META_INHERITED_ICON_CHANGED", function(event, icon, icToUse)
	...
	JBP:RegisterCallback("JBP_ICON_ANIMATION_START", function(event, icon, table)
		-- Inherit animations
...
JBP:RegisterCallback("JBP_ICON_UPDATED", function(event, icon)
...
JBP:RegisterCallback("JBP_ICON_SETUP_POST", SETUP_POST)
JBP:RegisterCallback("JBP_GROUP_SETUP_POST", SETUP_POST)
...
function Type:HandleYieldedInfo(icon, iconToSet, icToUse)
	if icToUse then
		...
		if dataSource ~= iconToSet.__currentIcon then
			...
			JBP:Fire("JBP_ICON_META_INHERITED_ICON_CHANGED", iconToSet, dataSource)
...
function InsertIcon(icon, GUID, ics)
	...
	if not ics then
		ics = JBP:GetSettingsFromGUID(GUID)
	end
	...
	for _, GUID in ipairs(icons) do
		if not alreadyinserted[GUID] then
			...
			elseif type == "group" then
				...
				local gs, group, domain, groupID = JBP:GetSettingsFromGUID(GUID)

				if gs and not group then
					group = JBP[domain][groupID]
				end
	...
	if icon.Enabled then
		for i, icGUID in pairs(icon.Icons) do
			...
			if icGUID and icGUID ~= "" then
				JBP:QueueValidityCheck(icon, icGUID, L["VALIDITY_META_DESC"], i)
			end
		end
	end
