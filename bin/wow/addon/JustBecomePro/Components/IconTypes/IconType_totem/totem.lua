...
local Type = JBP.Classes.IconType:New("totem")
...
if numSlots > 1 then
	Type:RegisterConfigPanel_ConstructorFunc(120, "JustBecomePro_TotemSlots", function(self)
			...
			if totemData[i] then
				tinsert(data, function(check)
					...
					check:CScriptAdd("ReloadRequested", function()
						check:SetTexts(JBP.get(totemData[i].name), nil)
					end)
				end)
			end
		end
...
function Type:Setup(icon)
	...
	icon.Spells = JBP:GetSpells(name, true)
	...
	if not icon.FirstTexture and onlySlot then
		icon.FirstTexture = onlySlot.texture and JBP.get(onlySlot.texture)
	end
