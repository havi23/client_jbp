...
-- GLOBALS: JustBecomePro_CLEUOptions


local Type = JBP.Types.cleu
Type.Config = {}
...
function Config:LoadConfig()
	if JustBecomePro_CLEUOptions then
...
function Config:CheckMasks()
	JBP.HELP:Hide("CLEU_WHOLECATEGORYEXCLUDED")
	...
	for _, key in JBP:Vararg("SourceFlags", "DestFlags") do
		for maskName, mask in pairs(Config.BetterMasks) do
			if bit.band(JBP.CI.ics[key], mask) == 0 then
				local category = L["CLEU_" .. maskName]
				JBP.HELP:Show{
					code = "CLEU_WHOLECATEGORYEXCLUDED",
					codeOrder = 2,
					icon = JBP.CI.icon,
					relativeTo = JustBecomePro_CLEUOptions[key],
					x = 23,
					y = 3,
...
function Config:Menus_SetTexts()
	local n = 0
	if JBP.CI.ics.CLEUEvents[""] then
		n = L["CLEU_EVENTS_ALL"]
	else
		for k, v in pairs(JBP.CI.ics.CLEUEvents) do
			if v then
				n = n + 1
			end
		end
	...
	if n == 0 then
		n = " |cFFFF5959(0)|r |TInterface\\AddOns\\JustBecomePro\\Textures\\Alert:0:2|t"
	else
		n = " (|cff59ff59" .. n .. "|r)"
	end
	JustBecomePro_CLEUOptions.CLEUEvents:SetText(L["CLEU_EVENTS"] .. n)

	local n = Config:CountDisabledBits(JBP.CI.ics.SourceFlags)
	if n ~= 0 then
		n = "|cFFFF5959(" .. n .. ")|r "
	else
		n = "(" .. n .. ") "
	end
	JustBecomePro_CLEUOptions.SourceFlags:SetText(n .. L["CLEU_FLAGS_SOURCE"])

	local n = Config:CountDisabledBits(JBP.CI.ics.DestFlags)
	if n ~= 0 then
		n = "|cFFFF5959(" .. n .. ")|r "
	else
		n = "(" .. n .. ") "
	end
	JustBecomePro_CLEUOptions.DestFlags:SetText(n .. L["CLEU_FLAGS_DEST"])
end


local function EventMenu_OnClick(button, dropdown)
	if button.value == "" and not JBP.CI.ics.CLEUEvents[""] then -- if we are checking "Any Event" then uncheck all others
		wipe(JBP.CI.ics.CLEUEvents)
		JBP.DD:CloseDropDownMenus()
	elseif button.value ~= "" and JBP.CI.ics.CLEUEvents[""] then -- if we are checking a specific event then uncheck "Any Event"
		JBP.CI.ics.CLEUEvents[""] = false
		JBP.DD:CloseDropDownMenus()
	end

	JBP.CI.ics.CLEUEvents[button.value] = not JBP.CI.ics.CLEUEvents[button.value]
...
function Config.EventMenu(dropdown)
	...
	for _, event in ipairs(Config.Events) do
		...
		elseif (JBP.DD.MENU_LEVEL == 1 and not currentCategory) or (JBP.DD.MENU_LEVEL == 2 and JBP.DD.MENU_VALUE == currentCategory) then
			if event == "SPACE" then
				...
				info.value = event
				info.checked = JBP.CI.ics.CLEUEvents[event]
...
local function FlagsMenu_OnClick(button, dropdown)
	JBP.CI.ics[dropdown.flagSet] = bit.bxor(JBP.CI.ics[dropdown.flagSet], _G[button.value])
...
function Config.FlagsMenu(dropdown)
	...
	for _, flag in ipairs(Config.Flags) do
		if flag == "SPACE" then
			...
			info.value = flag
			info.checked = bit.band(JBP.CI.ics[dropdown.flagSet], _G[flag]) ~= _G[flag]
			info.keepShownOnClick = true
			info.isNotRadio = true
...
function Module:Table_Get()
	return SpellCache:GetCache(), JBP.BE.buffs, JBP.BE.debuffs, JBP.BE.casts
end
