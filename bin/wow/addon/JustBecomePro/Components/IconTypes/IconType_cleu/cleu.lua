...
local Type = JBP.Classes.IconType:New("cleu")
...
local STATE_RUNNING = JBP.CONST.STATE.DEFAULT_SHOW
local STATE_EXPIRED = JBP.CONST.STATE.DEFAULT_HIDE
...
Type:RegisterConfigPanel_XMLTemplate(100, "JustBecomePro_ChooseName", {
	title = L["ICONMENU_CHOOSENAME3"] .. " " .. L["ICONMENU_CHOOSENAME_ORBLANK"],
	SUGType = "cleu",
})

Type:RegisterConfigPanel_XMLTemplate(165, "JustBecomePro_IconStates", {
	[STATE_RUNNING] = { text = "|cFF00FF00" .. L["ICONMENU_COUNTING"],    },
	[STATE_EXPIRED] = { text = "|cFFFF0000" .. L["ICONMENU_NOTCOUNTING"], },
})

Type:RegisterConfigPanel_XMLTemplate(150, "JustBecomePro_CLEUOptions")
...
JBP:RegisterCallback("JBP_GLOBAL_UPDATE", function()
	...
	-- JBP_GLOBAL_UPDATE is good enough. Its the first event that came to mind.
...
local function CLEU_OnEvent(icon, _, t, event, h, sourceGUID, sourceName, sourceFlags, sourceRaidFlags, destGUID, destName, destFlags, destRaidFlags, arg1, arg2, arg3, arg4, arg5, ...)
	...
	if icon.AllowAnyEvents or icon.CLEUEvents[event] then
		...
		if icon.CLEUNoRefresh then
			...
			if JBP.time - attributes.start < attributes.duration then
				return
			end
		end
		...
		if icon:IsGroupController() then
			tinsert(icon.capturedCLEUEvents, 1, {
				JBP.time, duration or icon.CLEUDur,
...
function Type:Setup(icon)
	icon.Spells = JBP:GetSpells(icon.Name, false)
	...
	if icon.SourceUnit ~= "" then
		local unitSet
		icon.SourceUnits, unitSet = JBP:GetUnits(icon, icon.SourceUnit, icon.SourceConditions)
		if unitSet.mightHaveWackyUnitRefs then
			icon.SourceUnits = JBP:GetUnits(icon, icon.SourceUnit)
		end
	end
	...
	if icon.DestUnit ~= "" then
		local unitSet
		icon.DestUnits, unitSet = JBP:GetUnits(icon, icon.DestUnit, icon.DestConditions)
		if unitSet.mightHaveWackyUnitRefs then
			icon.DestUnits = JBP:GetUnits(icon, icon.DestUnit)
		end
	end
	...
	if icon.AllowAnyEvents and not icon.SourceUnits and not icon.DestUnits and icon.Name == "" and not icon.SourceFlags and not icon.DestFlags then
		if JBP.Locked and icon.Enabled then
			JBP:Warn(L["CLEU_NOFILTERS"]:format(icon:GetIconName(true)))
		end
		return
	end
...
local Processor = JBP.Classes.IconDataProcessor:New("CLEU_SOURCEUNIT", "sourceUnit, sourceGUID")
function Processor:CompileFunctionSegment(t)
	...
	if attributes.sourceUnit ~= sourceUnit or attributes.sourceGUID ~= sourceGUID then
		...
		JBP:Fire(CLEU_SOURCEUNIT.changedEvent, icon, sourceUnit, sourceGUID)
		doFireIconUpdated = true
	end
	--]]
end
Processor:RegisterDogTag("JBP", "Source", {
	code = function(icon)
	...
	events = JBP:CreateDogTagEventString("CLEU_SOURCEUNIT"),
	ret = "string",
	doc = L["DT_DOC_Source"] .. "\r\n \r\n" .. L["DT_INSERTGUID_GENERIC_DESC"],
	example = ('[Source] => "target"; [Source:Name] => "Kobold"; [Source(icon="JBP:icon:1I7MnrXDCz8T")] => %q; [Source(icon="JBP:icon:1I7MnrXDCz8T"):Name] => %q'):format(UnitName("player"), JBP.NAMES and JBP.NAMES:TryToAcquireName("player", true) or "???"),
	category = L["ICON"],
})

local Processor = JBP.Classes.IconDataProcessor:New("CLEU_DESTUNIT", "destUnit, destGUID")
function Processor:CompileFunctionSegment(t)
	...
	if attributes.destUnit ~= destUnit or attributes.destGUID ~= destGUID then
		...
		JBP:Fire(CLEU_DESTUNIT.changedEvent, icon, destUnit, destGUID)
		doFireIconUpdated = true
	end
	--]]
end
Processor:RegisterDogTag("JBP", "Destination", {
	code = function(icon)
		icon = JBP.GUIDToOwner[icon]
	...
	events = JBP:CreateDogTagEventString("CLEU_DESTUNIT"),
	ret = "string",
	doc = L["DT_DOC_Destination"] .. "\r\n \r\n" .. L["DT_INSERTGUID_GENERIC_DESC"],
	example = ('[Destination] => "target"; [Destination:Name] => "Kobold"; [Destination(icon="JBP:icon:1I7MnrXDCz8T")] => %q; [Destination(icon="JBP:icon:1I7MnrXDCz8T"):Name] => %q'):format(UnitName("player"), JBP.NAMES and JBP.NAMES:TryToAcquireName("player", true) or "???"),
	category = L["ICON"],
})
...
local Processor = JBP.Classes.IconDataProcessor:New("CLEU_EXTRASPELL", "extraSpell")
-- Processor:CompileFunctionSegment(t) is default.
Processor:RegisterDogTag("JBP", "Extra", {
	...
	events = JBP:CreateDogTagEventString("CLEU_EXTRASPELL"),
	ret = "string",
	doc = L["DT_DOC_Extra"] .. "\r\n \r\n" .. L["DT_INSERTGUID_GENERIC_DESC"],
	example = ('[Extra] => %q; [Extra(link=true)] => %q; [Extra(icon="JBP:icon:1I7MnrXDCz8T")] => %q; [Extra(icon="JBP:icon:1I7MnrXDCz8T", link=true)] => %q'):format(GetSpellInfo(5782), GetSpellLink(5782), GetSpellInfo(5308), GetSpellLink(5308)),
	category = L["ICON"],
})
...
for _, key in JBP:Vararg("Source", "Dest") do
	local CNDT = JBP.CNDT
	...
	local ConditionSet = {
		...
		GetSettings = function(self)
			return JBP.CI.ics and JBP.CI.ics[key .. "Conditions"]
		end,
		...
		ShouldShowTab = function(self)
			return JustBecomePro_CLEUOptions and JustBecomePro_CLEUOptions:IsShown()
		end,
		...
		JBP_CNDT_GROUP_DRAWGROUP = function(self, event, conditionGroup, conditionData, conditionSettings)
			if CNDT.CurrentConditionSet == self then
				JBP.SUG:EnableEditBox(conditionGroup.Unit, "unitconditionunits", true)
				JBP:TT(conditionGroup.Unit, "CONDITIONPANEL_UNIT", "ICONMENU_UNIT_DESC_UNITCONDITIONUNIT")
			end
		end,
	}
	...
	JBP:RegisterCallback("JBP_OPTIONS_LOADED", function()
		-- This can't happen until after JBP_OPTIONS_LOADED because it has to be registered
		-- after the default callbacks are registered
		JBP:RegisterCallback("JBP_CNDT_GROUP_DRAWGROUP", ConditionSet)
	end)
end