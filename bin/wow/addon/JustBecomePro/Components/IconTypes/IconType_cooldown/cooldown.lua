...
local SpellHasNoMana = JBP.SpellHasNoMana
local GetSpellTexture = JBP.GetSpellTexture
local GetRuneCooldownDuration = JBP.GetRuneCooldownDuration
...
local Type = JBP.Classes.IconType:New("cooldown")
...
local STATE_UNUSABLE_NORANGE = JBP.CONST.STATE.DEFAULT_NORANGE
local STATE_UNUSABLE_NOMANA  = JBP.CONST.STATE.DEFAULT_NOMANA
...
Type:RegisterConfigPanel_ConstructorFunc(150, "JustBecomePro_CooldownSettings", function(self)
...
function Type:Setup(icon)
	icon.Spells = JBP:GetSpells(icon.Name, true)
	
	if pclass ~= "DEATHKNIGHT" then
		icon.IgnoreRunes =  nil
	end
	
	if JBP.HELP then JBP.HELP:Hide("ICONTYPE_COOLDOWN_VOIDBOLT") end
