...
local Type = JBP.Classes.IconType:New("dotwatch")
...
-- Type:RegisterConfigPanel_ConstructorFunc(10, "JustBecomePro_DotwatchSettings", function(self)
...
local function CreateAllUnits()
	AllUnits = AllUnits or JBP:GetUnits(nil, [[
		player;
		mouseover;
	...
	for i = 1, #AllUnits do
		...
		if GUID then
			...
			for k, v in next, auras do
				...
				if aura and aura.lastSeen ~= JBP.time then
					auras[spellID] = nil
	...
	for GUID, auras in pairs(Auras) do
		if not next(auras) then
			...
			for _, aura in pairs(auras) do
				...
				if type(aura) == "table" and aura:Remaining() > 0 and (aura.lastSeen > JBP.time - 30) then
					isGood = true
					break
				end
			end
...
Aura = JBP:NewClass("Aura"){
	...
	Remaining = function(self)
		...
		return self.duration - (JBP.time - self.start)
	end,
...
function Type:JBP_ONUPDATE_TIMECONSTRAINED_PRE()
	VerifyAll()
end
...
JBP:RegisterCallback("JBP_GLOBAL_UPDATE", function()
	...
	JBP:UnregisterCallback("JBP_ONUPDATE_TIMECONSTRAINED_PRE", Type)
