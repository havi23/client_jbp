...
local Type = JBP.Classes.IconType:New("unitcondition")
...
local STATE_SUCCEED = JBP.CONST.STATE.DEFAULT_SHOW
local STATE_FAIL = JBP.CONST.STATE.DEFAULT_HIDE
...
Type:RegisterConfigPanel_XMLTemplate(150, "JustBecomePro_ConditionIconSettings")
...
function Type:Setup(icon)
	...
	local _, UnitSet = JBP:GetUnits(icon, icon.Unit, icon:GetSettings().UnitConditions)
	...
	if UnitSet.allUnitsChangeOnEvent then
		...
		JBP:RegisterCallback("JBP_UNITSET_UPDATED", ConditionIcon_OnEvent, icon)
	end
...
function Type:DragReceived(icon, t, data, subType)
	...
	ics.CustomTex = JBP:CleanString(input)
