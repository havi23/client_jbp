...
if not JBP.COMMON.SwingTimerMonitor then
	return
end
...
local Type = JBP.Classes.IconType:New("swingtimer")
Type.name = L["ICONMENU_SWINGTIMER"]
Type.desc = L["ICONMENU_SWINGTIMER_DESC"]
Type.menuIcon = "Interface\\Icons\\INV_Gauntlets_04"
Type.hasNoGCD = true

local STATE_NOTREADY = JBP.CONST.STATE.DEFAULT_SHOW
local STATE_READY = JBP.CONST.STATE.DEFAULT_HIDE
...
if pclass == "HUNTER" then
	Type:RegisterConfigPanel_XMLTemplate(130, "JustBecomePro_AutoshootSwingTimerTip")
end
...
local function SwingTimer_OnEvent(icon, event, unit, _, _, _, spellID)
	...
	elseif event == "JBP_COMMON_SWINGTIMER_CHANGED" then
...
local function SwingTimer_OnUpdate(icon, time)
	...
	-- Get the SwingTimer object for the slow from JBP's common swing timer module
...
function Type:Setup(icon)
	...
	JBP:RegisterCallback("JBP_COMMON_SWINGTIMER_CHANGED", SwingTimer_OnEvent, icon)
