...
local Type = rawget(JBP.Types, "losecontrol")
...
function Config.DropdownMenu_OnClick_All(dropDownButton, LoseControlTypes)
	if not LoseControlTypes[dropDownButton.value] then
		...
		LoseControlTypes = JBP:CopyTableInPlaceUsingDestinationMeta(JBP.DEFAULT_ICON_SETTINGS.LoseControlTypes, LoseControlTypes)
	end
...
function Config.DropdownMenu_SelectTypes()
	local LoseControlTypes = JBP.CI.ics.LoseControlTypes
	...
	if JBP.DD.MENU_LEVEL == 1 then
		...
		for text, data in JBP:OrderedPairs(Config.Types) do
			local info = JBP.DD:CreateInfo()

			info.text = get(text)
			
			info.tooltipTitle = get(text)
			info.tooltipText = (JBP.debug and (data.value and data.value .. "\r\n") or "") .. (get(data.desc) or "")
	...
	elseif JBP.DD.MENU_LEVEL == 2 then
		if JBP.DD.MENU_VALUE == "SCHOOL_INTERRUPT" then
			for bitFlag, name in JBP:OrderedPairs(Config.schools) do
				local info = JBP.DD:CreateInfo()
			
				info.text = LOSS_OF_CONTROL_DISPLAY_INTERRUPT_SCHOOL:format(name .. "|r")
				
				info.value = JBP.DD.MENU_VALUE
				info.keepShownOnClick = true
				info.isNotRadio = true
				
				info.arg1 = LoseControlTypes
				info.arg2 = bitFlag
				info.checked = LoseControlTypes[""] or bit.band(LoseControlTypes[JBP.DD.MENU_VALUE], bitFlag) == bitFlag
				info.disabled = LoseControlTypes[""]
...
function Config:DropdownMenu_SetText()
	...
	if LoseControlTypes[""] then
		...
		for k, v in pairs(LoseControlTypes) do
			if k == "SCHOOL_INTERRUPT" then
				for bitFlag in JBP:OrderedPairs(Config.schools) do
	...
	JustBecomePro_LoseControlTypes.LocTypes:SetText(L["LOSECONTROL_DROPDOWNLABEL"] .. n)
end
