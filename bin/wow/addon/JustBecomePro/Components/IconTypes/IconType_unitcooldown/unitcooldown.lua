...
local Type = JBP.Classes.IconType:New("unitcooldown")
...
local STATE_USABLE = JBP.CONST.STATE.DEFAULT_SHOW
local STATE_USABLE_ALL = 10
local STATE_UNUSABLE = JBP.CONST.STATE.DEFAULT_HIDE
...
Type:RegisterConfigPanel_ConstructorFunc(150, "JustBecomePro_UnitCooldownSettings", function(self)
...
Type:RegisterConfigPanel_ConstructorFunc(170, "JustBecomePro_UCDSortSettings", function(self)
...
function Type:COMBAT_LOG_EVENT_UNFILTERED(e)
	...
	then
		...
		if cleuEvent == "SPELL_CAST_SUCCESS" then
			...
			cooldownsForGUID[spellID] = JBP.time
		else
			local time = JBP.time
...
local function UnitCooldown_OnEvent(icon, event, arg1)
	if event == "JBP_UNITSET_UPDATED" and arg1 == icon.UnitSet then
...
function Type:Setup(icon)
	...
	if icon.OnlySeen == "class" then
		classSpellNameCache = JBP:GetModule("ClassSpellCache"):GetNameCache()
	end
	...
	if icon.UnitSet.allUnitsChangeOnEvent then
		...
		JBP:RegisterCallback("JBP_UNITSET_UPDATED", UnitCooldown_OnEvent, icon)
	end
...
JBP:RegisterCallback("JBP_GLOBAL_UPDATE", function(event, icon)
