...
JBP.CONST.STATE.DEFAULT_DURATIONFAILED = 100
local STATE = JBP.CONST.STATE.DEFAULT_DURATIONFAILED

local Hook = JBP.Classes.IconDataProcessorHook:New("STATE_DURATIONREQ", "DURATION")

Hook:RegisterConfigPanel_XMLTemplate(222, "JustBecomePro_DurationRequirements")
...
local Processor = JBP.Classes.IconDataProcessor:New("STATE_DURATIONFAILED", "state_durationFailed")
...
Processor:RegisterAsStateArbitrator(20, Hook, false, function(icon)
	...
	if ics.DurationMinEnabled then
		text = L["DURATION"] .. " < " .. JBP:FormatSeconds(ics.DurationMin)
	end
	if ics.DurationMaxEnabled then
		if ics.DurationMinEnabled then
			text = text .. " " .. L["CONDITIONPANEL_OR"]:lower() .. " "
		end
		text = text .. L["DURATION"] .. " > " .. JBP:FormatSeconds(ics.DurationMax)
	end
