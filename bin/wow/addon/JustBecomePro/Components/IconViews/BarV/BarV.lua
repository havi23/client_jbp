...
local View = JBP.Classes.IconView:New("barv")
		...
		TextLayouts = {
			bar2 = {
				...
				{    -- [1] Duration        
					...
					DefaultText = "[Duration(gcd=true):JBPFormatDuration]",    
					Anchors = {
						{
							point = "TOP",
							relativePoint = "TOP",
...
JBP:RegisterUpgrade(80003, {
	group = function(self, gs)
		gs.SettingsPerView.barv.BorderColor = JBP:RGBATableToStringWithFallback(gs.SettingsPerView.barv.BorderColor, "ff000000")
	end,
})


if not JBP.Views.bar then
	JBP:Error("Some configuration for " .. View.name .. " depends on the normal Bar display method, which was not found.")
end
View:RegisterConfigPanel_XMLTemplate(50, "JustBecomePro_GM_Bar")
