...
local isNumber = JBP.isNumber
local huge = math.huge
	
local CNDT = JBP:NewModule("Conditions", "AceEvent-3.0", "AceSerializer-3.0")
JBP.CNDT = CNDT
...
setmetatable(CNDT.Condition_Defaults["**"], {
	__newindex = function(self, k, v)
		...
		JBP:Fire("JBP_CNDT_DEFAULTS_NEWVAL", k, v)
...
--- Registers default condition settings. Must be called before JBP's database is initialized.
...
function CNDT:RegisterConditionDefaults(defaults)
	JBP:ValidateType("2 (defaults)", "CNDT:RegisterConditionDefaults(defaults)", defaults, "table")
	...
	JBP:MergeDefaultsTables(defaults, CNDT.Condition_Defaults["**"])
end


JBP:RegisterUpgrade(61011, {
	condition = function(self, condition)
		...
		if condition.PrtsBefore ~= 0 or condition.PrtsAfter ~= 0 then
			JBP.db.global.HelpSettings.CNDT_PARENTHESES_FIRSTSEE = true
		end
		if condition.AndOr == "OR" then
			JBP.db.global.HelpSettings.CNDT_ANDOR_FIRSTSEE = true
		end
	end,
})
JBP:RegisterUpgrade(60026, {
	stances = {
...
JBP:RegisterUpgrade(46417, {
	-- cant use the conditions key here because it depends on Conditions.n, which is 0 until this is ran
	-- also, dont use JBP:InNLengthTable because it will use conditions.n, which is 0 until the upgrade is complete
...
JBP:RegisterCallback("JBP_DB_PRE_DEFAULT_UPGRADES", function() -- 46413
	...
	if JustBecomeProDB.profiles and JustBecomeProDB.Version < 46413 then
...
JBP:RegisterUpgrade(44202, {
	icon = function(self, ics)
		ics.Conditions["**"] = nil
	end,
})
JBP:RegisterUpgrade(42105, {
...
CNDT.Env = {	
	strlower = strlower,
	strlowerCache = JBP.strlowerCache,
	strfind = strfind,
	floor = floor,
	select = select,
	min = min,
	...
	JBP = JBP,
	GCDSpell = JBP.GCDSpell,
	GUIDToOwner = JBP.GUIDToOwner,
...
JBP:RegisterCallback("JBP_ONUPDATE_PRE", function(event, time_arg)
	Env.time = time_arg
end)

JBP:RegisterCallback("JBP_GLOBAL_UPDATE", function()
	Env.Locked = JBP.Locked
end)
...
function CNDT:GetItemRefForConditionChecker(name)
	local item = JBP:GetItems(name)[1]

	if not item then
		item = JBP:GetNullRefItem()
	end
...
function CNDT:GetUnit(setting)
	return JBP.UNITS:GetOriginalUnitTable(setting)[1] or ""
end


function CNDT:GetTableSubstitution(tbl)
	JBP:ValidateType("CNDT:GetTableSubstitution(tbl)", "tbl", tbl, "table")
	...
	if JBP.approachTable(tbl, getmetatable, "__tostring") then
...
{	src = "BITFLAGSMAPANDCHECK(%b())",
	...
	rep = function(conditionData, conditionSettings, name, name2)
		JBP:ValidateType("c.BitFlags", conditionData.identifier, conditionSettings.BitFlags, "table;number")
...
{	src = "c.Level",
	rep = function(conditionData, conditionSettings, name, name2)
		JBP:ValidateType("c.Level", conditionData.identifier, conditionSettings.Level, "number")
	...
	src = "c.Checked",
	rep = function(conditionData, conditionSettings, name, name2)
		JBP:ValidateType("c.Checked", conditionData.identifier, conditionSettings.Checked, "boolean")
	...
	src = "c.Checked2",
	rep = function(conditionData, conditionSettings, name, name2)
		JBP:ValidateType("c.Checked2", conditionData.identifier, conditionSettings.Checked2, "boolean")
	...
	src = "c.Operator",
	rep = function(conditionData, conditionSettings, name, name2)
		JBP:ValidateType("c.Operator", conditionData.identifier, conditionSettings.Operator, "string")
...

{
	src = "c.NameFirst2",
	rep = function(conditionData, conditionSettings, name, name2)
		return strWrap(JBP:GetSpells(name2).First)

	end,
},{
	src = "c.NameString2",
	rep = function(conditionData, conditionSettings, name, name2)
		return strWrap(JBP:GetSpells(name2).FirstString)
	end,
},{
	src = "c.ItemID2",
...

{
	src = "c.NameFirst",
	rep = function(conditionData, conditionSettings, name, name2)
		return strWrap(JBP:GetSpells(name).First)
	end,
},{
	src = "c.NameString",
	rep = function(conditionData, conditionSettings, name, name2)
		return strWrap(JBP:GetSpells(name).FirstString)
	end,
},{
	src = "c.ItemID",
	rep = "error('Condition sub c.ItemID is obsolete')",
...

{
	...
	rep = function(conditionData, conditionSettings, name, name2)
		...
		local name = JBP:GetSpells(name2).First
		if name == "gcd" then
			name = JBP.GCDSpell
		end
		return strWrap(name)
	...
	rep = function(conditionData, conditionSettings, name, name2)
		local name = JBP:GetSpells(name).First
		if name == "gcd" then
			name = JBP.GCDSpell
		end
		return strWrap(name)
...
function CNDT:GetConditionUnitSubstitution(unit)
	...
	local translatedUnits, unitSet = JBP:GetUnits(nil, unit)
...
function CNDT:DoConditionSubstitutions(conditionData, conditionSettings, funcstr)
	...
	for _, append in JBP:Vararg("2", "") do -- Unit2 MUST be before Unit
	...
	for k, subData in pairs(CNDT.Substitutions) do
		if funcstr:find(subData.src) then
			funcstr = funcstr:gsub(subData.src, JBP.get(subData.rep, conditionData, conditionSettings, name, name2))
		end
	end
...
function CNDT:GetConditionCheckFunctionString(parent, Conditions)
	...
	for n, conditionSettings in JBP:InNLengthTable(Conditions) do
		...
		if conditionData then
			...
			if conditionData:IsDeprecated() then
				JBP:QueueValidityCheck(parent, "<CONDITION>", L["VALIDITY_CONDITION2_DESC"], n)
				thisstr = "true"
			else
...
function CNDT:CheckParentheses(settings)	
	...
	if settings.n < 3 then
		...
		for _, Condition in JBP:InNLengthTable(settings) do
...
function CNDT:GetConditionObjectConstructor()
	for _, instance in pairs(JBP.Classes.ConditionObjectConstructor.instances) do
		if instance.status == "ready" then
			return instance
		end
	end
	
	return JBP.Classes.ConditionObjectConstructor:New()
end
...
function CNDT:GetConditionObject(parent, conditionSettings)
	...
	if conditionString and conditionString ~= "" then
		local instances = JBP.Classes.ConditionObject.instances
		for i, instance in pairs(instances) do
			if instance.conditionString == conditionString then
				return instance
			end
		end
		return JBP.Classes.ConditionObject:New(conditionSettings, conditionString)
	end
end
...
function CNDT:GetCategory(identifier, order, categoryName, spaceBefore, spaceAfter)
	JBP:ValidateType("2 (identifier)", "CNDT:GetCategory()", identifier, "string")
	
	if CNDT.CategoriesByID[identifier] then
		return CNDT.CategoriesByID[identifier]
	end
	
	JBP:ValidateType("3 (order)", "CNDT:GetCategory()", order, "number")
	JBP:ValidateType("4 (categoryName)", "CNDT:GetCategory()", categoryName, "string")
	
	return JBP.Classes.ConditionCategory:New(identifier, order, categoryName, spaceBefore, spaceAfter)
end
...
JBP:NewClass("ConditionSetImplementor"){
...
function CNDT:RegisterConditionSet(identifier, conditionSetData)
	...
	JBP:ValidateType("2 (identifier)", "CNDT:RegisterConditionSet()", identifier, "string")
	JBP:ValidateType("3 (conditionSetData)", "CNDT:RegisterConditionSet()", data, "table")
	
	JBP:ValidateType("parentSettingType", "conditionSetData", data.parentSettingType, "string")
	JBP:ValidateType("parentDefaults", "conditionSetData", data.parentDefaults, "table")
	JBP:ValidateType("modifiedDefaults", "conditionSetData", data.modifiedDefaults, "table;nil")
	
	JBP:ValidateType("ConditionTypeFilter", "conditionSetData", data.ConditionTypeFilter, "function;nil")
	
	JBP:ValidateType("settingKey", "conditionSetData", data.settingKey, "string")
	JBP:ValidateType("GetSettings", "conditionSetData", data.GetSettings, "function")
	
	JBP:ValidateType("iterFunc", "conditionSetData", data.iterFunc, "function")
	JBP:ValidateType("iterArgs", "conditionSetData", data.iterArgs, "table")
	
	JBP:ValidateType("useDynamicTab", "conditionSetData", data.useDynamicTab, "boolean;nil")
	JBP:ValidateType("GetTab", "conditionSetData", data.GetTab, "function;nil")
	JBP:ValidateType("tabText", "conditionSetData", data.tabText, "string")
	
	if data.useDynamicTab then
		JBP:ValidateType("ShouldShowTab", "conditionSetData", data.ShouldShowTab, "function")
	end
	...
	if data.modifiedDefaults then
		...
		JBP:CopyInPlaceWithMetatable(data.modifiedDefaults, defaults["**"])
		JBP:RegisterCallback("JBP_CNDT_DEFAULTS_NEWVAL", function(event, k, v)
			defaults["**"][k] = v
		end)
	end
	...
	if not data.useDynamicTab then
		JBP:RegisterCallback("JBP_CONFIG_TAB_CLICKED", function(event)
			CNDT:SetTabText(identifier)
		end)
	end
end

--- Inherits {{{JBP.Classes.ConditionSetImplementor}}} (Its only method is documented on this page) into the specified class. This should be done when a class implements a ConditionSet.
...
-- @usage JBP.CNDT:RegisterConditionSetImplementingClass("Icon")
function CNDT:RegisterConditionSetImplementingClass(className)
	JBP:ValidateType("2 (className)", "CNDT:RegisterConditionSetImplementingClass()", className, "string")
	
	if not JBP.Classes[className] then
		error(("No class named %q exists to embed ConditionSetImplementor into."):format(className), 2)
	end
	
	if next(JBP.Classes[className].instances) then
		error(("Class %q already has instances created! Can't make it into a condition set implementing class."):format(className), 2)
	end
	
	JBP.Classes[className]:Inherit("ConditionSetImplementor")
end


JBP:RegisterCallback("JBP_UPGRADE_PERFORMED", function(event, settingType, upgradeData, ...)
	...
	for identifier, conditionSetData in pairs(ConditionSets) do
		if conditionSetData.parentSettingType == settingType then
			...
			if type(parentSettings) ~= "table" then
				JBP:Error("ConditionSet %q is defined as having child settings of '%q', " .. 
				"but that settings type does not provide a settings table as the 4th arg "..
				"(right after upgradeData) to JBP:Upgrade(settingType, upgradeData, ...)", identifier, settingType)
				isGood = false
			end
			...
			if type(conditions) ~= "table" or type(conditions.n) ~= "number" then
				JBP:Error("ConditionSet %q does not have a settingKey that corresponds " .. 
				"to a valid table of condition settings in the settings of its defined parentSettingType", identifier)
				isGood = false
			end
			
			if isGood then
				for conditionID, condition in JBP:InNLengthTable(parentSettings[conditionSetData.settingKey]) do
					JBP:Upgrade("condition", upgradeData, condition, conditionID)
				end
			end
...
do -- InConditionSettings
	...
	-- @usage	for conditionSettings, conditionID, parentTable in JBP:InConditionSettings() do
	--		print(conditionSettings, conditionID)
	--		assert(conditionSetting == parentTable[conditionID])
	--	end
	function JBP:InConditionSettings()
