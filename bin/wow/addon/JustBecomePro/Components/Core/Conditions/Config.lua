...
JBP.HELP:NewCode("CNDT_UNIT_MISSING", 10, false)
JBP.HELP:NewCode("CNDT_UNIT_ONLYONE", 20, false)


local CNDT = JBP.CNDT -- created in JustBecomePro/conditions.lua
...
--TODO: try to get rid of all manual calls to LoadConfig (on other JBP modules too), and have these functions only be called from ReloadRequsted.
...
function CNDT:LoadConfig(conditionSetName)
	...
	JBP.HELP:Hide("CNDT_UNIT_MISSING")
	...
	if n > 0 then
		...
		for i in JBP:InNLengthTable(CNDT:GetSettings()) do
			CNDT[i]:Show()
			CNDT[i]:RequestReload()
		end
	end
	
	local AddCondition = JustBecomePro_IconEditor.Pages.Conditions.Groups.AddCondition
...
CNDT.DynamicConditionTab = JBP.IE:RegisterTab("ICON", "CNDTDYN", "Conditions", 25)
...
JBP:RegisterCallback("JBP_CONFIG_ICON_LOADED_CHANGED", function(event, icon)
	if JBP.IE.CurrentTab == CNDT.DynamicConditionTab then
		JBP.IE.TabGroups.ICON.MAIN:Click()
	end
end)

JBP:RegisterCallback("JBP_CONFIG_TAB_CLICKED", function(event, currentTab, oldTab)
...
function CNDT:GetTabText(conditionSetName)
	...
	if parenthesesAreValid then
		JBP.HELP:Hide("CNDT_PARENTHESES_ERROR")
	else
		JBP.HELP:Show{
			code = "CNDT_PARENTHESES_ERROR",
			icon = nil,
			relativeTo = JustBecomePro_IconEditor.Pages.Conditions,
			x = 0,
			y = 0,
	...
	if n > 0 then
		local prefix = (not parenthesesAreValid and "|TInterface\\AddOns\\JustBecomePro\\Textures\\Alert:0:2|t|cFFFF0000" or "")
...
function CNDT:SetTabText(conditionSetName)
	...
	JBP:TT(tab, ConditionSet.tabText, ConditionSet.tabTooltip, 1, 1)
end
...
local function AddConditionToDropDown(dropdown, conditionData)
	...
	local append = JBP.debug and not conditionData:ShouldList() and "(DBG)" or ""
...
function CNDT.TypeMenu_DropDown(dropdown)
	...
	elseif JBP.DD.MENU_LEVEL == 2 then
		local categoryData = CNDT.CategoriesByID[JBP.DD.MENU_VALUE]
		...
		for k, conditionData in ipairs(categoryData.conditionData) do
			if conditionData.IS_SPACER then
				...
				local shouldAdd = selected or conditionData:ShouldList() --or JBP.debug
...
function CNDT.OperatorMenu_DropDown(dropdown)
	...
	for k, v in pairs(JBP.operators) do
		if (not conditionData.specificOperators or conditionData.specificOperators[v.value]) then
			...
			info.value = v.value
			info.font = "Interface/Addons/JustBecomePro/Fonts/OpenSans-Regular.ttf"
...
function CNDT:InBitflags(bitFlags)
	...
	return JBP:OrderedPairs(bitFlags, tableValues and JBP.OrderSort or nil, tableValues)
end
...
function CNDT:ColorizeParentheses()
	if not JustBecomePro_IconEditor.Pages.Conditions:IsShown() then return end
...
JBP:NewClass("Config_Conditions_Paren", "Config_CheckButton") {
	OnNewInstance_Paren = function(self)
		local parent = self:GetParent()
		parent.parens = parent.parens or {}

		self.text:SetFont("Interface/Addons/JustBecomePro/Fonts/OpenSans-Regular.ttf", 16, "THINOUTLINE")
	...
	GetNext = function(self)
		...
		elseif self:GetID() < 13 then
			paren = CreateFrame("CheckButton", nil, parent, "JustBecomePro_ConditionEditorParenthesisTemplate", self:GetID() + 1)
	...
	OnClick = function(self)
		...
		JBP.HELP:Hide("CNDT_PARENTHESES_FIRSTSEE")
...
function CNDT:CreateGroups(num)
	...
	for i=start, num do
		JBP.Classes.CndtGroup:New("Frame", "JustBecomePro_IconEditorConditionsGroupsGroup" .. i, JustBecomePro_IconEditor.Pages.Conditions.Groups, "JustBecomePro_ConditionGroup", i)
	end
end
...
function CNDT:AddCondition(Conditions)
	...
	JBP:Fire("JBP_CNDT_CONDITION_ADDED", Conditions[Conditions.n])
...
function CNDT:DeleteCondition(Conditions, n)
	...
	JBP:Fire("JBP_CNDT_CONDITION_DELETED", n)
...
---------- CndtGroup Class ----------
local CndtGroup = JBP:NewClass("CndtGroup", "Config_Frame")
...
function CndtGroup:LoadAndDraw()
	...
	if conditionSettings then
		JBP:Fire("JBP_CNDT_GROUP_DRAWGROUP", self, conditionData, conditionSettings)
	end
...
JBP:RegisterCallback("JBP_CNDT_GROUP_DRAWGROUP", function(event, CndtGroup, conditionData, conditionSettings)
	...
	if conditionData then
		...
		if unit == nil then
			...
			JBP.SUG:EnableEditBox(CndtGroup.Unit, "units", true)
...
JBP:RegisterCallback("JBP_CNDT_GROUP_DRAWGROUP", function(event, CndtGroup, conditionData, conditionSettings)
	...
	JBP:TT(CndtGroup.Type, text, tooltip, 1, 1)
	JBP:TT(CndtGroup.Type.EditBox, text, tooltip, 1, 1)
end)
...
JBP:RegisterCallback("JBP_CNDT_GROUP_DRAWGROUP", function(event, CndtGroup, conditionData, conditionSettings)
	if not conditionData or conditionData.nooperator then
		...
		local v = CndtGroup.Operator:SetUIDropdownText(conditionSettings.Operator, JBP.operators)
		if v then
			JBP:TT(CndtGroup.Operator, v.tooltipText, nil, 1)
		end
	end
end)
...
JBP:RegisterCallback("JBP_CNDT_GROUP_DRAWGROUP", function(event, CndtGroup, conditionData, conditionSettings)
	...
	if CndtGroup:GetID() == 3 and CndtGroup:IsVisible() then
		JBP.HELP:Show{
			...
			x = 0,
			y = 0,
			text = format(JBP.L["HELP_CNDT_PARENTHESES_FIRSTSEE"])
		}
	end
end)
JBP.HELP:NewCode("CNDT_PARENTHESES_FIRSTSEE", 101, true)
...
JBP:RegisterCallback("JBP_CNDT_GROUP_DRAWGROUP", function(event, CndtGroup, conditionData, conditionSettings)
	if CndtGroup:GetID() == 2 and CndtGroup:IsVisible() then
		JBP.HELP:Show{
			...
			x = 0,
			y = 0,
			text = format(JBP.L["HELP_CNDT_ANDOR_FIRSTSEE"])
		}
	end
end)
JBP.HELP:NewCode("CNDT_ANDOR_FIRSTSEE", 100, true)
...
JBP:RegisterCallback("JBP_CNDT_GROUP_DRAWGROUP", function(event, CndtGroup, conditionData, conditionSettings)
	if conditionData then
		...
		for _, suffix in JBP:Vararg("", "2") do
			...
			JBP.SUG:DisableEditBox(EditBox)
			Check:Hide()
			...
			if dataName then
				...
				JBP.SUG:EnableEditBox(EditBox, conditionData.useSUG, not conditionData.allowMultipleSUGEntires)
		...
		if conditionData.noslide then
			...
			if conditionData.levelChecks then
				...
				for level = min, max do
					...
					if not frame then
						frame = JBP.C.Config_CheckButton:New("CheckButton", nil, LevelChecks, "JustBecomePro_CheckTemplate", frameID)
				...
				JBP.IE:DistributeCheckAnchorsEvenly(LevelChecks, unpack(LevelChecks.frames, 1, frameID))
			else
				CndtGroup.LevelChecks:Hide()
	...
	if conditionData then
		...
		if conditionData.funcstr == "DEPRECATED" then
			if conditionData.customDeprecated then
				...
				text = JBP.L["CNDT_DEPRECATED_DESC"]:format(get(conditionData.text))
			end
		...
		CndtGroup.Unknown:SetFormattedText(JBP.L["CNDT_UNKNOWN_DESC"], conditionSettings.Type)
	end
end)


JBP:NewClass("Config_Conditions_AndOr", "Config_Button") {
	value = nil,
	...
	OnClick = function(self)
		...
		JBP.HELP:Hide("CNDT_ANDOR_FIRSTSEE")
