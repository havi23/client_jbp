...
-- Creates a simple instance of JBP.Classes.UpdateTableManager.
function UpdateEngine:CreateUpdateTableManager()
	local manager = JBP.Classes.UpdateTableManager:New()
...
JBP:NewClass("EventUpdateTableManager", "UpdateTableManager"){
...
function UpdateEngine:CreateEventUpdateTableManager(event)
	local manager = JBP.Classes.EventUpdateTableManager:New(event)
...
function UpdateEngine:RegisterObjForEvent(ConditionObject, event)
	...
	if event:find("^JBP_") then
		JBP:RegisterCallback(event, ConditionObject.UpdateFunction, ConditionObject)
	else		
...
function UpdateEngine:UnregisterObjForEvent(ConditionObject, event)
	if event:find("^JBP_") then
		JBP:UnregisterCallback(event, ConditionObject.UpdateFunction, ConditionObject)
	else
		local UpdateTableManager = UpdateEngine.EventUpdateTableManagers[event]
...
JBP:RegisterCallback("JBP_ONUPDATE_TIMECONSTRAINED_PRE", UpdateEngine, "OnUpdate")


-- Automatically update all used ConditionObjects after a JBP_GLOBAL_UPDATE_POST.
JBP:RegisterCallback("JBP_GLOBAL_UPDATE_POST", function()
	for _, ConditionObject in pairs(JBP.C.ConditionObject.instances) do
