...
function Env.AuraCount(unit, nameRaw, filter)
	local n = 0
	local names = JBP:GetSpells(nameRaw).Hash
...
function Env.AuraDur(unit, name, filter)
	...
	if not buffName then
		...
		return expirationTime == 0 and huge or expirationTime - JBP.time, duration, expirationTime
	end
end
...
function Env.AuraPercent(unit, name, filter)
	...
	if not buffName then
		...
		return expirationTime == 0 and 1 or ((expirationTime - JBP.time) / duration)
	end
end
...
function Env.AuraTooltipNumber(...)
	local Parser, LT1, LT2 = JBP:GetParser()
	...
	local function JBP_UNITSET_UPDATED(event, UnitSet)
		local unit = unitSets[UnitSet]
		if unit and UnitSet.allUnitsChangeOnEvent then
			wipe(cache[unit])
		end
	end
	JBP:RegisterCallback("JBP_UNITSET_UPDATED", JBP_UNITSET_UPDATED)
	...
	function Env.AuraTooltipNumber(unit, name, filter, requestedIndex)
		...
		if not UnitSet then
			-- You're supposed to use JBP:GetUnits(), but this is much faster and is sufficient for our needs.
			UnitSet = JBP.UNITS:GetUnitSet(unit)
...
ConditionCategory:RegisterCondition(2.5, "BUFFPERC", {
	...
	NEVERMIND. STILL HAS APPLICATION FOR ROGUE DOTS WHOSE DURATION DEPENDS ON COMBO PTS USED
		...
		the BASE DURATION of the effect. JustBecomePro can't (reliably) determine the
	...
	formatter = JBP.C.Formatter.PERCENT,
...
ConditionCategory:RegisterCondition(9,	 "ABSORBAMT", {
	...
	formatter = JBP.C.Formatter.COMMANUMBER,
	tcoords = CNDT.COMMON.standardtcoords,
	Env = {
