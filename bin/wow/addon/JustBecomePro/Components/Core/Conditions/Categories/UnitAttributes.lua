...
ConditionCategory:RegisterCondition(8.5,  "LIBRANGECHECK", {
	...
	min = 0,
	max = 100,
	formatter = JBP.C.Formatter:New(function(val)
	...
	funcstr = function(c, parent)
		...
		if not Env.LibRangeCheck then
			JBP:Error("The %s condition requires LibRangeCheck-2.0", L["CNDT_RANGE"])
			return "false"
		end
		...
		elseif c.Operator == ">=" then
			...
			JBP:Error("Bad operator %q for range check condition of %s", c.Operator, tostring(parent))
		end
	end,
	-- events = absolutely no events
})
...
ConditionCategory:RegisterCondition(13.1,   "UNITRACE", {
	...
	bitFlags = (function()
		...
		local function Name(name)
			...
			if not lookup[name] then
				JBP:Debug("Missing LibBabble-Race-3.0 phrase for: " .. name)
				return name
			end
			return lookup[name]
		end
		...
		for token, data in pairs(bitFlags) do
			data.atlas = JBP:GetRaceIconInfo(token)
	...
	atlas = JBP:GetRaceIconInfo(select(2, UnitRace("player"))),
	tcoords = CNDT.COMMON.standardtcoords,
