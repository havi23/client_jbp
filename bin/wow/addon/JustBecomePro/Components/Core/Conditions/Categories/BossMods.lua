...
local function BigWigs_timer_init()
	...
	if not BigWigsLoader then
		JBP:Warn("BigWigsLoader wasn't loaded when BigWigs timer conditions tried to initialize.")
	...
	local function stop(module, text)
		for k = #Timers, 1, -1 do
			...
			if t.module == module and (not text or t.text == text) then
				tremove(Timers, k)
				JBP:Fire("JBP_CNDT_BOSSMODS_BIGWIGS_TIMER_CHANGED")
			elseif t.start + t.duration < JBP.time then
				tremove(Timers, k)
				JBP:Fire("JBP_CNDT_BOSSMODS_BIGWIGS_TIMER_CHANGED")
			end
		end
	...
	BigWigsLoader.RegisterMessage(owner, "BigWigs_StartBar", function(_, module, key, text, time)
			...
			tinsert(Timers, {module = module, key = key, text = text:lower(), start = JBP.time, duration = time})
	...
	function Env.BigWigs_GetTimeRemaining(text)
		for k = 1, #Timers do
			...
			if t.text:match(text) then
				...
				local remaining = (expirationTime) - JBP.time
...
ConditionCategory:RegisterCondition(1,	 "BIGWIGS_TIMER", {
	...
	events = function(ConditionObject, c)
		return
			ConditionObject:GenerateNormalEventString("JBP_CNDT_BOSSMODS_BIGWIGS_TIMER_CHANGED")
	end,
...
local function BigWigs_engaged_init()
	...
	if not BigWigsLoader then
		JBP:Warn("BigWigsLoader wasn't loaded when BigWigs engaged conditions tried to initialize.")
	...
	BigWigsLoader.RegisterMessage(owner, "BigWigs_OnBossEngage", function(_, module, diff)
			...
			JBP:Fire("JBP_CNDT_BOSSMODS_BIGWIGS_ENGAGED_CHANGED")
	end)
	BigWigsLoader.RegisterMessage(owner, "BigWigs_OnBossDisable", function(_, module)
...
ConditionCategory:RegisterCondition(2,	 "BIGWIGS_ENGAGED", {
	...
	events = function(ConditionObject, c)
		return
			ConditionObject:GenerateNormalEventString("JBP_CNDT_BOSSMODS_BIGWIGS_ENGAGED_CHANGED")
	end,
})
...
local function DBM_timer_init()
	DBM_timer_init = nil
	if not DBM then
		JBP:Warn("DBM wasn't loaded when DBM timer conditions tried to initialize.")
	...
	DBM:RegisterCallback("DBM_TimerStart", function(_, id, text, timerRaw)
		...
		Timers[id] = {text = text:lower(), start = JBP.time, duration = duration}

		JBP:Fire("JBP_CNDT_BOSSMODS_DBM_TIMER_CHANGED")
	end)
...
ConditionCategory:RegisterCondition(10,	 "DBM_TIMER", {
	...
	events = function(ConditionObject, c)
		return
			ConditionObject:GenerateNormalEventString("JBP_CNDT_BOSSMODS_DBM_TIMER_CHANGED")
	end,
	anticipate = function(c)
		local name 
		if c.Checked then
...
local function DBM_engaged_init()
	...
	if not DBM then
		JBP:Warn("DBM wasn't loaded when DBM engaged conditions tried to initialize.")
	...
	hooksecurefunc(DBM, "StartCombat", function(DBM, mod, delay, event)
		if event ~= "TIMER_RECOVERY" then
			...
			JBP:Fire("JBP_CNDT_BOSSMODS_DBM_ENGAGED_CHANGED")
		end
	end)
...
ConditionCategory:RegisterCondition(11,	 "DBM_ENGAGED", {
	...
	events = function(ConditionObject, c)
		return
			ConditionObject:GenerateNormalEventString("JBP_CNDT_BOSSMODS_DBM_ENGAGED_CHANGED")
	end,
})
