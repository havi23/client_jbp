...
ConditionCategory:RegisterCondition(7,	 "MELEEHASTE", {
	text = STAT_HASTE,
	percent = true,
	formatter = JBP.C.Formatter.PLUSPERCENT,
	min = 0,
	range = 100,
	unit = PLAYER,
...
if MAX_SPELL_SCHOOLS ~= 7 then
	JBP:Error("MAX_SPELL_SCHOOLS has changed, so the spell school dependent conditions need updating")
end
...
ConditionCategory:RegisterCondition(31,	 "SPELLDMG", {
	...
	events = function(ConditionObject, c)
		return
			...
			ConditionObject:GenerateNormalEventString("SPELL_POWER_CHANGED") --JBP.ISMOP
	end,
})
...
ConditionCategory:RegisterCondition(35,	 "MANAREGEN", {
	...
	range = 1000/5,
	unit = PLAYER,
	texttable = function(k) return format(L["MP5"], JBP.C.Formatter.COMMANUMBER:Format(k)*5) end,
