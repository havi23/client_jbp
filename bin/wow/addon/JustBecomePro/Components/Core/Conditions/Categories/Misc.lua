...
ConditionCategory:RegisterCondition(1,	 "ICON", {
	...
	funcstr = function(c, icon)
		...
		JBP:QueueValidityCheck(icon, c.Icon, L["VALIDITY_CONDITION_DESC"])
	...
	--[[events = function(ConditionObject, c)
		local event = JBP.Classes.IconDataProcessor.ProcessorsByName.REALALPHA.changedEvent
...
local function RegisterShownHiddenTimerCallback()
	JBP:RegisterCallback(JBP.Classes.IconDataProcessor.ProcessorsByName.REALALPHA.changedEvent, function(event, icon, realAlpha, oldalpha)
		if realAlpha == 0 then
			icon.__CNDT__ICONSHOWNTME = 0
			icon.__CNDT__ICONHIDDENTME = JBP.time
		elseif oldalpha == 0 then
			icon.__CNDT__ICONSHOWNTME = JBP.time
			icon.__CNDT__ICONHIDDENTME = 0
		end
	end)
	
	RegisterShownHiddenTimerCallback = JBP.NULLFUNC
end
...
JBP:RegisterLuaImportDetector(function(table)
