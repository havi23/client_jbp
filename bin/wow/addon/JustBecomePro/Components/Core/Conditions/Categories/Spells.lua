...
function Env.CooldownDuration(spell)
	if spell == "gcd" then
		local start, duration = GetSpellCooldown(JBP.GCDSpell)
		return duration == 0 and 0 or (duration - (JBP.time - start))
	end

	local start, duration = GetSpellCooldown(spell)
	if duration then
		return ((duration == 0 or OnGCD(duration)) and 0) or (duration - (JBP.time - start))
	end
	return 0
end
...
local SwingTimers = JBP.COMMON.SwingTimerMonitor.SwingTimers
function Env.SwingDuration(slot)
	local SwingTimer = SwingTimers[slot]
	
	if SwingTimer then
		return max(SwingTimer.duration - (JBP.time - SwingTimer.startTime), 0)
	end
	return 0
end
...
ConditionCategory:RegisterCondition(2.6, "SPELLCHARGETIME", {
	...
	formatter = JBP.C.Formatter:New(function(value)
		local s = JBP.C.Formatter.TIME_YDHMS:Format(value)
		if value == 0 then
...
ConditionCategory:RegisterCondition(2.8, "LASTCAST", {
		...
		if not module then
			...
			function()
				...
				if e == "SPELL_CAST_SUCCESS" and sourceGuid == pGUID and not blacklist[spellID] then
					...
					JBP:Fire("JBP_CNDT_LASTCAST_UPDATED")
				end
			end)
	...
	events = function(ConditionObject, c)
		...
		return
			...
			ConditionObject:GenerateNormalEventString("JBP_CNDT_LASTCAST_UPDATED")
	end,
})
...
ConditionCategory:RegisterCondition(3,	 "REACTIVE", {
	...
	useSUG = true,
	unit = false,
	formatter = JBP.C.Formatter.BOOL_USABLEUNUSABLE,
...
ConditionCategory:RegisterCondition(4,	 "MANAUSABLE", {
	...
	Env = {
		SpellHasNoMana = JBP.SpellHasNoMana
	},
	events = function(ConditionObject, c)
		return
			ConditionObject:GenerateNormalEventString("SPELL_UPDATE_USABLE"),
...
ConditionCategory:RegisterCondition(6,	 "GCD", {
	...
	tcoords = CNDT.COMMON.standardtcoords,
	funcstr = [[BOOLCHECK( (JBP.GCD > 0 and JBP.GCD < 1.7) )]],
	events = function(ConditionObject, c)
		return
			ConditionObject:GenerateNormalEventString("SPELL_UPDATE_COOLDOWN"),
			ConditionObject:GenerateNormalEventString("SPELL_UPDATE_USABLE")
...
ConditionCategory:RegisterCondition(19,	 "MHSWING", {
	...
	hidden = not JBP.COMMON.SwingTimerMonitor,
	anticipate = [[
...
ConditionCategory:RegisterCondition(19.5,	 "OHSWING", {
	...
	events = function(ConditionObject, c)
		...
		return
			"event == 'JBP_COMMON_SWINGTIMER_CHANGED' and arg1.slot == " .. GetInventorySlotInfo("SecondaryHandSlot")
	end,
	hidden = not JBP.COMMON.SwingTimerMonitor,
	anticipate = [[
...
local totemData = JBP.COMMON.CurrentClassTotems
...
function Env.TotemHelper(slot, nameString)
	...
	return duration and duration ~= 0 and (duration - (JBP.time - start)) or 0
end
...
for i = 1, 5 do
	...
	ConditionCategory:RegisterCondition(20 + i,	 "TOTEM" .. i, {
		...
		formatter = JBP.C.Formatter.TIME_0ABSENT,
...
Env.UnitCastTime = function(unit, level, matchname)
	...
	local remaining = endTime and endTime/1000 - JBP.time or 0
	if level == 0 then -- only interruptible
...
local function CASTCOUNT_COMBAT_LOG_EVENT_UNFILTERED()
	...
	if cleuEvent == "SPELL_CAST_SUCCESS" then
		...
		JBP:Fire("JBP_CNDT_CASTCOUNT_UPDATE")
...
ConditionCategory:RegisterCondition(32,	 "CASTCOUNT", {
	...
	events = function(ConditionObject, c)
		return
			ConditionObject:GetUnitChangedEventString(CNDT:GetUnit(c.Unit)),
			ConditionObject:GenerateNormalEventString("JBP_CNDT_CASTCOUNT_UPDATE")
	end,
})
