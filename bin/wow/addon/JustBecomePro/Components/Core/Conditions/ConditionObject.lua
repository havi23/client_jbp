...
-- Changes to a ConditionObject's state may be listed to by registering to the {{{JBP_CNDT_OBJ_PASSING_CHANGED(ConditionObject, failed)}}} event with {{{JBP:RegisterCallback()}}}.
-- 
-- @class file
-- @name ConditionObject.lua


local ConditionObject = JBP:NewClass("ConditionObject")
...
function ConditionObject:OnNewInstance(Conditions, conditionString)
	...
	for n, condition in JBP:InNLengthTable(Conditions) do
	...
	if func then
		-- func = setfenv(func, JBP.CNDT.Env) -- why was this here? commented out because it is redundant
		self.CheckFunction = setfenv(func, JBP.CNDT.Env)
	elseif err then
		JBP:Error(err)
	end
...
function ConditionObject:CompileUpdateFunction(Conditions)
	...
	for _, c in JBP:InNLengthTable(Conditions) do
		local t = c.Type
		local condition = CNDT.ConditionsByType[t]
		
		if condition and condition.events then
			local voidNext
			for n, argCheckerString in JBP:Vararg(JBP.get(condition.events, self, c)) do
				...
				elseif type(argCheckerString) == "string" then
					...
					elseif argCheckerString == "" then
						JBP:Error("Condition.events shouldn't return blank strings! (From condition %q). Return STRING 'false' if you don't want the condition to update OnUpdate but it also has no events (basically, if it is static).", t)
					else
		...
		if condition.anticipate then
			...
			local thisstr = JBP.get(condition.anticipate, c) -- get the anticipator string from the condition data
	...
	if argCheckerStringComposite ~= "" then
		...
		JBP:Error("The arg checker string compiled for ConditionObject %s was blank. This should not have happened.", tostring(self))
	end
	...
	if func then
		func = setfenv(func, JBP.CNDT.Env)
	elseif err then
		JBP:Error(err)
	end
...
-- <<code lua>>
--	if ConditionObject.UpdateNeeded or ConditionObject.NextUpdateTime < JBP.time then
--		ConditionObject:Check()
--	end
-- <</code>>
...
function ConditionObject:Check()
	if self.CheckFunction then
		if not self.registeredForUpdating then
			JBP:Debug("condition was checked, but nobody said they would be checking it!")
		end

		local failed = not self:CheckFunction()
		if self.Failed ~= failed then
			self.Failed = failed
			JBP:Fire("JBP_CNDT_OBJ_PASSING_CHANGED", self, failed)
		end
		...
		if self.NextUpdateTime < JBP.time then
			self.NextUpdateTime = huge
		end
	end
end
...
--   ConditionObject:RequestEvent("JBP_COMMON_SWINGTIMER_CHANGED")
--   ConditionObject:SetNumEventArgs(1)
--   return
--     "event == 'JBP_COMMON_SWINGTIMER_CHANGED' and arg1.slot == " .. GetInventorySlotInfo("MainHandSlot")
-- end
...
-- @param event [string] An event to request. Can either be a Blizzard event or a JBP event.
-- @usage events = function(ConditionObject, c)
--   ConditionObject:RequestEvent("JBP_COMMON_SWINGTIMER_CHANGED")
--   ConditionObject:SetNumEventArgs(1)
--   return
...
-- @param event [string] An event to check. Can either be a Blizzard event or a JBP event.
...
function ConditionObject:GenerateNormalEventString(event, ...)
	...
	for n, arg in JBP:Vararg(...) do
		...
		if 
			...
			arg_type ~= "nil" 
		then
			JBP:Error("Unsupported event arg type: " .. arg_type)
		elseif arg ~= nil then
			str = str .. " and arg"
			str = str .. n
			str = str .. " == "
