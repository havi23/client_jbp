...
-- STATE: "state"
do
	local Processor = JBP.Classes.IconDataProcessor:New("STATE", "state")
	...
	function Processor:CompileFunctionSegment(t)
		...
		--if state ~= nil then
			...
			if attributes.state ~= stateData then
				...
				JBP:Fire(STATE.changedEvent, icon, stateData, state)
	...
	JBP:RegisterCallback("JBP_ICON_SETUP_PRE", function(event, icon)
...
do
	local Processor = JBP.Classes.IconDataProcessor:New("CALCULATEDSTATE", "calculatedState")
	Processor.dontInherit = true
end
...
do
	...
	local Processor = JBP.Classes.IconDataProcessor:New("ALPHAOVERRIDE", "alphaOverride")
	...
	function Processor:CompileFunctionSegment(t)
		...
		t[#t+1] = [[
			...
			if attributes.alphaOverride ~= stateData then
				...
				JBP:Fire(STATE.changedEvent, icon, stateData, alphaOverride)
				doFireIconUpdated = true
			end
		--]]
	end
...
-- REALALPHA: "realAlpha"
do
	local Processor = JBP.Classes.IconDataProcessor:New("REALALPHA", "realAlpha")
	Processor.dontInherit = true
	Processor:AssertDependency("SHOWN")

	JBP.Classes.Icon.attributes.realAlpha = 0
	...
	Processor:RegisterIconEvent(13, "OnAlphaInc", {
		...
		conditionChecker = function(icon, eventSettings)
			return JBP.CompareFuncs[eventSettings.Operator](icon.attributes.realAlpha * 100, eventSettings.Value)
		end,
	})
	...
	function Processor:CompileFunctionSegment(t)
		...
		if realAlpha ~= attributes.realAlpha then
			...
			JBP:Fire(REALALPHA.changedEvent, icon, realAlpha, oldalpha)
			doFireIconUpdated = true
		end
		--]]
	end
	...
	JBP:RegisterCallback("JBP_INITIALIZE", function()
		local IconPosition_Sortable = JBP.C.GroupModule_IconPosition_Sortable
	...
	Processor:RegisterDogTag("JBP", "IsShown", {	
		code = function(icon)
			icon = JBP.GUIDToOwner[icon]
		...
		events = JBP:CreateDogTagEventString("SHOWN", "REALALPHA"),
		ret = "boolean",
		doc = L["DT_DOC_IsShown"] .. "\r\n \r\n" .. L["DT_INSERTGUID_GENERIC_DESC"],
		example = '[IsShown] => "true"; [IsShown(icon="JBP:icon:1I7MnrXDCz8T")] => "false"',
		category = L["ICON"],
	})
	Processor:RegisterDogTag("JBP", "Opacity", {	
		code = function(icon)
			icon = JBP.GUIDToOwner[icon]
		...
		events = JBP:CreateDogTagEventString("REALALPHA"),
		ret = "number",
		doc = L["DT_DOC_Opacity"] .. "\r\n \r\n" .. L["DT_INSERTGUID_GENERIC_DESC"],
		example = '[Opacity] => "1"; [Opacity(icon="JBP:icon:1I7MnrXDCz8T")] => "0.42"',
		category = L["ICON"],
	})
end
...
do
	local Processor = JBP.Classes.IconDataProcessor:New("CONDITION", "conditionFailed")
	Processor.dontInherit = true
...
do
	local Processor = JBP.Classes.IconDataProcessor:New("DURATION", "start, duration")
	Processor:DeclareUpValue("OnGCD", JBP.OnGCD)

	JBP.Classes.Icon.attributes.start = 0
	JBP.Classes.Icon.attributes.duration = 0
	JBP.Classes.Icon.__realDuration = 0
	...
	Processor:RegisterIconEvent(23, "OnDuration", {
		...
		conditionChecker = function(icon, eventSettings)
			local attributes = icon.attributes
			local d = attributes.duration - (JBP.time - attributes.start)
			d = d > 0 and d or 0

			return JBP.CompareFuncs[eventSettings.Operator](d, eventSettings.Value)
		end,
		applyDefaultsToSetting = function(EventSettings)
	...
	function Processor:CompileFunctionSegment(t)
		...
		if EventHandlersSet.OnDuration then
			local d = duration - (JBP.time - start)
			d = d > 0 and d or 0
		...
		if attributes.start ~= start or attributes.duration ~= duration then
			...
			JBP:Fire(DURATION.changedEvent, icon, start, duration)
			doFireIconUpdated = true
		end
		--]]
	end
	...
	function Processor:RegisterDurationTrigger(icon, duration)
		...
		if not JBP.tContains(DurationTriggers[icon], duration) then
	...
	JBP:RegisterCallback("JBP_ONUPDATE_TIMECONSTRAINED_PRE", function(event, time, Locked)
...
-- REVERSE: "reverse"
do
	local Processor = JBP.Classes.IconDataProcessor:New("REVERSE", "reverse")
	-- Processor:CompileFunctionSegment(t) is default.

	JBP:RegisterCallback("JBP_ICON_DISABLE", function(event, icon)
...
-- SPELL: "spell"
do
	local Processor = JBP.Classes.IconDataProcessor:New("SPELL", "spell")
	...
	function Processor:CompileFunctionSegment(t)
		...
		if attributes.spell ~= spell then
			...
			JBP:Fire(SPELL.changedEvent, icon, spell)
			doFireIconUpdated = true
		end
		--]]
	end
	...
	Processor:RegisterDogTag("JBP", "Spell", {
		...
		events = JBP:CreateDogTagEventString("SPELL"),
		ret = "string",
		doc = L["DT_DOC_Spell"] .. "\r\n \r\n" .. L["DT_INSERTGUID_GENERIC_DESC"],
		example = ('[Spell] => %q; [Spell(link=true)] => %q; [Spell(icon="JBP:icon:1I7MnrXDCz8T")] => %q; [Spell(icon="JBP:icon:1I7MnrXDCz8T", link=true)] => %q'):format(GetSpellInfo(2139), GetSpellLink(2139), GetSpellInfo(1766), GetSpellLink(1766)),
		category = L["ICON"],
	})
...
do
	local Processor = JBP.Classes.IconDataProcessor:New("SPELLCHARGES", "charges, maxCharges, chargeStart, chargeDur")
	...
	function Processor:CompileFunctionSegment(t)
		...
		or attributes.chargeDur ~= chargeDur then
			...
			JBP:Fire(SPELLCHARGES.changedEvent, icon, charges, maxCharges, chargeStart, chargeDur)
			doFireIconUpdated = true
		end
		--]]
	end
...
do
	...
	JBP.C.IconComponent:RegisterDogTag("JBP", "Duration", {
		code = function(icon, gcd, ignoreCharges)
			...
			if icon then
				...
				if not ignoreCharges and chargeDur and chargeDur > 0 then
					...
					local remaining = chargeDur - (JBP.time - attributes.chargeStart)
					if remaining > 0 then
				...
				local remaining = duration - (JBP.time - attributes.start)
		...
		example = '[Duration] => "1.435"; [Duration(gcd=false)] => "0"; [Duration(ignorecharges=true)] => "0"; [Duration:JBPFormatDuration] => "1.4"; [Duration(icon="JBP:icon:1I7MnrXDCz8T")] => "97.32156"; [Duration(icon="JBP:icon:1I7MnrXDCz8T"):JBPFormatDuration] => "1:37"',
		category = L["ICON"],
	})
	
	JBP.C.IconComponent:RegisterDogTag("JBP", "MaxDuration", {
		...
		example = '[MaxDuration] => "3"; [MaxDuration:JBPFormatDuration] => "3.0"; [MaxDuration(icon="JBP:icon:1I7MnrXDCz8T")] => "60"',
		category = L["ICON"],
	})
end
...
do
	local Processor = JBP.Classes.IconDataProcessor:New("VALUE", "value, maxValue, valueColor")
	...
	function Processor:CompileFunctionSegment(t)
		...
		if attributes.value ~= value or attributes.maxValue ~= maxValue or attributes.valueColor ~= valueColor then
			...
			JBP:Fire(VALUE.changedEvent, icon, value, maxValue, valueColor)
			doFireIconUpdated = true
		end
		--]]
	end
	...
	Processor:RegisterDogTag("JBP", "Value", {
		code = function(icon)
			icon = JBP.GUIDToOwner[icon]
		...
		events = JBP:CreateDogTagEventString("VALUE"),
		ret = "number",
		doc = L["DT_DOC_Value"] .. "\r\n \r\n" .. L["DT_INSERTGUID_GENERIC_DESC"],
		example = '[Value] => "256891"; [Value(icon="JBP:icon:1I7MnrXDCz8T")] => "2"',
		category = L["ICON"],
	})
		
	Processor:RegisterDogTag("JBP", "ValueMax", {
		code = function(icon)
			icon = JBP.GUIDToOwner[icon]
		...
		example = '[ValueMax] => "312856"; [ValueMax(icon="JBP:icon:1I7MnrXDCz8T")] => "3"',
		category = L["ICON"],
	})
end
...
do
	local Processor = JBP.Classes.IconDataProcessor:New("STACK", "stack, stackText")
	...
	function Processor:CompileFunctionSegment(t)
		...
		if attributes.stack ~= stack or attributes.stackText ~= stackText then
			...
			JBP:Fire(STACK.changedEvent, icon, stack, stackText)
			doFireIconUpdated = true
		end
		--]]
	end
	...
	Processor:RegisterIconEvent(51, "OnStack", {
		...
		conditionChecker = function(icon, eventSettings)
			...
			return JBP.CompareFuncs[eventSettings.Operator](count, eventSettings.Value)
		end,
	})
	...
	Processor:RegisterDogTag("JBP", "Stacks", {
		code = function(icon)
			icon = JBP.GUIDToOwner[icon]
		...
		events = JBP:CreateDogTagEventString("STACK"),
		ret = "number",
		doc = L["DT_DOC_Stacks"] .. "\r\n \r\n" .. L["DT_INSERTGUID_GENERIC_DESC"],
		example = '[Stacks] => "9"; [Stacks(icon="JBP:icon:1I7MnrXDCz8T")] => "3"',
		category = L["ICON"],
	})
...
-- TEXTURE: "texture"
do
	local Processor = JBP.Classes.IconDataProcessor:New("TEXTURE", "texture")
	...
	function Processor:CompileFunctionSegment(t)
		...
		if texture ~= nil and attributes.texture ~= texture then
			...
			JBP:Fire(TEXTURE.changedEvent, icon, texture)
			doFireIconUpdated = true
		end
		--]]
	end
...
-- UNIT: "unit, GUID"
do
	local Processor = JBP.Classes.IconDataProcessor:New("UNIT", "unit, GUID")
	...
	function Processor:CompileFunctionSegment(t)
		...
		if attributes.unit ~= unit or attributes.GUID ~= GUID then
			...
			JBP:Fire(UNIT.changedEvent, icon, unit, previousUnit, GUID)
			doFireIconUpdated = true
		end
		--]]
	end
	...
	Processor:RegisterDogTag("JBP", "Unit", {
		code = function(icon)
			icon = JBP.GUIDToOwner[icon]
		...
		events = JBP:CreateDogTagEventString("UNIT"),
		ret = "string",
		doc = L["DT_DOC_Unit"] .. "\r\n \r\n" .. L["DT_INSERTGUID_GENERIC_DESC"],
		example = '[Unit] => "target"; [Unit:Name] => "Kobold"; [Unit(icon="JBP:icon:1I7MnrXDCz8T")] => "focus"; [Unit(icon="JBP:icon:1I7MnrXDCz8T"):Name] => "Gamon"',
		category = L["ICON"],
	})
	Processor:RegisterDogTag("JBP", "PreviousUnit", {
		code = function(icon)
			icon = JBP.GUIDToOwner[icon]
		...
		example = '[PreviousUnit] => "target"; [PreviousUnit:Name] => "Kobold"; [PreviousUnit(icon="JBP:icon:1I7MnrXDCz8T")] => "focus"; [PreviousUnit(icon="JBP:icon:1I7MnrXDCz8T"):Name] => "Gamon"',
		category = L["ICON"],
	})
...
do
	...
	local Processor = JBP.Classes.IconDataProcessor:New("DOGTAGUNIT", "dogTagUnit")
	Processor:AssertDependency("UNIT")


	--Here's the hook (the whole point of this thing)

	local Hook = JBP.Classes.IconDataProcessorHook:New("UNIT_DOGTAGUNIT", "UNIT")

	Hook:DeclareUpValue("DogTag", DogTag)
	Hook:DeclareUpValue("JBP_UNITS", JBP.UNITS)
