...
local ObjectModule = JBP:NewClass("ObjectModule")
...
function ObjectModule:Enable()
	...
	if not self.IsEnabled then
		...
		if self.class.NumberEnabled == 1 and self.class.OnUsed then
			JBP.safecall(self.class.OnUsed, self.class)
		end
		
		if self.OnEnable then
			JBP.safecall(self.OnEnable, self)
		end
	end
end
...
local function OnDisableDelayedHandler()
	...
	for i, module in ipairs(queuedDisableDelayed) do
		if not module.IsEnabled then
			JBP.safecall(module.OnDisableDelayed, module)
		end
	end
...
function ObjectModule:Disable()
	...
	if self.IsEnabled then
		...
		if self.class.NumberEnabled == 0 and self.class.OnUnused then
			JBP.safecall(self.class.OnUnused, self.class)
		end
		
		if self.OnDisable then
			JBP.safecall(self.OnDisable, self)
		end
...
function ObjectModule:SetScriptHandler(script, func)
	...
	JBP:ValidateType(2, "Module:SetScriptHandler()", script, "string")
...
-- @see http://wow.curseforge.com/addons/justbecomepro/pages/api/icon-views/api-documentation/#w-icon-view-implements-module-module-name-order-implementor
function ObjectModule:SetImplementorForView(viewName, order, implementorFunc)
	self:AssertSelfIsClass()
	
	local IconView = JBP.Views[viewName]
	local moduleName = self.className
	
	if IconView then
		IconView:ImplementsModule(moduleName, order, implementorFunc)
	else
		JBP:RegisterCallback("JBP_VIEW_REGISTERED", function(event, IconView)
