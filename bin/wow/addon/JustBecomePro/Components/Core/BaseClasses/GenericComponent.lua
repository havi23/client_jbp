...
local GenericComponent = JBP:NewClass("GenericComponent")
...
function GenericComponent:RegisterConfigPanel(panelInfo)
	JBP:ValidateType("2 (panelInfo)", "RegisterConfigPanel", panelInfo, "ConfigPanelInfo")
...
-- @param xmlTemplateName [string] The name of an XML template. This template must inherit from the XML template {{{JustBecomePro_OptionsModuleContainer}}}. The frame will be created once it is needed, and it will be named after the template it is based on (the string passed as this param). Some XML templates for commonly used settings are built into JBP, like {{{JustBecomePro_ChooseName}}} and {{{JustBecomePro_IconStates}}}.
-- @param supplementalData [.*] Any data that will be associated with the created frame when it is used for this specific {{{JustBecomePro.Classes.GenericComponent}}} implementation. This data can be accessed through the 2nd arg of event {{{JBP_CONFIG_PANEL_SETUP}}} as can be seen in the usage example below.
-- @usage
--  -- Registering an XML Template config panel:
--  Type:RegisterConfigPanel_XMLTemplate(100, "JustBecomePro_ConfigPanel_Example", {
...
function GenericComponent:RegisterConfigPanel_XMLTemplate(order, xmlTemplateName, supplementalData)
	...
	local panelInfo = JBP.C.XmlConfigPanelInfo:New(order, xmlTemplateName, supplementalData)
...
-- @param func [function] A function that will be called immediately after the frame for this panel has been constructed. The panel inherits from XML template {{{JustBecomePro_OptionsModuleContainer}}}, and will be passed as the first arg to this function.
-- @param supplementalData [.*] Any data that will be associated with the created frame when it is used for this specific {{{JustBecomePro.Classes.GenericComponent}}} implementation. This data can be accessed through the 2nd arg of event {{{JBP_CONFIG_PANEL_SETUP}}} as can be seen in the usage example below. Using {{{supplementalData}}} with {{{:RegisterConfigPanel_ConstructorFunc}}} is highly impractical since panels created using this method cannot be shared with other [[api/base-classes/generic-component/|GenericComponent]], and the purpose of {{{supplementalData}}} is to encourage the reusability of frames.
-- @usage
--  -- Taken from the example at api/icon-type/how-to-use/
--  Type:RegisterConfigPanel_ConstructorFunc(150, "JustBecomePro_TestTypeSettings", function(self)
...
function GenericComponent:RegisterConfigPanel_ConstructorFunc(order, frameName, func, supplementalData)
	local panelInfo = JBP.C.LuaConfigPanelInfo:New(order, frameName, func, supplementalData)
