...
local IconComponent = JBP:NewClass("IconComponent", "GenericComponent")
...
--- Register some icon event handler data with a {{{JBP.Classes.EventHandler}}}. This event handler data will only be avaialable to a {{{JBP.Classes.EventHandler}}} when the [[api/icon/api-documentation/|Icon]] for which an event is being handled implements this [[api/base-classes/icon-component/|IconComponent]].
-- @param identifier [string] name of an instance of {{{JBP.Classes.EventHandler}}} that the data should be registered and validated with. The instance does not have to exist before this method is called.
-- @param ... [...] Data as required by the {{{JBP.Classes.EventHandler}}} that it is being registered with. See the documentation of the {{{OnRegisterEventHandlerDataTable}}} method of individual {{{JBP.Classes.EventHandler}}} instances for more information.
-- @usage
...
function IconComponent:RegisterEventHandlerData(identifier, ...)
	local EventHandler = JBP.EVENTS:GetEventHandler(identifier)
	...
	if EventHandler then
		...
		local data = {...}
		JBP:RegisterSelfDestructingCallback("JBP_CLASS_EventHandler_INSTANCE_NEW", function(event, class, EventHandler)
...
--- Register a set of icon defaults with an [[api/base-classes/icon-component/|IconComponent]]. These defaults will be merged into {{{JBP.Icon_Defaults}}} and the value of their settings for each icon will be avaialable as {{{JBP.Classes.Icon[settingKey]}}} when this [[api/base-classes/icon-component/|IconComponent]] has been implemented into a [[api/icon/api-documentation/|Icon]].
-- @param defaults [table] A table of default settings that will be merged into {{{JBP.Icon_Defaults}}}.
-- @usage -- Taken from JBP.Types.meta:
...
function IconComponent:RegisterIconDefaults(defaults)
	JBP:ValidateType("2 (defaults)", "IconComponent:RegisterIconDefaults(defaults)", defaults, "table")
	
	if not JBP.InitializedDatabase then
		-- Copy the defaults into the main defaults table.
		JBP:MergeDefaultsTables(defaults, JBP.Icon_Defaults)
	else
		JBP:Error("Defaults for component %q (%q) are being registered too late. They need to be registered before the database is initialized.", self.name, self.className)
	end
	
	
	-- Copy the defaults into defaults for this component. Used to implement relevant settings.
	JBP:MergeDefaultsTables(defaults, self.IconSettingDefaults)
end
...
-- @param event [string] The identifier of the event that will be used across JBP to refer to the event being registered.
...
function IconComponent:RegisterIconEvent(order, event, eventData)
	JBP:ValidateType("2 (order)", "IconComponent:RegisterIconEvent()", order, "number")
	JBP:ValidateType("3 (event)", "IconComponent:RegisterIconEvent()", event, "string")
	JBP:ValidateType("4 (eventData)", "IconComponent:RegisterIconEvent()", eventData, "table")
	
	JBP:ValidateType("event", "eventData", eventData.event, "nil")
	JBP:ValidateType("order", "eventData", eventData.order, "nil")
	
	JBP:ValidateType("text", "IconComponent:RegisterIconEvent() arg4 (eventData)", eventData.text, "string")
	JBP:ValidateType("desc", "IconComponent:RegisterIconEvent() arg4 (eventData)", eventData.desc, "string")
	JBP:ValidateType("settings", "IconComponent:RegisterIconEvent() arg4 (eventData)", eventData.settings, "table;nil")
	JBP:ValidateType("valueName", "IconComponent:RegisterIconEvent() arg4 (eventData)", eventData.valueName, "string;nil")
	JBP:ValidateType("valueSuffix", "IconComponent:RegisterIconEvent() arg4 (eventData)", eventData.valueSuffix, "string;nil")
	JBP:ValidateType("category", "IconComponent:RegisterIconEvent() arg4 (eventData)", eventData.category, "string;nil")
	JBP:ValidateType("conditionChecker", "IconComponent:RegisterIconEvent() arg4 (eventData)", eventData.conditionChecker, "function;nil")
	
	eventData.event = event
	eventData.order = order
	
	if JBP.EventList[event] then
		error(("An event with the event identifier %q already exists!"):format(event), 2)
	end
	
	JBP.EventList[#JBP.EventList + 1] = eventData
	JBP.EventList[event] = eventData
	...
	if not icon.ComponentsLookup[self] then
		...
		for setting in pairs(self.IconSettingDefaults) do
			if icon[setting] ~= nil and type(ics[setting]) ~= "table" then
				JBP:Error("Possible setting conflict detected! Setting %q, with value %q, trying to be implemented by %q, already exists as %q", setting, tostring(ics[setting]), self.name or self.className or "??", tostring(icon[setting]))
			end
		...
		if self.OnImplementIntoIcon then
			JBP.safecall(self.OnImplementIntoIcon, self, icon)
		end
	end
end
