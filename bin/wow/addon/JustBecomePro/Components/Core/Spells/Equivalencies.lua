...
local addonVersion = tonumber(GetAddOnMetadata("JustBecomePro", "X-Interface"))
...
JBP.BE.buffs.DefensiveBuffs	= CopyTable(JBP.BE.buffs.DefensiveBuffsSingle)
for k, v in pairs(JBP.BE.buffs.DefensiveBuffsAOE) do
	tinsert(JBP.BE.buffs.DefensiveBuffs, v)
end
...
local function ProcessEquivalencies()
	JBP.EquivOriginalLookup = {}
	JBP.EquivFullIDLookup = {}
	JBP.EquivFullNameLookup = {}
	JBP.EquivFirstIDLookup = {}

	JBP:Fire("JBP_EQUIVS_PROCESSING")
	JBP:UnregisterAllCallbacks("JBP_EQUIVS_PROCESSING")

	for dispeltype, texture in pairs(JBP.DS) do
		JBP.EquivFirstIDLookup[dispeltype] = texture
		JBP.SpellTexturesMetaIndex[strlower(dispeltype)] = texture
	end

	for category, b in pairs(JBP.BE) do
		for equiv, tbl in pairs(b) do
			JBP.EquivOriginalLookup[equiv] = CopyTable(tbl)
			JBP.EquivFirstIDLookup[equiv] = abs(tbl[1])
			JBP.EquivFullIDLookup[equiv] = ""
			JBP.EquivFullNameLookup[equiv] = ""
			...
			for i, spellID in pairs(tbl) do
				...
				JBP.EquivFullIDLookup[equiv] = JBP.EquivFullIDLookup[equiv] .. ";" .. realSpellID
				JBP.EquivFullNameLookup[equiv] = JBP.EquivFullNameLookup[equiv] .. ";" .. (name or realSpellID)
				...
				if spellID < 0 then
					...
					if name then
						...
						JBP.SpellTexturesMetaIndex[realSpellID] = tex
						JBP.SpellTexturesMetaIndex[JBP.strlowerCache[name]] = tex

						tbl[i] = name
					else

						if clientVersion >= addonVersion then -- only warn for newer clients using older versions
							JBP:Debug("Invalid spellID found: %s (%s - %s)!",
...
JBP:RegisterCallback("JBP_INITIALIZE", ProcessEquivalencies)
