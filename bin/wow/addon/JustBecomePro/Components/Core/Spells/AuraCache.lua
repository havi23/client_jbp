...
local AuraCache = JBP:NewModule("AuraCache", "AceEvent-3.0")
...
function AuraCache:GetCache()
	...
	elseif not OptionsAreLoaded then
		error("AuraCache is only designed to work when JBP_Options is loaded (or AuraCache did not initialize properly).", 2)
	end
...
JBP:RegisterCallback("JBP_DB_INITIALIZED", function()
	...
	JustBecomeProDB.AuraCache = nil
	
	Cache = JBP.db.global.AuraCache
...
JBP:RegisterCallback("JBP_OPTIONS_LOADED", function()
	...
	local Cache_OptDB = JBP.IE.db.global.AuraCache

	if Cache == JBP.db.global.AuraCache then
	...
	if JBP.IE.db.global.XPac_AuraCache < XPac then
		wipe(Cache_OptDB)
		JBP.IE.db.global.XPac_AuraCache = XPac
	end
