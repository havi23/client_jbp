...
local SpellCache = JBP:NewModule("SpellCache", "AceEvent-3.0", "AceTimer-3.0")
...
JBP.IE:RegisterUpgrade(71016, {
	global = function(self)
		JBP.IE.db.global.SpellCache = nil
		JBP.IE.db.global.CacheLength = nil
		JBP.IE.db.global.IncompleteCache = nil
		JBP.IE.db.global.WoWVersion = nil
	end,
})
...
JBP.IE:RegisterUpgrade(84201, {
	locale = function(self, locale)
		locale.SpellCacheWoWVersion = 0
	end,
})

JBP.IE:RegisterUpgrade(84405, {
	-- I managed to make the spell cache fast enough that its no longer worth persisting to disk.
	-- We're going to recreate it on each load of JBP_Options.
	-- This eliminates the significant factor in the load time of JBP_Options, and will also increase logout speeds.
	locale = function(self, locale)
		JBP.IE.db.locale.SpellCache = nil
		JBP.IE.db.locale.IncompleteSpellCache = nil
	end,
})
...
function SpellCache:SetNumCachePerFrame(num)
	JBP:ValidateType(2, "SpellCache:SetNumCachePerFrame()", num, "number")
	
	if NumCachePerFrame ~= num then
		NumCachePerFrame = num
		JBP:Fire("JBP_SPELLCACHE_NUMCACHEPERFRAME_CHANGED", num)
	end
end
...
function SpellCache:GetExpectedCacheLength()
	return JBP.IE.db.locale.SpellCacheLength
end
...
JBP:RegisterCallback("JBP_OPTIONS_LOADED", function()
	...
	local SpellCacheInvalidRanges = JBP.IE.db.locale.SpellCacheInvalidRanges

	local haveSpellCacheInvalidRanges = true
	if JBP.IE.db.locale.SpellCacheWoWVersion ~= clientBuild then
		wipe(SpellCacheInvalidRanges)
		haveSpellCacheInvalidRanges = false
	end

	JBP:Fire("JBP_SPELLCACHE_EXPECTEDCACHELENGTH_UPDATED", JBP.IE.db.locale.SpellCacheLength)
	...
	local function SpellCacher()
		...
		JBP:Fire("JBP_SPELLCACHE_NUMCACHED_CHANGED", spellID)
		if spellID > JBP.IE.db.locale.SpellCacheLength then
			JBP.IE.db.locale.SpellCacheLength = spellID
			JBP:Fire("JBP_SPELLCACHE_EXPECTEDCACHELENGTH_UPDATED", JBP.IE.db.locale.SpellCacheLength)
		end
	end

	IsCaching = true
	JBP:Fire("JBP_SPELLCACHE_STARTED")
	...
	f:SetScript("OnUpdate", function()
		...
		local success = JBP.safecall(SpellCacher)
		if success and spellsFailed < MAX_FAILED_SPELLS then
			...
			if success then
				-- We didn't error.
				JBP.IE.db.locale.SpellCacheWoWVersion = clientBuild
			end
			...
			IsCaching = nil
			JBP:Fire("JBP_SPELLCACHE_COMPLETED")
		end
