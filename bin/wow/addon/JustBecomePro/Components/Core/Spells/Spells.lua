...
-- These functions are as old as JBP itself (except for duration stuff). They have changed much over the years,
...
local function splitSpellAndDuration(str)
	...
	if not duration then
		...
		duration = tonumber( JBP.toSeconds(duration:trim(" :;.")) )
	end
...
local function parseSpellsString(setting, doLower, keepDurations)
	...
	local buffNames = JBP:SplitNames(setting) -- Get a table of everything
	
	if doLower then
		buffNames = JBP:LowerNames(buffNames)
	end
	...
	local k = #buffNames
	while k > 0 do
		local eqtt = JBP:EquivToTable(buffNames[k]) -- Get the table form of the equivalency string
		if eqtt then
	...
	-- REMOVE DUPLICATES
	JBP.tRemoveDuplicates(buffNames)
	...
	while k > 0 do
		...
		if (type(v) == "string" and v:match("^%-")) or (type(v) == "number" and v < 0) then
			...
			if not removed then
				JBP:Printf(L["SPELL_EQUIV_REMOVE_FAILED"], thingToRemove, tconcat(buffNames, "; "))
			end
		else
	...
	for k = #buffNames, 1, -1 do
		...
		if (tonumber(spell) or 0) >= 2^31 or duration >= 2^31 then
			...
			tremove(buffNames, k)
			JBP:Warn(L["ERROR_INVALID_SPELLID2"]:format(v))
		end
	end
...
parseSpellsString = JBP:MakeNArgFunctionCached(3, parseSpellsString)
...
local function getSpellDurations(setting)
	...
	for k, buffName in pairs(NameArray) do
		...
		if not dur then
			...
			DurationArray[k] = tonumber( JBP.toSeconds(dur:trim(" :;.")) )
		end
	end
...
JBP:RegisterCallback("JBP_GLOBAL_UPDATE", function()
	-- We need to wipe the stored objects/strings on every JBP_GLOBAL_UPDATE because of issues with
	-- spells that replace other spells in different specs, like Corruption/Immolate.
	-- JBP_GLOBAL_UPDATE might fire a little too often for this, but it is a surefire way to prevent issues that should
	-- hold up through out all future game updates because JBP_GLOBAL_UPDATE should always react to any changes in spec/talents/etc

	for _, instance in pairs(JBP.C.SpellSet.instances) do
		instance:Wipe()
	end
end)


--- Returns an instance of {{{JBP.C.SpellSet}}} for the given spellString.
-- The following can be accessed as members of the {{{JBP.C.SpellSet}}}:
-- * 
...
-- will be parsed and made available in various forms by the {{{JBP.C.SpellSet}}}.
...
-- @return [JBP.C.SpellSet] An instance of {{{JBP.C.SpellSet}}} for the requested spells.
function JBP:GetSpells(spellString, allowRenaming)
	JBP:ValidateType("2 (spellString)", "JBP:GetSpells(spellString, allowRenaming)", spellString, "string;number")

	-- Make sure that allowRenaming is a boolean.
	allowRenaming = not not allowRenaming

	return JBP.C.SpellSet.instancesByName[allowRenaming][spellString] or JBP.C.SpellSet:New(spellString, allowRenaming)
end
...
-- The original case is saved so that it can be used by JBP:RestoreCase() to restore the capitalization of spells that have been lowered.
...
-- Numbers are returned immediately. Otherwise, the cache of lowered names from JBP:LowerNames() 
-- is checked, and if it isn't in there, JBP.strlowerCache is scanned for an original string.
function JBP:RestoreCase(str)
...
function JBP:EquivToTable(name)
	...
	-- Iterate over all of JBP.BE's sub-categories ('buffs', 'debuffs', 'casts', etc)
	for k, v in pairs(JBP.BE) do
...
JBP:MakeSingleArgFunctionCached(JBP, "EquivToTable")
...
elseif pclass == "DEATHKNIGHT" then
	local npcName = function(npcID)
		local cachedName = JBP:TryGetNPCName(npcID)
		return function()
			if cachedName then return cachedName end
			cachedName = JBP:TryGetNPCName(npcID)
			return cachedName
		end
	end
