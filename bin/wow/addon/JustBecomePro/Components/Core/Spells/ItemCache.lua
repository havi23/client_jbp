...
local ItemCache = JBP:NewModule("ItemCache", "AceEvent-3.0", "AceTimer-3.0")
...
JBP.IE:RegisterUpgrade(62217, {
	global = function(self)
		JBP.IE.db.global.ItemCache = nil
		JBP.IE.db.global.XPac_ItemCache = nil
	end,
})
...
JBP:RegisterCallback("JBP_OPTIONS_LOADED", function()
	...
	Cache = JBP.IE.db.locale.ItemCache
	CompiledCache = {}
	...
	if JBP.IE.db.locale.XPac_ItemCache < XPac then
		wipe(Cache)
		JBP.IE.db.locale.XPac_ItemCache = XPac
	end
	
	-- Delete old item caches.
	for _, locale in pairs(JBP.IE.db.sv.locale) do
