...
local IconDataProcessor = JBP:NewClass("IconDataProcessor", "IconDataProcessorComponent")
...
function IconDataProcessor:OnNewInstance(name, attributes)
	JBP:ValidateType("2 (name)", "IconDataProcessor:New(name, attributes)", name, "string")
	JBP:ValidateType("3 (attributes)", "IconDataProcessor:New(name, attributes)", attributes, "string")
	...
	for _, attribute in JBP:Vararg(strsplit(",", self.attributesStringNoSpaces)) do
	...
	self.changedEvent = "JBP_ICON_DATA_CHANGED_" .. name
	
	JBP.Classes.Icon:ClearSetInfoFunctionCache()
end
...
--- Wrapper method around JBP.IconStateArbitrator:AddHandler().
function IconDataProcessor:RegisterAsStateArbitrator(...)
	JBP.IconStateArbitrator:AddHandler(self, ...)
end
...
-- * A reference, by name, to every IconDataProcessor (E.g. {{{DURATION = JBP.Classes.IconDataProcessor.ProcessorsByName.DURATION}}})
...
IconDataProcessor:DeclareUpValue("JBP", JBP)
