...
local GetCurrentSpecializationRole = JBP.GetCurrentSpecializationRole
...
local Group = JBP:NewClass("Group", "Frame", "UpdateTableManager", "GenericModuleImplementor")
Group:UpdateTable_Set(JBP.GroupsToUpdate)


do	-- JBP.CNDT implementation
	local tab
	
	JBP.CNDT:RegisterConditionSetImplementingClass("Group")
	JBP.CNDT:RegisterConditionSet("Group", {
		parentSettingType = "group",
		parentDefaults = JBP.Group_Defaults,
		
		settingKey = "Conditions",
		GetSettings = function(self)
			if JBP.CI.gs then
				return JBP.CI.gs.Conditions
			end
		end,
		
		iterFunc = JBP.InGroupSettings,
		iterArgs = {
			[1] = JBP,
		},
	...
	JBP:RegisterCallback("JBP_OPTIONS_LOADED", function()
		tab = JBP.IE:RegisterTab("GROUP", "CNDTGROUP", "Conditions", 15)
		tab:SetText(L["GROUPCONDITIONS"])
		tab:SetHistorySet(JBP.C.HistorySet:GetHistorySet("GROUP"))
		
		tab:HookScript("OnClick", function()
			JBP.CNDT:LoadConfig("Group")
		end)

		JBP.IconDragger:RegisterIconDragHandler(320, -- Copy Conditions
			function(IconDragger, info)
				...
				if IconDragger.desticon and IconDragger.desticon.group ~= IconDragger.srcicon.group and n > 0 then
					if IconDragger.desticon.group:GetSettings().Conditions.n > 0 then
						info.text = L["ICONMENU_COPYCONDITIONS_GROUP"]:format(n) .. " |TInterface\\AddOns\\JustBecomePro\\Textures\\Alert:0:2|t"
						...
						.. "\r\n\r\n|cFFFF5959|TInterface\\AddOns\\JustBecomePro\\Textures\\Alert:0:2|t"
			...
			function(IconDragger)
				...
				IconDragger.desticon.group:GetSettings().Conditions = JBP:CopyWithMetatable(srcgs.Conditions)
			end
		)
	end)
end
...
JBP:RegisterCallback("JBP_GLOBAL_UPDATE_POST", Group, "UpdateTable_PerformAutoSort")
...
function Group.Show(group)
	if not group.__shown then
		JBP:Fire("JBP_GROUP_SHOW_PRE", group)
		group:Show_Blizz()
		group.__shown = 1
		JBP:Fire("JBP_GROUP_SHOW_POST", group)
	end
end
...
function Group.Hide(group)
	if group.__shown then
		JBP:Fire("JBP_GROUP_HIDE_PRE", group)
		group:Hide_Blizz()
		group.__shown = nil
		JBP:Fire("JBP_GROUP_HIDE_POST", group)
	end
end
...
function Group.Update(group)
	...
	elseif JBP.Locked and group.OnlyInCombat and not UnitAffectingCombat("player") then
		allConditionsPassed = false
...
-- [INTERNAL]
function Group.JBP_CNDT_OBJ_PASSING_CHANGED(group, event, ConditionObject, failed)
...
function Group.InIconSettings(group)
	return JBP:InIconSettings(group.Domain, group.ID)
end
...
function Group.GetGUID(group)
	...
	if not GUID or GUID == "" then
		...
		if group:GetID() > JBP.db[group.Domain].NumGroups then
			-- If this group is out of range, don't generate a GUID for it
			-- and don't report a GUID for it.
			return nil
		end

		GUID = JBP:GenerateGUID("group", JBP.CONST.GUID_SIZE)
...
-- @paramsig
-- @return [{{{JBP.Group_Defaults}}}] The settings table that holds the settings for the group.
...
function Group.GetSettings(group)
	return JBP.db[group.Domain].Groups[group:GetID()]
end
...
-- @return [{{{JBP.Group_Defaults.SettingsPerView[view]}}}] The settings table that holds the view-specific settings for the group.
...
function Group.ShouldUpdateIcons(group)
	...
	if	(group:GetID() > JBP.db[group.Domain].NumGroups)
...
function Group.IsEnabled(group)
	if group.Domain == "global" then
		return group.Enabled and group.EnabledProfiles[JBP.db:GetCurrentProfile()]
	else
...
function Group.Setup_Conditions(group)
	...
	JBP:UnregisterCallback("JBP_CNDT_OBJ_PASSING_CHANGED", group)
	
	-- Determine if we should process conditions
	if group:ShouldUpdateIcons() and JBP.Locked and group.Conditions_GetConstructor then
		...
		if group.ConditionObject then
			...
			JBP:RegisterCallback("JBP_CNDT_OBJ_PASSING_CHANGED", group)
		else
...
function Group.Setup(group, noIconSetup)
	...
	if GUID then
		JBP:DeclareDataOwner(GUID, group)
	end
	
	for k, v in pairs(JBP.Group_Defaults) do
		group[k] = gs[k]
	end
	...
	local viewData = JBP.Views[gs.View]
	group.viewData = viewData

	JBP:Fire("JBP_GROUP_SETUP_PRE", group)
	...
	if group:ShouldUpdateIcons() then
		if not noIconSetup then
			...
			for iconID = 1, group.numIcons do
				...
				if not icon then
					icon = JBP.Classes.Icon:New("Button", group:GetName() .. "_Icon" .. iconID, group, "JustBecomePro_IconTemplate", iconID)
				end
	...
	JBP:Fire("JBP_GROUP_SETUP_POST", group)
end
