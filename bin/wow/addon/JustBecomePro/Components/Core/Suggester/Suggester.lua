...
local ClassSpellCache = JBP:GetModule("ClassSpellCache")
local AuraCache = JBP:GetModule("AuraCache")
local SpellCache = JBP:GetModule("SpellCache")
local ItemCache = JBP:GetModule("ItemCache")

local SUG = JBP:NewModule("Suggester", "AceEvent-3.0", "AceComm-3.0", "AceSerializer-3.0", "AceTimer-3.0")
JBP.SUG = SUG


local DEBOUNCE_TIMER = 0.05

JBP.IE:RegisterUpgrade(62217, {
	global = function(self)
		-- These are both old and unused. Kill them.
		JBP.IE.db.global.CastCache = nil
		JBP.IE.db.global.ClassSpellCache = nil
	end,
})
...
JBP:RegisterCallback("JBP_CONFIG_ICON_TYPE_CHANGED", function(event, icon)
	if icon == JBP.CI.icon then
		SUG.redoIfSame = 1
		SUG.SuggestionList:Hide()
	end
end)

function SUG:JBP_SPELLCACHE_STARTED()
	SUG.SuggestionList.Caching:Show()
end
JBP:RegisterCallback("JBP_SPELLCACHE_STARTED", SUG)

function SUG:JBP_SPELLCACHE_COMPLETED()
...
JBP:RegisterCallback("JBP_SPELLCACHE_COMPLETED", SUG)
...
local function progressCallback(countdown)
	-- This is called for each step of JBP.shellSortDeferred.
...
function SUG:SuggestingComplete(doSort)
	...
	if doSort and not SUG.CurrentModule.dontSort then
		...
		if sorterBucket then
			...
			for k, bucket in JBP:OrderedPairs(buckets) do
				if next(bucket) then
					-- Sort the bucket.
			...
			JBP.shellsortDeferred(SUGpreTable, sorter, nil, SUG.SuggestingComplete, SUG, progressCallback)
			return
		end
	...
	if suggestedForModule ~= SUG.CurrentModule then
		JBP:Debug("SUG module changed mid-suggestion")
		return
	end
	...
	local numFramesNeeded = JBP.SUG:GetNumFramesNeeded()
	...
	if SUG.mousedOver then
		JBP:TT_Update(SUG.mousedOver)
	else
		-- Otherwise, show the tooltip of the current tab index
		local f = SUG[SUG.tabIndex]
		if f and f:IsVisible() then
			f:GetScript("OnEnter")(f)
			JBP.SUG.mousedOver = nil -- this gets set on the OnEnter, but it isn't correct.
		end
	end
end
...
function SUG:NameOnCursor(isClick)
	...
	SUG.lastName = strlower(JBP:CleanString(strsub(text, SUG.startpos, SUG.endpos)))
...
do	-- KeyManger
	...
	function KeyManager:HandlePress(key)
		...
		elseif key == "DOWN" then
			if JBP.SUG[SUG.tabIndex + 1] and JBP.SUG[SUG.tabIndex + 1]:IsVisible() then
				SUG.tabIndex = SUG.tabIndex + 1
			else
				SUG.offset = SUG.offset + 1
			end
	...
	KeyManager:SetScript("OnKeyDown", function(self, key)
		if SUG.SuggestionList:IsVisible() and (key == "UP" or key == "DOWN") then
			...
			self.down = {key = key, start = JBP.time}
	...
	KeyManager:SetScript("OnUpdate", function(self, key)
		...
		if (not data.last and data.start + 0.5 < JBP.time) or (data.last and data.last + repeatRate < JBP.time) then
			self:HandlePress(data.key)
			data.last = (data.last or JBP.time) + repeatRate
		end
	end)
end
...
local EditBoxHooks = {
	...
	OnEditFocusGained = function(self)
		if self.SUG_Enabled then
			...
			SUG.SuggestionList:SetParent(self.SUG_parent or JustBecomePro_IconEditor)
			SUG:NameOnCursor()
		end
	end,
...
-- @param parent [Frame|nil] A frame to reparent the suggestion list to when active. Defaults to JustBecomePro_IconEditor
function SUG:EnableEditBox(editbox, inputType, onlyOneEntry, inline, parent)
	editbox.SUG_Enabled = 1

	inputType = JBP.get(inputType)
...
function SUG:GetNumFramesNeeded()
	...
	return floor((JBP.SUG.SuggestionList:GetHeight() + 5)/JBP.SUG[1]:GetHeight()) - (self.inline and 1 or 2)
end

function SUG:GetHeightForFrames(numFrames)
	return (numFrames * JBP.SUG[1]:GetHeight()) + 6
end

function SUG:SetInline(inline)
	local firstItem = JBP.SUG:GetFrame(1)
	self.inline = inline
	...
	if inline then
		...
		firstItem:SetPoint("TOP", 0, -6 - JBP.SUG[1]:GetHeight())

		List:SetScale(1)
		List:ClearAllPoints()
		List:SetPoint("TOPLEFT", JBP.IE, "TOPRIGHT", 1, 0)
		List:SetPoint("BOTTOMLEFT", JBP.IE, "BOTTOMRIGHT", 1, 0)
		--List:SetParent(JBP.IE)
...
function SUG:GetFrame(id)
	local Suggest = JBP.SUG.SuggestionList
	local f = JBP.SUG[id]
	
	if not f then
		f = CreateFrame("Button", Suggest:GetName().."Item"..id, Suggest, "JustBecomePro_SpellSuggestTemplate", id)
		JBP.SUG[id] = f
		
		if JBP.SUG[id-1] then
			f:SetPoint("TOPRIGHT", JBP.SUG[id-1], "BOTTOMRIGHT", 0, 0)
			f:SetPoint("TOPLEFT", JBP.SUG[id-1], "BOTTOMLEFT", 0, 0)
		end
	end
...
local StartsWithCache = JBP:NewClass("StartsWithCache") {
...
local InitialismCache = JBP:NewClass("InitialismCache") {
		...
		for equiv in pairs(tbl) do
			...
			elseif long then
				if SUGIsNumberInput then
					for _, id in pairs(JBP:SplitNamesCached(JBP.EquivFullIDLookup[equiv])) do
					...
					for _, name in pairs(JBP:SplitNamesCached(JBP.EquivFullNameLookup[equiv])) do
...
function Module:Entry_Insert(insert)
	if insert then
		...
		if SUG.Box.SUG_onlyOneEntry then
			SUG.Box:SetText(JBP:CleanString(insert))
			SUG.Box:ClearFocus()
			return
		end
		...
		-- clean it up
		SUG.Box:SetText(JBP:CleanString(newtext))
...
function Module:Table_Get()
	return JBP:GetModule("ItemCache"):GetCache()
end
function Module:Table_GetSpecialSuggestions_1(suggestions)
	local id = tonumber(SUG.lastName)

	if id and GetItemInfo(id) and not JBP.tContains(suggestions, id) then
...
function Module:OnSuggest()
	...
	EquivFirstIDLookup = JBP.EquivFirstIDLookup
end
function Module:Table_Get()
...
function Module:Entry_AddToList_1(f, id)
	if tonumber(id) then --sanity check
		...
		f.tooltipmethod = "JBP_SetSpellByIDWithClassIcon"
		f.tooltiparg = id

		if JBP.EquivFirstIDLookup[name] then
			-- Things that conflict with equivalencies should only be inserted as IDs
			f.insert = id
			f.insert2 = name
			f.overrideInsertName = JBP.L["SUG_INSERTNAME_INTERFERE"]
		else
			f.insert = SUG.inputType == "number" and id or name
			f.insert2 = SUG.inputType ~= "number" and id or name
		end
...
function Module:Entry_OnClick(f, button)
	...
	local Parser, LT1, LT2, LT3, RT1, RT2, RT3 = JBP:GetParser()
	Parser:SetOwner(UIParent, "ANCHOR_NONE")
	Parser:SetSpellByID(spellID)

	local dur

	for _, text in JBP:Vararg(RT2:GetText(), RT3:GetText()) do
		if text then
...
function Module:Entry_IsValid(id)
	...
	local Parser, LT1, LT2, LT3 = JBP:GetParser()
...
function Module:Table_Get()
	return SpellCache:GetCache(), JBP.BE.buffs, JBP.BE.debuffs
end
function Module:Entry_Colorize_2(f, id)
	if JBP.DS[id] then
		f.Background:SetVertexColor(1, .49, .04, 1) -- druid orange
	elseif JBP.BE.buffs[id] then
		f.Background:SetVertexColor(.2, .9, .2, 1) -- lightish green
	elseif JBP.BE.debuffs[id] then
		f.Background:SetVertexColor(.77, .12, .23, 1) -- deathknight red
	end
end
function Module:Entry_AddToList_2(f, id)
	if JBP.DS[id] then -- if the entry is a dispel type (magic, poison, etc)
		local dispeltype = id
		...
		f.Icon:SetTexture(JBP.DS[id])

	elseif JBP.EquivFirstIDLookup[id] then -- if the entry is an equivalacy (buff, cast, or whatever)
...
function Module:Table_GetSpecialSuggestions_1(suggestions)
	...
	for dispeltype in pairs(JBP.DS) do
...
Module.Table_GetSpecialSuggestions_1 = JBP.NULLFUNC
