...
--- The fields avaiable to instances of JBP.Classes.IconType. JBP.Classes.IconType inherits from JBP.Classes.IconComponent.
-- @class table
-- @name JBP.Classes.IconType
...
local IconType = JBP:NewClass("IconType", "IconComponent")
...
-- @return [JBP.Classes.IconType] A new IconType instance.
-- @usage IconType = JBP.Classes.IconType:New("cooldown")
...
--- Performs JBP.Classes.Icon:Setup() on all icons that use this icon type.
...
-- @param icon [JBP.Classes.Icon] The icon that the spell is being formatted for.
...
-- @usage -- This method should only be called internally by JustBecomePro and some of its components.
-- texture = JBP.Types.cooldown:FormatSpellForOutput(icon, icon.attributes.spell, true)
...
-- [REQUIRED, FALLBACK]
--- Attempts to figure out what the configuration texture of an icon will be without actually creating the icon. This is a base method written for handling spells. It should be overridden for IconTypes that don't take spell input for their ics.Name setting. It is acceptable to delay the declaration of overrides of this method until after JustBecomePro_Options has loaded if needed.
-- @param ics [JBP.Icon_Defaults] The settings of the icon that the texture is being guessed for.
-- @return [string] The guessed texture of the icon. 
-- @usage -- This method should only be called internally by JustBecomePro and some of its components. 
-- texture = JBP.Types[ics.Type]:GuessIconTexture(ics)
function IconType:GuessIconTexture(ics)
	self:AssertSelfIsInstance()
	
	if ics.Name and ics.Name ~= "" then

		local name = JBP:GetSpells(ics.Name).First
		if name then
...
-- @param icon [JBP.Classes.Icon] The icon to get the config mode texture for.
...
function IconType:GetConfigIconTexture(icon)
	if icon.Name == "" and not self.AllowNoName then
		...
		if icon.Name ~= "" then
			...
			local tbl = JBP:GetSpells(icon.Name).Array
...
-- @paramsig icon, ...
-- @param icon [JBP.Classes.Icon] The icon that the dragged data was released onto.
-- @param ... [...] The return values from [[http://wowprogramming.com/docs/api/GetCursorInfo|GetCursorInfo()]]. Don't call GetCursorInfo yourself in your definition, because JBP will pass in its own data in special cases that can't be obtained from GetCursorInfo.
...
function IconType:DragReceived(icon, t, data, subType, param4)
	...
	ics.Name = JBP:CleanString(ics.Name .. ";" .. input)
	return true -- signal success
end

-- [REQUIRED, FALLBACK]
--- Returns brief information about what an icon is configured to track. Used mainly in import/export menus. This is a default method, and may be overridden if it does not provide the desired functionality for an IconType. It is acceptable to delay the declaration of overrides of this method until after JustBecomePro_Options has loaded if needed.
-- @param ics [JBP.Icon_Defaults] The settings of the icon that information is being requested about.
...
--- Register the IconType for use in JustBecomePro. IconTypes cannot be used or accessed until this method is called. Should be the very last line of code in the file that defines an IconType.
-- @param order [number] The order of this IconType relative to other IconTypes in configuration UI.
-- @return self [JBP.Classes.IconType] The IconType this method was called on.
-- @usage IconType:Register(10)
function IconType:Register(order)
	self:AssertSelfIsInstance()
	
	JBP:ValidateType("IconType.name", "IconType:Register(order)", self.name, "function;string")
	JBP:ValidateType("IconType.desc", "IconType:Register(order)", self.desc, "function;string;nil")
	JBP:ValidateType("IconType.tooltipTitle", "IconType:Register(order)", self.tooltipTitle, "function;string;nil")
	JBP:ValidateType("IconType.menuIcon", "IconType:Register(order)", self.menuIcon, "function;string;number;nil")
	JBP:ValidateType("IconType.menuSpaceBefore", "IconType:Register(order)", self.menuSpaceBefore, "boolean;nil")
	JBP:ValidateType("IconType.menuSpaceAfter", "IconType:Register(order)", self.menuSpaceAfter, "boolean;nil")
	JBP:ValidateType("IconType.hidden", "IconType:Register(order)", self.menuSpaceAfter, "function;boolean;nil")
	
	JBP:ValidateType("2 (order)", "IconType:Register(order)", order, "number")
	...
	if JBP.debug and rawget(JBP.Types, typekey) then
	...
	JBP.Types[typekey] = self -- put it in the main Types table
	tinsert(JBP.OrderedTypes, self) -- put it in the ordered table (used to order the type selection dropdown in the icon editor)
	JBP:SortOrderedTables(JBP.OrderedTypes)
	...
	JBP:RegisterCallback("JBP_CLASS_IconDataProcessor_INSTANCE_NEW", self, "UpdateUsedProcessors")
...
-- @param attributesString [string] The attributesString whose usage state is being set. (an attributesString is passed as a segment of the first arg to {{{icon:SetInfo(attributesStrings, ...)}}}, and also as the second arg to the constructor of a JBP.Classes.IconDataProcessor).
...
function IconType:UsesAttributes(attributesString, uses)
	...
	JBP:ValidateType("3 (uses)", "IconView:Register(attributesString, uses)", uses, "boolean;nil")
	...
	for Processor in pairs(self.UsedProcessors) do
		for _, ProcessorHook in ipairs(Processor.hooks) do
			...
			for processorRequirementName in pairs(ProcessorHook.processorRequirements) do
				...
				local Processor = JBP.Classes.IconDataProcessor.ProcessorsByName[processorRequirementName]
...
function IconType:SetModuleAllowance(moduleName, allow)
	local IconModule = JBP.Classes[moduleName]
	...
	elseif not IconModule then
		JBP:RegisterSelfDestructingCallback("JBP_CLASS_NEW", function(event, class)
...
function IconType:SetAllowanceForView(viewName, allow)
	...
	JBP:ValidateType(2, "IconType:SetAllowanceForView()", viewName, "string")
	
	-- allow cannot be nil
	JBP:ValidateType(3, "IconType:SetAllowanceForView()", allow, "boolean")
	
	if self.ViewAllowances[viewName] == nil then
		self.ViewAllowances[viewName] = allow
	else
		JBP:Error("An icon type's view allowance cannot be set once it has already been declared by either a view or an icon type.")
	end
end
...
-- 
-- JBP.Types.resouce:SetDefaultAllowanceForTypes(false)
-- 
...
function IconType:SetDefaultAllowanceForViews(allow)
	...
	JBP:ValidateType(2, "IconType:SetDefaultAllowanceForViews()", allow, "boolean")
...
-- @usage isAllowed = JBP.Types.resource:IsAllowedByType("icon")
...
IconType:RegisterConfigPanel_ConstructorFunc(1, "JustBecomePro_IsViewAllowed", function(self)
	...
	self:CScriptAdd("PanelSetup", function()
		if JBP.CI.icon.typeData:IsAllowedByView(JBP.CI.icon.group.View) then
			self:Hide()
		end
	end)
end)
...
-- @param icon [JBP.Classes.Icon] The icon that yielded the info.
-- @param iconToSet  [JBP.Classes.Icon] The icon that should recieve the yielded info.
-- @param ... [vararg] The parameters passed info icon:YieldInfo(hasInfo, ...).
function IconType:HandleYieldedInfo(icon, iconToSet, ...)
	JBP:Error("If your icon type %q calls icon:YieldInfo(hasInfo, ...), you must define an IconType:HandleYieldedInfo(icon, iconToSet, ...) method.", self.type)
end
