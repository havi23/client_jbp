...
--- The fields avaiable to instances of JBP.Classes.IconView. JBP.Classes.IconView Inherits JBP.Classes.GroupComponent and JBP.Classes.IconComponent.
-- @class table
-- @name JBP.Classes.IconView
...
local IconView = JBP:NewClass("IconView", "GroupComponent", "IconComponent")
...
-- @usage IconView = JBP.Classes.IconView:New("icon")
function IconView:OnNewInstance(view)
	self.view = view
	
	--JBP.Icon_Defaults.SettingsPerView[view] = {}
...
--- Register the IconView for use in JustBecomePro. IconViews cannot be used or accessed until this method is called. Should be the very last line of code in the file that defines an IconView.
-- @param order [number] The order of this IconView relative to other IconViews in configuration UI.
-- @return self [JBP.Classes.IconView] The IconView this method was called on.
-- @usage IconView:Register(10)
function IconView:Register(order)
	self:AssertSelfIsInstance()
	JBP:ValidateType("IconView.name", "IconView:Register(order)", self.name, "string")
	JBP:ValidateType("IconView.desc", "IconView:Register(order)", self.desc, "string")
	JBP:ValidateType("2 (order)", "IconView:Register(order)", order, "number")

	local viewkey = self.view
	
	self.order = order

	if JBP.debug and rawget(JBP.Views, viewkey) then
	...
	JBP.Views[viewkey] = self -- put it in the main Views table
	tinsert(JBP.OrderedViews, self)
	JBP:SortOrderedTables(JBP.OrderedViews)

	JBP:Fire("JBP_VIEW_REGISTERED", self)
...
function IconView:ImplementsModule(moduleName, order, implementorFunc)
	...
	JBP:ValidateType(2, "IconView:ImplementsModule()", moduleName, "string")
	JBP:ValidateType(3, "IconView:ImplementsModule()", order, "number")
	...
	JBP:SortOrderedTables(self.ModuleImplementors)
end
...
-- implements = JBP.Views.icon:DoesImplementModule("IconModule_CooldownSweep")
-- 
-- -- Check if a module is implemented by default to all IconViews (can be overridden on a per-IconView basis).
-- implements = JBP.Classes.IconView:DoesImplementModule("IconModule_CooldownSweep")
...
function IconView:SetTypeAllowance(typeName, allow)
	local IconType = rawget(JBP.Types, typeName)
	
	if IconType and IconType.SetViewAllowance then
		IconType:SetViewAllowance(self.view, allow)
		
	elseif not IconType then
		JBP:RegisterSelfDestructingCallback("JBP_CLASS_IconType_INTANCE_NEW", function(event, instance)
...
-- @param icon [JBP.Classes.Icon] The icon the IconView was just implemented into.
...
-- @param icon [JBP.Classes.Icon] The icon whose size is being queried.
...
-- @param group [JBP.Classes.Group] The group the IconView was just implemented into.
...
-- @param gs [JBP.Group_Defaults] The settings of the group that was just created. This method does not provide access to an actual JBP.Classes.Group since one will not have been created before this is called.
-- @usage
--	function IconView:Group_OnCreate(gs)
--		-- Switch the default number of rows with the number of columns. This is the default behavior for JBP.Views.bar
...
function IconView:OnImplementIntoIcon(icon)
	for i, implementationData in ipairs(self.ModuleImplementors) do
		...
		local ModuleClass = moduleName:find("IconModule") and JBP.Classes[moduleName]
...
function IconView:OnImplementIntoGroup(group)
	for i, implementationData in ipairs(self.ModuleImplementors) do
		...
		-- Get the class of the module that we might be implementing.
		local ModuleClass = moduleName:find("GroupModule") and JBP.Classes[moduleName]
