...
local HELP = JBP:NewModule("Help", "AceTimer-3.0")
JBP.HELP = HELP
...
function HELP:OnInitialize()
	self.Arrow = JustBecomePro_HelpFrame
	self.Frame = JustBecomePro_HelpFrame.body
...
function HELP:Show(help)
	...
	JBP:ValidateType(1, 					"JBP.HELP:Show(help)", help, "table")
	JBP:ValidateType("help.code", 			"JBP.HELP:Show(help)", help.code, "string")
	JBP:ValidateType("help.icon", 			"JBP.HELP:Show(help)", help.icon, "frame;nil")
	JBP:ValidateType("help.text", 			"JBP.HELP:Show(help)", help.text, "string")

	JBP:ValidateType("help.parent", 		"JBP.HELP:Show(help)", help.parent, "frame;nil")
	JBP:ValidateType("help.relativeTo", 	"JBP.HELP:Show(help)", help.relativeTo, "frame")
	JBP:ValidateType("help.relativePoint", 	"JBP.HELP:Show(help)", help.relativePoint, "string;nil")
	JBP:ValidateType("help.x", 				"JBP.HELP:Show(help)", help.x, "number;nil")
	JBP:ValidateType("help.y", 				"JBP.HELP:Show(help)", help.y, "number;nil")
	
	local code = help.code

	if not HELP:IsCodeRegistered(code) then
		if not help.codeOrder then
			JBP:Error("Code %q was not defined before calling HELP:Show(), and help.codeOrder was not set for auto-registration.", help.code)
			return
		else
			JBP:ValidateType("help.codeOrder", "JBP.HELP:Show(help)", help.codeOrder, "number")
			JBP:ValidateType("help.codeOnlyOnce", "JBP.HELP:Show(help)", help.codeOnlyOnce, "boolean;nil")
	...
	help.parent = help.parent or JBP.IE
	...
	if help.onlyOnceSetting and JBP.db.global.HelpSettings[help.onlyOnceSetting] then
		self.Queued[code] = nil
		help = nil
		return
	end
...
function HELP:Hide(code, userDidntAcknowledge)
	...
	elseif self:GetShown() == code then
		...
		if self.showingHelp.onlyOnceSetting and userDidntAcknowledge then
			JBP.db.global.HelpSettings[self.showingHelp.onlyOnceSetting] = false
		end
...
function HELP:NewCode(code, order, OnlyOnce)
	JBP:ValidateType(2, "HELP:NewCode(code, order, OnlyOnce)", code, "string")
...
function HELP:IsCodeRegistered(code)
	return JBP.tContains(HELP.Codes, code)
end
...
function HELP:ShowNext()
	...
	if help.onlyOnceSetting then
		JBP.db.global.HelpSettings[help.onlyOnceSetting] = true
	end
...
JBP:RegisterCallback("JBP_CONFIG_ICON_TYPE_CHANGED", function(event, icon)
	if JBP.CI.icon then
		HELP:HideForIcon(JBP.CI.icon)
	end
end)

JBP:RegisterCallback("JBP_CONFIG_ICON_LOADED_CHANGED", function(event, icon, icon_old)
	HELP:HideForIcon(icon_old)
end)

JBP:RegisterCallback("JBP_ICON_SETTINGS_RESET", function(event, icon)
	HELP:HideForIcon(icon)
end)	

JBP:RegisterCallback("JBP_CONFIG_TAB_CLICKED", function(event, tab, oldTab)
	HELP:ShowNext()
end)
