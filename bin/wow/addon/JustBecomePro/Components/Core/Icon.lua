...
JBP:RegisterCallback("JBP_GLOBAL_UPDATE", function()
	UPD_INTV = JBP.UPD_INTV
end)



local Icon = JBP:NewClass("Icon", "Button", "UpdateTableManager", "GenericModuleImplementor")
Icon:UpdateTable_Set(JBP.IconsToUpdate)
Icon.IsIcon = true
Icon.attributes = {}
Icon.runEvents = true
Icon.QueuedIcons = {}
...
do
	...
	JBP.CNDT:RegisterConditionSetImplementingClass("Icon")
	JBP.CNDT:RegisterConditionSet("Icon", {
		parentSettingType = "icon",
		parentDefaults = JBP.Icon_Defaults,
		
		settingKey = "Conditions",
		GetSettings = function(self)
			if JBP.CI.ics then
				return JBP.CI.ics.Conditions
			end
		end,
		
		iterFunc = JBP.InIconSettings,
		iterArgs = {
			[1] = JBP,
		},
	...
	JBP:RegisterCallback("JBP_OPTIONS_LOADED", function()
		tab = JBP.IE:RegisterTab("ICON", "CNDTICON", "Conditions", 5)
		tab:SetText(L["CONDITIONS"])
		tab:SetHistorySet(JBP.C.HistorySet:GetHistorySet("ICON"))
		
		tab:HookScript("OnClick", function()
			JBP.CNDT:LoadConfig("Icon")
		end)

		JBP.IconDragger:RegisterIconDragHandler(110, -- Copy Conditions
			...
			function(IconDragger)
				...
				IconDragger.desticon:GetSettings().Conditions = JBP:CopyWithMetatable(srcics.Conditions)
			end
		)

		JBP.IconDragger:RegisterIconDragHandler(111, -- Copy Unit Conditions
			function(IconDragger, info)
			...
			function(IconDragger)
				...
				IconDragger.desticon:GetSettings().UnitConditions = JBP:CopyWithMetatable(srcics.UnitConditions)
			end
		)
...
function Icon.ScriptSort(iconA, iconB)
	local gOrder = 1 -- -JBP.db.profile.CheckOrder
...
JBP:RegisterCallback("JBP_GLOBAL_UPDATE_POST", Icon, "UpdateTable_PerformAutoSort")
...
function Icon.SetScript(icon, handler, func)
	...
	if icon.cpu_startTime and handler == "OnEvent" and func then
		local origFunc = func
		func = function(...) 
			local start = JBP:CpuProfilePush()

			origFunc(...)

			local currentUsage = JBP:CpuProfilePop()
...
-- 
-- Func will be called with signature (icon, time). Time is equal to JBP.time and GetTime().
...
-- @paramsig
-- @return [{{{JBP.Icon_Defaults}}}] The settings table that holds the settings for the icon.
...
function Icon.GetGUID(icon, generate)
	...
	if not GUID then
		if not icon.TempGUID then
			icon.TempGUID = JBP:GenerateGUID("icon", JBP.CONST.GUID_SIZE)
			GUID = icon.TempGUID
		end
		if generate then
			GUID = icon.TempGUID
...
-- @return [{{{JBP.Icon_Defaults.SettingsPerView[view]}}}] The settings table that holds the view-specific settings for the icon.
...
function Icon.IsBeingEdited(icon)
	if JBP.IE and JBP.CI.icon == icon and JBP.IE.CurrentTab and JBP.IE:IsVisible() then
		return JBP.IE.CurrentTab.identifier
	end
end
...
--- Returns information about the icon that should be included when listing it in a dropdown menu. Wrapper around JBP:GetIconMenuText() with the groupID and iconID added to the tooltip.
...
function Icon.GetIconMenuText(icon)
	local text, textshort, tooltip = JBP:GetIconMenuText(icon:GetSettings())
...
--      -- MapConditionObjectToEventSettings maps JBP.CLasses.ConditionObject instances to the eventSettings they were created for.
...
function Icon.QueueEvent(icon, eventInfo)
	...
	-- but it will be done automatically for all icons during JBP_ONUPDATE_TIMECONSTRAINED_POST (in EventHandler.lua)
...
function Icon.RestoreEvents(icon)
	...
	if icon.EventHandlersSet.OnEventsRestored and JBP.Locked then
...
-- 
-- This method is automatically called at the end of a JBP update cycle for all icons that have queued an event,
...
-- 
-- Search JustBecomePro's source for calls of this method and of {{{icon:QueueEvent()}}} for examples of proper usage.
...
function Icon.ProcessQueuedEvents(icon)
	...
	if EventsToFire and icon.eventIsQueued then
		...
		for i = 1, (icon.Events.n or 0) do
			...
			if eventData and EventSettings then
				...
				if shouldProcess and icon.runEvents and icon.attributes.shown then
					local EventHandler = JBP.EVENTS:GetEventHandler(EventSettings.Type)
		...
		if handledOne then
			JBP:Fire("JBP_ICON_UPDATED", icon)
		end
	end
end
...
-- 
-- Setting to "manual" causes the icon to be updated only when {{{icon.NextUpdateTime < JBP.time}}}, or when {{{icon:Update(true)}}} is called (this should not be done outside of special circumstances. Manual updating should be restricted to using only icon.NextUpdateTime).
...
JBP.IconEventUpdateEngine = IconEventUpdateEngine
...
-- 
-- It is automatically called for every valid icon that has an update function defined (through {{{icon:SetUpdateFunction()}}}) each update cycle in the JBP icon update engine.
-- 
...
function Icon.YieldInfo(icon, isNotDone, ...)
	...
	if not icon:IsGroupController() then
		...
		if not isNotDone or not JBP.Locked then
			return nil
		end
...
function Icon.JBP_CNDT_OBJ_PASSING_CHANGED(icon, event, ConditionObject, failed)
...
function Icon.DisableIcon(icon, soft)
	...
	if not soft then
		...
		if iconGUID then
			JBP:DeclareDataOwner(iconGUID, nil)
		end
	end
	
	JBP:Fire("JBP_ICON_DISABLE", icon, soft)
	...
	JBP:UnregisterCallback("JBP_CNDT_OBJ_PASSING_CHANGED", icon)
...
-- 
-- This method should not be called manually while JustBecomePro is locked. It may be called liberally from wherever you see fit when in configuration mode.
-- @name Icon:Setup
-- @paramsig 
function Icon.Setup(icon)
	...
	local typeData = JBP.Types[ics.Type]
	local viewData = group.viewData
	local iconGUID = icon:GetGUID()	

	if not typeData then
		error("JustBecomePro: Critical error: Couldn't find type data or fallback type data for " ..
	...
	-- during setup. Listen for JBP_ICON_SETUP_POST to find when this gets set to nil.
	...
	for k in pairs(JBP.Icon_Defaults) do
	...
	if icon.Enabled or not JBP.Locked then
		...
		JBP:Fire("JBP_ICON_SETUP_PRE", icon)
		...
		if not icon:IsControlled() then 
			icon.LastUpdate = 0
			icon.NextUpdateTime = 0
			JBP.safecall(typeData.Setup, typeData, icon)
		end
		...
		if icon.typeData.type ~= "" then
			...
			if icon.ConditionObject then
				...
				JBP:RegisterCallback("JBP_CNDT_OBJ_PASSING_CHANGED", icon)
	...
	if JBP.Locked then
		...
		if icon.attributes.texture == "Interface\\AddOns\\JustBecomePro\\Textures\\Disabled" then
		...
		if icon.attributes.texture == "" then
			icon:SetInfo("texture", "Interface\\AddOns\\JustBecomePro\\Textures\\Disabled")
		end
	...
	if icon:IsControlled() then
		if JBP.Locked then
			...
			icon:SetInfo("texture; alphaOverride",
				"Interface\\AddOns\\JustBecomePro\\Textures\\Controlled",
	...
	JBP:Fire("JBP_ICON_SETUP_POST", icon)
end
...
function Icon.SetupAllModulesForIcon(icon, sourceIcon)
	for moduleName, Module in pairs(icon.Modules) do
		if Module.SetupForIcon and Module.IsEnabled and not Module.dontInherit then
			JBP.safecall(Module.SetupForIcon, Module, sourceIcon)
		end
	end
end
...
JBP.IconStateArbitrator = {
	...
	UPDATE = function(self, event, icon)
		...
		for i = 1, #StateHandlers do
			...
			if stateData and stateData.Alpha == 0 then
				...
				-- This functionality has existed in JBP since practically day one, by the way. So don't be clever and remove it.
		...
		if handlerToUse then			
			...
			if not state.Alpha then
				...
				-- One case I've seen is doing an undo/redo while JBP is locked. 
	...
	AddHandler = function(self, IconDataProcessor, order, dependant, haltImmediatelyIfFound, configGetter)
		JBP:ValidateType(1, "IconStateArbitrator:AddHandler()", IconDataProcessor, "IconDataProcessor")
		JBP:ValidateType(2, "IconStateArbitrator:AddHandler()", order, "number")
		JBP:ValidateType(3, "IconStateArbitrator:AddHandler()", dependant, "IconDataProcessorComponent;nil")
		JBP:ValidateType(4, "IconStateArbitrator:AddHandler()", haltImmediatelyIfFound, "boolean;nil")
		JBP:ValidateType(5, "IconStateArbitrator:AddHandler()", configGetter, "function;nil")
		...
		sort(self.StateHandlers, JBP.OrderSort)
		
		JBP:RegisterCallback(IconDataProcessor.changedEvent, self, "UPDATE")
	end,	
}
...
function Icon.InheritDataFromIcon(iconDestination, iconSource)
	if not InheritAllFunc then
		...
		for _, Processor in pairs(JBP.Classes.IconDataProcessor.instances) do
			if not Processor.dontInherit then
				attributes[#attributes+1] = Processor.attributesStringNoSpaces
				for _, attribute in JBP:Vararg(strsplit(",", Processor.attributesStringNoSpaces)) do
...
local function SetInfo_GenerateFunction(signature, isInternal)
	...
	for UVSetID, UVSet in ipairs(JBP.Classes.IconDataProcessor.SIUVs) do
		t[#t+1] = "local "
		t[#t+1] = UVSet.variables
		t[#t+1] = " = "
		for referenceID, reference in ipairs(UVSet) do
			t[#t+1] = "JBP.Classes.IconDataProcessor.SIUVs["
			t[#t+1] = UVSetID
			t[#t+1] = "]["
			t[#t+1] = referenceID
			t[#t+1] = "]"
	...
	while #signature > 0 do
		local match
		for _, Processor in ipairs(JBP.Classes.IconDataProcessor.instances) do
	...
	local funcstr = table.concat(t)
	if JBP.debug then
		funcstr = JBP.debug.enumLines(funcstr)
		JBP.debug.SetInfoFuncsToFuncStrs[tostring(isInternal) .. originalSignature] = funcstr
	end
...
-- @usage icon:SetInfo("texture", "Interface\\AddOns\\JustBecomePro\\Textures\\Disabled")
--  
...
--- icon:SetInfo_INTERNAL() is a slightly modified version of icon:SetInfo() that doesn't fire JBP_ICON_UPDATED at the end.
-- It must only be called from within SetInfo (inside IconDataProcessorHooks).
-- SetInfo will fire JBP_ICON_UPDATED at the end (and only once, isntead of multiple times), so SetInfo_INTERNAL won't fire it at all to prevent excessive firings.
-- 
...
--        dogTagUnit = dogTagUnit and JBP_UNITS:TestUnit(dogTagUnit)
--        if not DogTag.IsLegitimateUnit[dogTagUnit] then
--          dogTagUnit = "player"
--        end
--      end
