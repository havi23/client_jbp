	...
	icon = function(self, ics)
		...
		for event, eventSettings in pairs(EventsCopy) do
			...
			if type(event) == "string" and event ~= "n" then
				local addedAnEvent
				for identifier, EventHandler in pairs(JBP.Classes.EventHandler.instancesByName) do
...
JBP:RegisterUpgrade(47321, {
	icon = function(self, ics)
		ics.Events["**"] = nil -- wtf?
	end,
})
JBP:RegisterUpgrade(47320, {
	iconEventHandler = function(self, eventSettings)
...
JBP:RegisterCallback("JBP_DB_PRE_DEFAULT_UPGRADES", function()
	-- The default value of eventSettings.PassThrough changed from false to true.
	if JustBecomeProDB.profiles and JustBecomeProDB.Version < 50035 then
...
JBP:RegisterCallback("JBP_UPGRADE_PERFORMED", function(event, type, upgradeData, ...)
	if type == "icon" then
		...
		for eventID, eventSettings in JBP:InNLengthTable(ics.Events) do
			JBP:Upgrade("iconEventHandler", upgradeData, eventSettings, eventID, ics)
		end
	end
end)


local EventHandler = JBP:NewClass("EventHandler")
...
do	-- EVENTS:InIconEventSettings
	...
	local function getstate()
		...
		state.extIter, state.extIterState = JBP:InIconSettings()
...
function EventHandler:OnNewInstance_EventHandler(identifier, order)
	...
	JBP:SortOrderedTables(EventHandler.orderedInstances)
end
...
function EventHandler:RegisterEventHandlerDataTable(eventHandlerData, ...)
	...
	JBP:ValidateType("eventHandlerData.eventHandler", "EventHandler:RegisterEventHandlerDataTable(eventHandlerData, ...)", eventHandlerData.eventHandler, "table")
	JBP:ValidateType("eventHandlerData.identifier", "EventHandler:RegisterEventHandlerDataTable(eventHandlerData, ...)", eventHandlerData.identifier, "string")
	
	JBP.safecall(self.OnRegisterEventHandlerDataTable, self, eventHandlerData, ...)
...
-- @param defaults [table] The defaults table that will be merged into {{{JBP.Icon_Defaults.Events["**"]}}}
...
function EventHandler:RegisterEventDefaults(defaults)
	JBP:ValidateType("defaults", "EventHandler:RegisterEventDefaults(defaults)", defaults, "table")
	...
	JBP:MergeDefaultsTables(defaults, JBP.Icon_Defaults.Events["**"])
end


--- Tests an event for JBP.CI.icon
...
function EventHandler:TestEvent(eventID)
	...
	return self:HandleEvent(JBP.CI.icon, eventSettings)
end
...
JBP:RegisterCallback("JBP_ICON_SETUP_PRE", function(_, icon)
	...
	for _, eventSettings in JBP:InNLengthTable(icon.Events) do
		...
		if event then
			...
			if thisHasEventHandlers then
				...
				JBP:Fire("JBP_ICON_EVENTS_PROCESSED_EVENT_FOR_USE", icon, event, eventSettings)
	...
	icon.runEventsTimerHandler = EVENTS.ScheduleTimer(icon, "RestoreEvents", JBP.UPD_INTV*2.1)
end)

JBP:RegisterCallback("JBP_ONUPDATE_TIMECONSTRAINED_POST", function(event, time, Locked)
	...
	if Locked and QueuedIcons[1] then
		...
		for i = 1, #QueuedIcons do
			...
			JBP.safecall(icon.ProcessQueuedEvents, icon)
		end
		wipe(QueuedIcons)
	end
end)

JBP:RegisterCallback("JBP_GLOBAL_UPDATE_POST", function(event, time, Locked)
	-- Wipe all queued events when we do a complete update of JBP
	-- in order to kill any events that got queued while setting up an icon (OnShow, etc).
	
	local QueuedIcons = JBP.Classes.Icon.QueuedIcons
...
JBP:NewClass("EventHandler_ColumnConfig", "EventHandler"){
...
-------------------------------------------
--[[
While Condition Set Passing (WCSP) is very tightly integrated with JBP.C.EventHandler
...
if JBP.C.IconType then
	error("Bad load order! JBP.C.IconType shouldn't exist at this point!")
end
...
JBP:NewClass("EventHandler_WhileConditions", "EventHandler"){
	supportWCSP = true,
	...
	OnNewInstance_WhileConditions = function(self)
		...
		JBP:RegisterCallback("JBP_ICON_DISABLE", self)
		JBP:RegisterCallback("JBP_ICON_EVENTS_PROCESSED_EVENT_FOR_USE", self)
		JBP:RegisterCallback("JBP_ICON_SETUP_POST", self)
	end,


	JBP_ICON_DISABLE = function(self, _, icon, soft)
	...
	JBP_ICON_EVENTS_PROCESSED_EVENT_FOR_USE = function(self, _, icon, iconEvent, eventSettings)
		...
		if ConditionObject then
			...
			JBP:RegisterCallback("JBP_CNDT_OBJ_PASSING_CHANGED", self)


			-- DO NOT check the state right here, since animations might be missing required components. 
			-- just let it happen during the queued update for JBP_ICON_SETUP_POST
			-- self:CheckState(ConditionObject)

			-- Queue an update during JBP_ICON_SETUP_POST, 
	...
	JBP_ICON_SETUP_POST = function(self, _, icon)
		-- Run updates for anything that is queued. 
		-- There should only be one icon worth of ConditionObjects in here,
		-- since JBP_ICON_SETUP_PRE and JBP_ICON_SETUP_POST are always called in pairs.
	...
	JBP_CNDT_OBJ_PASSING_CHANGED = function(self, _, ConditionObject, failed)
		self:CheckState(ConditionObject)
	end,
	...
	CheckState = function(self, ConditionObject)
		...
		if JBP.Locked and matches then
			-- If JBP is locked, and there are eventSettings that are using this ConditionObject,
...
JBP:NewClass("EventHandler_WhileConditions_Repetitive", "EventHandler_WhileConditions"){
	frequencyMinimum = 0.2,

	OnNewInstance_WhileConditions_Repetitive = function(self)
		self.RunningTimers = {}

		JBP:RegisterCallback("JBP_ONUPDATE_POST", self)
		JBP:RegisterCallback("JBP_ICON_DISABLE", self, "JBP_ICON_DISABLE_2")
	end,

	JBP_ICON_DISABLE_2 = function(self, _, icon, soft)
	...
	JBP_ONUPDATE_POST = function(self, event, time, Locked)
		...
		if not failed then
			...
			for eventSettings, icon in pairs(eventSettingsList) do
				...
				if not timerTable then
					...
					self.RunningTimers[eventSettings] = {icon = icon, nextRun = JBP.time}
				else
					...
					if timerTable.nextRun < JBP.time then
						timerTable.nextRun = JBP.time
					end
				end
			end
		else
...
do
	...
	local ConditionSet = {
		...
		ShouldShowTab = function(self)
			local button = JustBecomePro_IconEditor.Pages.Events.EventSettingsContainer.IconEventWhileCondition
