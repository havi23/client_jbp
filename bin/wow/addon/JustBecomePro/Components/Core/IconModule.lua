...
local IconModule = JBP:NewClass("IconModule", "IconComponent", "ObjectModule")
...
function IconModule:OnFirstInstance_IconModule()
	...
	for event, func in pairs(self.EventListners) do
		if type(func) == "function" then
			...
			JBP:RegisterCallback(event, function(event, icon, ...)
	...
	for name in pairs(self.anchorableChildren) do
		...
		if not localizedName then
			JBP:Error("Localized name for %q is missing! (JBP.L[%q])", identifier, identifier)
		end
...
function IconModule:OnImplementIntoIcon(icon)
	...
	if self.IsEnabled then
		JBP.safecall(self.SetupForIcon, self, icon)
	end
end
...
--- Create an event listener for JBP events that are fired with icon as their first arg (these events are always prefixed with "JBP_ICON_"). The function will only be called if the icon that the event was fired for is the icon that an instance of this [[api/icon-module/api-documentation/|IconModule]] has been implemented into, and if {{{IconModule.IsEnabled == true}}} for that instance. This method must be called on a class - you cannot set icon event handlers separately for individual instances of [[api/icon-module/api-documentation/|IconModule]] using this method.
-- @param event [string] An event. Should be prefixed with "JBP_ICON_".
...
--	PowerBar_Overlay:SetIconEventListner("JBP_ICON_SETUP_POST", function(Module, icon)
--		if JBP.Locked then
...
--- Creates an event handler that listens to changes in data from the specified [[api/icon-data-processor/api-documentation/|IconDataProcessor]]. This method is essentially a wrapper for {{{JBP.Classes.IconModule:SetIconEventListner( JBP.Classes.IconDataProcessor.ProcessorsByName[processorName].changedEvent [,func])}}}
...
function IconModule:SetDataListener(processorName, func)
	...
	local Processor = JBP.Classes.IconDataProcessor.ProcessorsByName[processorName]
...
--	function JBP.Classes.IconModule_CooldownSweep:OnNewInstance(icon)
...
--- Declare a frame or layer as being anchorable so that components across JBP can be user-configured to use it as an anchor (for things like animation and text strings). The localized name must be defined to JBP.L as {{{self.className .. name}}}.
...
--	-- <...>
-- 
--	JBP.Classes.IconModule_CooldownSweep:RegisterAnchorableFrame("Cooldown")
--	
--	function JBP.Classes.IconModule_CooldownSweep:OnNewInstance(icon)
--		self.cooldown = CreateFrame("COOLDOWN", self:GetChildNameBase() .. "Cooldown", icon, "CooldownFrameTemplate")
--		
...
function IconModule:RegisterAnchorableFrame(name)
	self:AssertSelfIsClass()
	JBP:ValidateType("2 (name)", "IconModule:RegisterAnchorableFrame(name)", name, "string")
...
function IconModule:SetAllowanceForType(typeName, allow)
	...
	JBP:ValidateType(2, "IconModule:SetAllowanceForType()", typeName, "string")
	
	-- allow cannot be nil
	JBP:ValidateType(3, "IconModule:SetAllowanceForType()", allow, "boolean")
	
	if self.TypeAllowances[typeName] == nil then
		self.TypeAllowances[typeName] = allow
	else
		JBP:Error("A module's type allowance cannot be set once it has already been declared by either a module or an icon type.")
	end
end
...
-- JBP.Classes.IconModule_PowerBar_Overlay:SetDefaultAllowanceForTypes(false)
-- 
...
function IconModule:SetDefaultAllowanceForTypes(allow)
	...
	JBP:ValidateType(2, "IconModule:SetDefaultAllowanceForTypes()", allow, "boolean")
