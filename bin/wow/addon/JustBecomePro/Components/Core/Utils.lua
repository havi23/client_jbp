...
local Formatter = JBP:NewClass("Formatter"){
...
Formatter{
	NONE = Formatter:New(JBP.NULLFUNC),
	...
	TIME_COLONS = Formatter:New(function(value)
		return JBP:FormatSeconds(value, nil, 1)
	end),
...
function JBP:MakeFunctionCached(obj, method)
	local func
	...
	elseif type(obj) == "function" then
		...
		error("Usage: JBP:MakeFunctionCached(object/function [, method])")
	end
...
function JBP:MakeNArgFunctionCached(argCount, obj, method)
	local func
	if type(obj) == "table" and type(method) == "string" then
		func = obj[method]
	...
	elseif type(obj) == "function" then
		...
		error("Usage: JBP:MakeNArgFunctionCached(argCount, object/function [, method])")
	end
	...
	--[[
		...
		local index = 10000
		local f1 = JBP:MakeFunctionCached(Baseline)
		local f2 = JBP:MakeNArgFunctionCached(1, Baseline)
...
function JBP:MakeSingleArgFunctionCached(obj, method)
	...
	elseif type(obj) == "function" then
		...
		error("Usage: JBP:MakeFunctionCached(object/function [, method])", 2)
	end
...
function JBP:DoInitialWarn()
	...
	JBP.Warned = true
	JBP.DoInitialWarn = JBP.NULLFUNC
end

function JBP:Warn(text)
	if warn[text] then
		return
	elseif JBP.Warned then
		JBP:Print(text)
		warn[text] = true
	elseif not JBP.tContains(warn, text) then
		tinsert(warn, text)
	end
end

function JBP:Debug(...)
	if JBP.debug or not JBP.Initialized then
		JBP.print(format(...))
	end
end

function JBP:Error(text, ...)
	text = text or ""
	local success, result = pcall(format, text, ...)
	if success then
		text = result
	end
	geterrorhandler()("JustBecomePro: " .. text)
end

function JBP:Assert(statement, text, ...)
...
function JBP:SplitNames(input, stringsOnly)
	input = JBP:CleanString(input)
...
JBP.SplitNamesCached = JBP.SplitNames
JBP:MakeSingleArgFunctionCached(JBP, "SplitNamesCached")


function JBP:FormatSeconds(seconds, skipSmall, keepTrailing)
	local ret = ""
...
local function parseFlagTable(tbl)
	...
	for k, v in pairs(tbl) do
		if v then
			local f = JBP.tContains(flagMap, k)
			if f or #k == 1 then
				ret = ret .. (f or k)
			end
		end
...
function JBP:RGBATableToStringWithoutFlags(table)
	if type(table) == "string" then
		return table
	end

	return JBP:RGBAToString(table.r, table.g, table.b, table.a)
end

function JBP:RGBATableToStringWithFallback(table, fallbackStr)
	if type(table) == "string" then
		return table
	...
	local r, g, b, a, flags = JBP:StringToRGBA(fallbackStr)
	
	return JBP:RGBAToString(table.r or r, table.g or g, table.b or b, table.a or a, table.flags or flags)
end

local function to8Bit(v)
	return floor(v * 0xFF + 0.5)
end
function JBP:RGBAToString(r, g, b, a, flags)
	return format("%02x%02x%02x%02x%s", to8Bit(a or 1), to8Bit(r), to8Bit(g), to8Bit(b), parseFlagTable(flags))
end

function JBP:StringToRGBA(str)
...
function JBP:StringToCachedRGBATable(str)
	if type(str) == "table" then
		return str
	end

	local r, g, b, a, flags = JBP:StringToRGBA(str)
	return {r=r,g=g,b=b,a=a, flags=flags}
end
JBP:MakeSingleArgFunctionCached(JBP, "StringToCachedRGBATable")
...
local getColorsTemp = {}
function JBP:GetColors(colorSettings, enableSetting, ...)
	if not colorSettings then
		error("colorSettings missing")
	end
	for n, settings, length in JBP:Vararg(...) do
...
function JBP:ColorStringToCachedHSVATable(str)
	local r, g, b, a, flags = JBP:StringToRGBA(str)
	local h, s, v = JBP:RGBToHSV(r, g, b)
	return {h=h, s=s, v=v, a=a, flags=flags}
end
JBP:MakeSingleArgFunctionCached(JBP, "ColorStringToCachedHSVATable")

function JBP:HSVAToColorString(h, s, v, a, flags)
	local r, g, b = JBP:HSVToRGB(h, s, v)
	return JBP:RGBAToString(r, g, b, a, flags)
end
...
function JBP.tContains(table, item, returnNum)
	local firstkey
	local num = 0
...
function JBP.tDeleteItem(table, item, onlyOne)
	local i = 1
	local removed
	while table[i] do
...
function JBP.tRemoveDuplicates(table)
...
function JBP.binaryInsert(table, value, comp)
...
function JBP:SortOrderedTables(parentTable)
	sort(parentTable, JBP.OrderSort)
	return parentTable
end

function JBP:CopyWithMetatable(source, blocker)	
	local dest = {}
	...
	for k, v in pairs(source) do
		...
		if keyBlocker ~= true then
			if type(v) == "table" then
				dest[k] = JBP:CopyWithMetatable(v, keyBlocker)
			else
				dest[k] = v
			end
		end
...
function JBP:CopyInPlaceWithMetatable(source, dest, blocker)
	...
	for key in pairs(source) do
		...
		if keyBlocker ~= true then
			if type(source[key]) == "table" then
				...
				JBP:CopyInPlaceWithMetatable(source[key], dest[key], keyBlocker)
			else
				dest[key] = source[key]
			end
		end
...
function JBP:CopyTableInPlaceUsingDestinationMeta(src, dest, allowUnmatchedSourceTables)
	...
	for k in pairs(src) do
		if type(dest[k]) == "table" and type(src[k]) == "table" then
			JBP:CopyTableInPlaceUsingDestinationMeta(src[k], dest[k], allowUnmatchedSourceTables)
...
function JBP:DeepCompare(t1, t2)
	...
	for k1, v1 in pairs(t1) do
		...
		if v1 ~= v2 and not JBP:DeepCompare(v1, v2) then
			return false
		end
	end
...
do	-- JBP.shellsortDeferred
	...
	function JBP.shellsortDeferred(t, before, n, callback, callbackArg, progressCallback, progressCallbackArg)
...
do -- InNLengthTable
	...
	function JBP:InNLengthTable(arg)
		if arg then
			return iter, getstate(0, arg)
		else
			error("Bad argument #1 to 'JBP:InNLengthTable(arg)'. Expected table, got nil.", 2)
		end
	end
end
...
do -- ordered pairs
	...
	function JBP:OrderedPairs(t, compare, byValues, rev)
		if not next(t) then
			return JBP.NULLFUNC
		end
...
local function TTOnEnter(self)
	...
	if  (not self.__ttshowchecker or JBP.get(self[self.__ttshowchecker], self))
	...
	then
		...
		GameTooltip:AddLine(JBP.get(self.__title, self), HIGHLIGHT_FONT_COLOR.r, HIGHLIGHT_FONT_COLOR.g, HIGHLIGHT_FONT_COLOR.b, false)
		local text = JBP.get(self.__text, self)
		if text then
...
function JBP:TT_Anchor(f)
	GameTooltip:SetOwner(f, "ANCHOR_NONE")
	GameTooltip:SetPoint("TOPLEFT", f, "BOTTOMRIGHT", 0, 0)
end

function JBP:TT(f, title, text, actualtitle, actualtext, showchecker)
	...
	-- if they arent set and it isnt a global, then it must be a JBP localized string, so use that

	JBP:ValidateType(2, "JBP:TT()", f, "frame")
	
	f.__title = JBP:TT_Parse(title, actualtitle)
	f.__text = JBP:TT_Parse(text, actualtext)
...
function JBP:TT_Parse(text, literal)
	if text then
...
function JBP:TT_Copy(src, dest)
	JBP:TT(dest, src.__title, src.__text, 1, 1, src.__ttshowchecker)
end

function JBP:TT_Update(f)
...
function JBP.NULLFUNC()
	-- Do nothing
end

function JBP.oneUpString(string)
...
animator.OnUpdate = function()
	for f in pairs(animator.frames) do
		if JBP.time - f.__animateHeight_startTime > f.__animateHeight_duration then
			animator.frames[f] = nil
			f:SetClipsChildren(f.__animateHeight_wasClipping)
			f:SetHeight(f.__animateHeight_end)
		else
			local pct = (JBP.time - f.__animateHeight_startTime)/f.__animateHeight_duration
...
function JBP:AnimateHeightChange(f, endHeight, duration)
	...
	f.__animateHeight_startTime = JBP.time
...
function JBP.SpellHasNoMana(spell)
...
function JBP.GetRuneCooldownDuration()
...
function JBP.GetSpellCost(spell)
...
function JBP.GetCurrentSpecializationRole()
...
function JBP:GetRaceIconInfo(race)
	race = race:lower()
...
JBP:MakeSingleArgFunctionCached(JBP, "GetRaceIconInfo")

function JBP:TryGetNPCName(id)
    local tooltip, LT1 = JBP:GetParser()
...
function JBP:FormatAtlasString(atlasName, trimPercent)
...
local detectors = {}
function JBP:RegisterLuaImportDetector(func)
	detectors[func] = true
end
...
local function recursivelyDetectLua(results, table, ...)
	if type(table) == "table" then
		for func in pairs(detectors) do
			local success, code, name = JBP.safecall(func, table, ...)
...
function JBP:DetectImportedLua(table)
	local results = {}
