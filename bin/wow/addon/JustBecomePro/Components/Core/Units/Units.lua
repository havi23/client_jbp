...
local UNITS = JBP:NewModule("Units", "AceEvent-3.0")
...
function JBP:GetUnits(icon, setting, Conditions)
	local iconName = (icon and icon:GetName() or "<icon>")
	assert(setting, "Setting was nil for JBP:GetUnits(" .. iconName .. ", setting)")
	...
	if Conditions then
		serializedConditions = JBP:Serialize(Conditions)
	...
	if UnitSet.ConditionObjects then
		...
		JBP:RegisterCallback("JBP_CNDT_OBJ_PASSING_CHANGED", UnitSet)
	end
...
local UnitSet = JBP:NewClass("UnitSet"){
	...
	OnNewInstance = function(self, unitSettings, Conditions, parent)		
		...
		if Conditions and Conditions.n > 0 then
			for k, unit in ipairs(self.originalUnits) do
				...
				for _, condition in JBP:InNLengthTable(ModifiableConditions) do
	...
	JBP_CNDT_OBJ_PASSING_CHANGED = function(self, event, ConditionObject, failed)
	...
	Update = function(self, forceNoExists)
		...
		if changed then
			JBP:Fire("JBP_UNITSET_UPDATED", self)
		end
...
JBP:RegisterCallback("JBP_GLOBAL_UPDATE", function()
	for i, UnitSet in ipairs(JBP.Classes.UnitSet.instances) do
		if UnitSet.ConditionObjects then
			for i, ConditionObject in ipairs(UnitSet.ConditionObjects) do
				ConditionObject:RequestAutoUpdates(UnitSet, false)
			end
			JBP:UnregisterCallback("JBP_CNDT_OBJ_PASSING_CHANGED", UnitSet)
		end
	end
end)
...
function UNITS:GetUnitSet(unitSettings, SerializedConditions)
	...
	unitSettings = JBP:CleanString(unitSettings):
...
JBP:MakeNArgFunctionCached(2, UNITS, "GetUnitSet")
...
function UNITS:GetOriginalUnitTable(unitSettings)
	...
	for _, wholething in JBP:Vararg(strsplit(";", unitSettings)) do
		local added = false
	...
	unitSettings = JBP:CleanString(unitSettings_new)
	...
	for wholething, unit, firstnum, lastnum, append in gmatch(unitSettings, "(([%a%d]+) ?(%d+) ?%- ?(%d+) ?([%a%d]*)) ?;?") do
		if unit and firstnum and lastnum then
			...
			if abs(lastnum - firstnum) > 100 then
				JBP:Print("Why on Earth would you want to track more than 100", unit, "units? I'll just ignore it and save you from possibly crashing.")
			else
				local str = ""
	...
	local Units = JBP:SplitNames(unitSettings, true) -- get a table of everything

	-- REMOVE DUPLICATES
	JBP.tRemoveDuplicates(Units)

	return Units
end
JBP:MakeSingleArgFunctionCached(UNITS, "GetOriginalUnitTable")
	...
	for i = 1, #instances do
		...
		if unitSet.updateEvents[event] then
			if not unitSet:Update(forceNoExists) then
				-- If the units in the UnitSet didn't change, still fire a JBP_UNITSET_UPDATED to signal that they may have shuffled around a bit
				-- (for example, the player changed target, or raid members moved around)
				JBP:Fire("JBP_UNITSET_UPDATED", unitSet)
			end
		end
	end
	forceNoExists = nil
...
do
	...
	JBP:RegisterUpgrade(60344, {
		icon = function(self, ics)
			for n, condition in JBP:InNLengthTable(ics.UnitConditions) do
				condition.Unit = "unit"
			end
		end,
	})
	...
	local ConditionSet = {
		...
		GetSettings = function(self)
			return JBP.CI.ics and JBP.CI.ics.UnitConditions
		end,
		...
		ShouldShowTab = function(self)
			return JustBecomePro_Unit and JustBecomePro_Unit:IsShown()
		end,
