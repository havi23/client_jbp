...
-- Module creation
JBP.COMMON.Textures = CreateFrame("Frame")

-- Upvalues
local Textures = JBP.COMMON.Textures
...
local function UpdateVarTextures_item()
	...
	if updated then
		JBP:Fire("JBP_TEXTURES_VARTEX_ITEM_CHANGED")
	end
end
...
function Textures:EvaluateTexturePath(CustomTex, ...)
	...
	elseif CustomTex:sub(1, 1) == "$" then
		...
		if varType == "item" then
			...
			if varData and varData >= INVSLOT_FIRST_EQUIPPED and varData <= INVSLOT_LAST_EQUIPPED then
				...
				JBP:RegisterCallback("JBP_TEXTURES_VARTEX_ITEM_CHANGED", ...)
	...
	if not keepItemCallback then
		JBP:UnregisterCallback("JBP_TEXTURES_VARTEX_ITEM_CHANGED", ...)
	end
...
function Textures:GetTexturePathFromSetting(setting)
	...
	if setting and setting ~= "" then
		...
		if JBP.GetSpellTexture(setting) then
			return JBP.GetSpellTexture(setting)
		end
