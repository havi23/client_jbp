...
local NAMES = JBP:NewModule("Names", "AceEvent-3.0")
JBP.NAMES = NAMES
NAMES.ClassColors = {}
...
function NAMES:OnInitialize()
	...
	-- Now, it is managed by JBP's unit code, so only units that can possibly exist will be checked.
	self.unitList = JBP:GetUnits(self, self.CONST.UNIT_LIST)
...
function NAMES:UpdateClassColors()
	...
	for class, color in pairs(CUSTOM_CLASS_COLORS or RAID_CLASS_COLORS) do
		if color.colorStr then
			...
			self.ClassColors[class] = "|c" .. JBP:RGBATableToStringWithoutFlags(color)
		end
	end
end
...
-- This tag is registered with LibDogTag-Unit-3.0's namespace instead of JBP's namespace because it requires the event processing that is provided by the Unit namespace
DogTag:AddTag("Unit", "JBPName", {
...
DogTag:AddTag("JBP", "Name", {
	alias = "JBPName(unit=unit, color=color, server=server)",
	arg = {
		'unit', 'string;undef', 'player',
