...
function Item:GetRepresentation(what)
	...
	if type(what) == "number" then
		...
		elseif what <= INVSLOT_LAST_EQUIPPED then
			return JBP.Classes.ItemBySlot:New(what)
		else
			return JBP.Classes.ItemByID:New(what)
		end
	...
	elseif type(what) == "string" then
		...
		elseif what:find("|H") then
			return JBP.Classes.ItemByLink:New(what)
		else
			return JBP.Classes.ItemByName:New(what)
		end
...
JBP:MakeSingleArgFunctionCached(Item, "GetRepresentation")

function JBP:GetItems(setting)
	local names = JBP:SplitNames(setting)
...
JBP:MakeSingleArgFunctionCached(JBP, "GetItems")
...
JBP:RegisterEvent("BAG_UPDATE", UPDATE_ITEM_COUNT)
JBP:RegisterEvent("BAG_UPDATE_COOLDOWN", UPDATE_ITEM_COUNT)
...
function Item:OnNewInstance()
	if self.className == "Item" then
		error("JustBecomePro Class 'Item' should not be instantiated.")
	end
end
...
local ItemByID = JBP:NewClass("ItemByID", Item)

function ItemByID:OnNewInstance(itemID)
	JBP:ValidateType("2 (itemID)", "ItemByID:New(itemID)", itemID, "number")
...
local ItemByName = JBP:NewClass("ItemByName", Item)

function ItemByName:OnNewInstance(itemName)
	JBP:ValidateType("2 (itemName)", "ItemByID:New(itemName)", itemName, "string")
...
local ItemBySlot = JBP:NewClass("ItemBySlot", Item)

function ItemBySlot:OnNewInstance(itemSlot)
	JBP:ValidateType("2 (itemSlot)", "ItemByID:New(itemSlot)", itemSlot, "number")
...
local ItemByLink = JBP:NewClass("ItemByLink", Item)

function ItemByLink:OnNewInstance(itemLink)
	JBP:ValidateType("2 (itemLink)", "ItemByID:New(itemLink)", itemLink, "string")
