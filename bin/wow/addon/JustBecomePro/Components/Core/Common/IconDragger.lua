...
local IconDragger = JBP:NewModule("IconDragger", "AceTimer-3.0", "AceEvent-3.0")
JBP.IconDragger = IconDragger
...
IconDragger.Dropdown = JBP.C.Config_DropDownMenu_NoFrame:New()
...
function IconDragger:Start(icon)
	...
	if tex == "" or not tex then
		tex = "Interface\\AddOns\\JustBecomePro\\Textures\\Disabled"
	end
...
function IconDragger:CompleteDrag(script, icon)
	...
	if IconDragger.IsDragging then
		...
		if not DropDownList1:IsShown() or JBP.DD.OPEN_MENU ~= Dropdown then
			Dropdown:Toggle(1)
		end
	end
end
...
function IconDragger:RegisterIconDragHandler(order, dropdownFunc, actionFunc)
	JBP:ValidateType("2 (order)", "IconDragger:RegisterIconDragHandler(order, dropdownFunc, actionFunc)", order, "number")
	JBP:ValidateType("3 (func)", "IconDragger:RegisterIconDragHandler(order, dropdownFunc, actionFunc)", dropdownFunc, "function")
	JBP:ValidateType("4 (func)", "IconDragger:RegisterIconDragHandler(order, dropdownFunc, actionFunc)", actionFunc, "function")
	...
	JBP:SortOrderedTables(IconDragger.Handlers)
end
...
IconDragger:RegisterIconDragHandler(1,	-- Move
	...
	function(IconDragger)
		...
		JBP:PrepareIconSettingsForCopying(srcics, srcgs)
...
IconDragger:RegisterIconDragHandler(2,	-- Copy
	...
	function(IconDragger)
		...
		IconDragger.desticon.group:GetSettings().Icons[IconDragger.desticon:GetID()] = JBP:CopyWithMetatable(srcics)
...
IconDragger:RegisterIconDragHandler(3,	-- Swap
	...
	function(IconDragger)
		...
		local srcgs = IconDragger.srcicon.group:GetSettings()
		local srcics = IconDragger.srcicon:GetSettings()
		JBP:PrepareIconSettingsForCopying(destics, destgs)
		JBP:PrepareIconSettingsForCopying(srcics, srcgs)
...
local function Split(IconDragger, domain)
	if InCombatLockdown() then
		...
		error("JBP: Can't create groups while in combat")
	end

	local group = JBP:Group_Add(domain)
	...
	local success, err = pcall(function()
		...
		JBP:CopyTableInPlaceUsingDestinationMeta(IconDragger.srcicon.group:GetSettings(), group:GetSettings())
	end)
