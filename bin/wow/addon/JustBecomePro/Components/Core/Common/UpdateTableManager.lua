...
local UpdateTableManager = JBP:NewClass("UpdateTableManager")
...
function UpdateTableManager:UpdateTable_Register(target)
	...
	JBP:ValidateType("2 (target)", "UpdateTableManager:UpdateTable_Register(target)", target, "!boolean")
	
	if target == nil then
		if self.class == UpdateTableManager then
		JBP:ValidateType("2 (target)", "UpdateTableManager:UpdateTable_Register(target)", target, "!nil")
		else
			target = self
		end
	end
	...
	if not JBP.tContains(self.UpdateTable_UpdateTable, target) then

		if self.UpdateTable_DoAutoSort then
			JBP.binaryInsert(self.UpdateTable_UpdateTable, target, self.UpdateTable_AutoSortFunc)
		else
...
function UpdateTableManager:UpdateTable_Unregister(target)
	...
	JBP.tDeleteItem(self.UpdateTable_UpdateTable, target, true)
