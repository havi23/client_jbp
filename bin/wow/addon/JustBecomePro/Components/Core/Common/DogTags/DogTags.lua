...
local DOGTAG = JBP:NewModule("DogTags")
JBP.DOGTAG = DOGTAG

JBP.DOGTAG.nsList = "Base;JBP;Unit;Stats"
...
function JBP:CreateDogTagEventString(...)
	-- We return a function here so that we aren't performing
	-- JBP:RegisterCallback(Processor.changedEvent, DogTagEventHandler)
	-- unless the DogTag ever gets actually used by an icon.
	-- This is a non-trivial performance boost in high-icon-count setups.
	local processors = { ... }
	return function() 
		local eventString = "JBP_GLOBAL_UPDATE_POST"

		for i, dataProcessorName in pairs(processors) do
			local Processor = JBP.Classes.IconDataProcessor.ProcessorsByName[dataProcessorName]
			JBP:RegisterCallback(Processor.changedEvent, DogTagEventHandler)
...
JBP:RegisterCallback("JBP_GLOBAL_UPDATE_POST", DogTag.FireEvent, DogTag)

DogTag:AddTag("JBP", "JBPFormatDuration", {
	code = JBP:MakeSingleArgFunctionCached(function(seconds)
		return JBP:FormatSeconds(seconds, seconds == 0 or abs(seconds) > 10, true)
	end),
	arg = {
		'seconds', 'number', '@req',
	},
	ret = "string",
	static = true,
	doc = L["DT_DOC_JBPFormatDuration"],
	example = '[0.54:JBPFormatDuration] => "0.5"; [20:JBPFormatDuration] => "20"; [80:JBPFormatDuration] => "1:20"; [10000:JBPFormatDuration] => "2:46:40"',
	category = L["TEXTMANIP"]
})

DogTag:AddTag("JBP", "gsub", {
	code = gsub,
	arg = {
		'value', 'string', '@req',
		'pattern', 'string', '@req',
...
-- if the unit kwarg doesn't exist. This happens all the time in JBP, but it doesn't mean we should
...
-- can only break things for JBP, which is perfect.
DogTag:AddCompilationStep("JBP", "finish", function(t, ast, kwargTypes, extraKwargs)
	if kwargTypes["unit"] then
		...
		-- Remove the UnitExists check that wraps around the JBPName tag.
		for i = 1, #t do
			if t[i] == "JBPName"
