...
local DOGTAG = JBP.DOGTAG



JBP:NewClass("Config_EditBox_DogTags", "Config_EditBox"){
	OnNewInstance_EditBox_DogTags = function(self, data)
		self:CScriptAdd("ModifyValueForSave", self.ModifyValueForSave)

		JBP.SUG:EnableEditBox(self, "dogtags")
	end,
	...
	ChatEdit_InsertLink_Hook = function(self, text, linkType, linkData)
		...
		JBP.HELP:Show{
			...
			x = 0,
			y = 0,
			text = format(JBP.L["HELP_ANN_LINK_INSERTED"])
		}
	...
	EnableLinkInsertion = function(self)
		JBP.Classes.ChatEdit_InsertLink_Hook:New(self, self.ChatEdit_InsertLink_Hook)
		self.EnableLinkInsertion = JBP.NULLFUNC
	end,
	...
	METHOD_EXTENSIONS_PRE = {
		OnTextChanged = function(self, userInput)			
			if userInput and (GetLocale() == "zhCN" or GetLocale() == "zhTW") then
				...
				-- See http://wow.curseforge.com/addons/justbecomepro/tickets/641-typing-chinese-error/
				return
			end 
...
local extendedTags = {
	Duration = "Duration:JBPFormatDuration",
	Source = "Source:Name",
	...
	Timer = "Timer:JBPFormatDuration",
}

local function prepareEditBox(box)
	if not box.PreparedForDogTagInsertion then
		box:SetAcceptsJBPLinks(true, L["DT_INSERTGUID_TOOLTIP"])

		JBP.Classes.ChatEdit_InsertLink_Hook:New(box, function(self, text, linkType, linkData)
			-- if this editbox is active
			-- attempt to extract a JBP icon link and insert it into the box.
...
local function getCurrentTag(editbox)
	for i = editbox:GetCursorPosition(), 1, -1 do
	    ...
	    if color then
	    	local tokenType = JBP.tContains(colors, color)
...
function Module:Table_GetNormalSuggestions(suggestions, tbl)
	...
	for _, namespaceName in pairs(DogTag.unpackNamespaceList[DOGTAG.nsList]) do
		...
		if namespace and (currentTag or namespaceName == "JBP") then
	...
	JBP.tRemoveDuplicates(suggestions)
end
...
function GameTooltip:JBP_SetDogTag(tagName)
	local tag = "[" .. tagName .. "]"
	local colorized = DogTag:ColorizeCode(tag)
	...
	for i, tag in JBP:Vararg(strsplit(":", tagName)) do
		...
		if not tagData then
			JBP:Debug("NO TAG DATA FOR TAG %s", tag)
		else
			...
			if tag_colorized then
				...
				for i, v in JBP:Vararg((";"):split(tagData.example)) do
...
do
	...
	function JBP:TestDogTagString(icon, text, ns, kwargs)
		--icon:Setup()
		
		ns = ns or JBP.DOGTAG.nsList
		kwargs = kwargs or {
			icon = icon.ID,
			group = icon.group.ID,
