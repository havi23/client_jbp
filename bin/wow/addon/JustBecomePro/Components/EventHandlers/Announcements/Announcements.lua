...
local Announcements = JBP:NewClass(nil, "EventHandler_WhileConditions_Repetitive", "EventHandler_ColumnConfig"):New("Announcements", 20)
...
JBP:RegisterUpgrade(80003, {
	iconEventHandler = function(self, eventSettings)
		eventSettings.TextColor = JBP:RGBAToString(eventSettings.r or 1, eventSettings.g or 1, eventSettings.b or 1, 1)
...
function Announcements:HandleEvent(icon, eventSettings)
	...
	if Channel ~= "" then
		...
		if chandata.isBlizz then
			JBP.NAMES.dogTag_forceUncolored = true
		end
		Text = DogTag:Evaluate(Text, JBP.DOGTAG.nsList, Announcements.kwargs)
		JBP.NAMES.dogTag_forceUncolored = nil
		...
		if Text then
			...
			elseif chandata.isBlizz then
				...
				if Channel == "WHISPER" then
					...
					Location = DogTag:Evaluate(Location, JBP.DOGTAG.nsList, Announcements.kwargs)
					if Location then
...
function Announcements:OnRegisterEventHandlerDataTable(eventHandlerData, order, channel, channelData)
	JBP:ValidateType("2 (order)", '[RegisterEventHandlerData - Announcements](order, channel, channelData)', order, "number")
	JBP:ValidateType("3 (channel)", '[RegisterEventHandlerData - Announcements](order, channel, channelData)', channel, "string")
	JBP:ValidateType("4 (channelData)", '[RegisterEventHandlerData - Announcements](order, channel, channelData)', channelData, "table")
	...
	JBP:SortOrderedTables(Announcements.AllChannelsOrdered)
end
	...
	handler = function(icon, eventSettings, Text)
		...
		while _G["ChatFrame"..i] do
			...
			if Location == frame.name then
				frame:AddMessage(Text, JBP:StringToRGBA(eventSettings.TextColor))
				break
			end
			i = i+1
		end
...
Announcements:RegisterEventHandlerDataNonSpecific(72, "ERRORS_FRAME", {
	...
	handler = function(icon, eventSettings, Text)
		...
		UIErrorsFrame:AddMessage(Text, JBP:StringToRGBA(eventSettings.TextColor))
	end,
})
...
Announcements:RegisterEventHandlerDataNonSpecific(81, "SCT", {
	...
	handler = function(icon, eventSettings, Text)
		if SCT then
			local color = JBP:StringToCachedRGBATable(eventSettings.TextColor)
...
Announcements:RegisterEventHandlerDataNonSpecific(83, "MSBT", {
	...
	handler = function(icon, eventSettings, Text)
		if MikSBT then
			...
			local c = JBP:StringToCachedRGBATable(eventSettings.TextColor)
