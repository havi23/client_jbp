...
local Animations = JBP:NewClass(nil, "EventHandler_WhileConditions", "EventHandler_ColumnConfig"):New("Animations", 30)
...
JBP:RegisterUpgrade(80003, {
	iconEventHandler = function(self, eventSettings)
		eventSettings.AnimColor = JBP:RGBAToString(
...
function Animations:JBP_ONUPDATE_POST()
...
JBP:RegisterCallback("JBP_ONUPDATE_POST", Animations)
...
function Animations:OnRegisterEventHandlerDataTable(eventHandlerData, order, animation, animationData)
	JBP:ValidateType("2 (order)", '[RegisterEventHandlerData - Animations](order, animation, animationData)', order, "number")
	JBP:ValidateType("3 (animation)", '[RegisterEventHandlerData - Animations](order, animation, animationData)', animation, "string")
	JBP:ValidateType("4 (animationData)", '[RegisterEventHandlerData - Animations](order, animation, animationData)', animationData, "table")
	...
	JBP:ValidateType("text", "animationData", animationData.text, "string")
	JBP:ValidateType("Play", "animationData", animationData.Play, "function")
	JBP:ValidateType("ConfigFrames", "animationData", animationData.ConfigFrames, "table")
	
	for i, configFrameIdentifier in ipairs(animationData.ConfigFrames) do
		JBP:ValidateType(i, "animationData.ConfigFrames", configFrameIdentifier, "string")
	end
	...
	JBP:SortOrderedTables(self.AllAnimationsOrdered)
end
...
local function GetAnchorOrWarn(icon, anchorTo)
	...
	if not frame then
		JBP:Warn(L["ANIM_ANCHOR_NOT_FOUND"]:format(name))
		if JBP.debug then
			JBP:Error(L["ANIM_ANCHOR_NOT_FOUND"]:format(name))
		end
		return icon
	end
...
Animations:RegisterEventHandlerDataNonSpecific(10, "SCREENSHAKE", {
	...
	Play = function(icon, eventSettings)
		if not WorldFrame:IsProtected() or not InCombatLockdown() then
			...
			if not WorldFrame.JBPShakeAnim then
				WorldFrame.JBPShakeAnim = JBP.Classes.AnimatedObject:New()
			end

			WorldFrame.JBPShakeAnim:Animations_Start{
				...
				-- animation table is proxied to UIParent.JBPFlashAnim.
				proxyTarget = icon,
	...
	OnUpdate = function(JBPShakeAnim, table)
		local remaining = table.Duration - (JBP.time - table.Start)

		if remaining < 0 then
			JBPShakeAnim:Animations_Stop(table)
		else
			...
			for _, v in pairs(JBPShakeAnim.WorldFramePoints) do
				WorldFrame:SetPoint(v[1], v[2], v[3], v[4] + moveX, v[5] + moveY)
			end
		end
	end,
	OnStart = function(JBPShakeAnim, table)
		if not JBPShakeAnim.WorldFramePoints then
			JBPShakeAnim.WorldFramePoints = {}
			for i = 1, WorldFrame:GetNumPoints() do
				JBPShakeAnim.WorldFramePoints[i] = { WorldFrame:GetPoint(i) }
			end
		end
	end,
	OnStop = function(JBPShakeAnim, table)
		WorldFrame:ClearAllPoints()
		for _, v in pairs(JBPShakeAnim.WorldFramePoints) do
...
Animations:RegisterEventHandlerDataNonSpecific(11, "SCREENFLASH", {
	...
	Play = function(icon, eventSettings)
		...
		if not UIParent.JBPFlashAnim then
			UIParent.JBPFlashAnim = JBP.Classes.AnimatedObject:New()
		end

		local c = JBP:StringToCachedRGBATable(eventSettings.AnimColor)
		UIParent.JBPFlashAnim:Animations_Start{
			-- This lets us find the correct proxy for non-icon-based animations.
			-- Otherwise, the condition object is proxied to the icon but the 
			-- animation table is proxied to UIParent.JBPFlashAnim.
			proxyTarget = icon,
	...
	OnUpdate = function(JBPFlashAnim, table)
		local FlashPeriod = table.Period
		local animation_flasher = JBPFlashAnim.animation_flasher

		local timePassed = JBP.time - table.Start
		...
		if timePassed > table.Duration then
			JBPFlashAnim:Animations_Stop(table)
		end
	end,
	OnStart = function(JBPFlashAnim, table)
		local animation_flasher
		if JBPFlashAnim.animation_flasher then
			animation_flasher = JBPFlashAnim.animation_flasher
		else
			...
			JBPFlashAnim.animation_flasher = animation_flasher
		end

		animation_flasher:Show()
		animation_flasher:SetColorTexture(table.r, table.g, table.b, 1)
	end,
	OnStop = function(JBPFlashAnim, table)
		JBPFlashAnim.animation_flasher:Hide()
	end,
})
...
Animations:RegisterEventHandlerDataNonSpecific(70, "ICONBORDER", {
	...
	OnStart = function(icon, table)
		...
		for _, pos in JBP:Vararg("TOP", "BOTTOM", "LEFT", "RIGHT") do
...
Animations:RegisterEventHandlerDataNonSpecific(80, "ICONOVERLAYIMG", {
	...
	Play = function(icon, eventSettings)
		...
		icon:Animations_Start{
			...
			Image = JBP.COMMON.Textures:GetTexturePathFromSetting(eventSettings.Image),
...
local AnimatedObject = JBP:NewClass("AnimatedObject")
...
function AnimatedObject:Animations_Start(table)
	...
	if AnimationData then
		...
		JBP:Fire("JBP_ICON_ANIMATION_START", self, table)
	end
end
function AnimatedObject:Animations_Stop(arg1)
	...
	if AnimationData then
		...
		JBP:Fire("JBP_ICON_ANIMATION_STOP", self, table)
...
JBP.Classes.Icon:Inherit("AnimatedObject")

JBP:RegisterCallback("JBP_ICON_DISABLE", function(event, icon, soft)
	if not soft or not JBP.Locked then
...
JBP:RegisterCallback("JBP_ICON_ANIMATION_START", function(_, self, table)
	...
	if eventSettings.Event == "WCSP" then
		...
		elseif table.Duration then
			table.Duration = 5
			JBP:Print("Restricted animation duration to 5 seconds for testing")
		end
	end
end)
...
function Animations:DetermineNextPlayingAnimation(icon, Animation)
	...
	for i, eventSettings in JBP:InNLengthTable(icon.Events) do
